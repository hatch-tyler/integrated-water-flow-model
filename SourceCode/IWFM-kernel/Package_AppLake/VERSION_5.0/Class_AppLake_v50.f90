...
CONTAINS
  ...
  ! -------------------------------------------------------------
  SUBROUTINE AppLake_v50_SetModuleLogger(Logger)
    TYPE(MessageLoggerType), TARGET, INTENT(IN) :: Logger
    ModuleLogger => Logger
  END SUBROUTINE AppLake_v50_SetModuleLogger
...
! ******************************************************************
  ...
  ! -------------------------------------------------------------
  SUBROUTINE AppLake_v50_SetDynamicComponent(AppLake,IsForInquiry,cFileName,cWorkingDirectory,cPackageVersion,TimeStep,NTIME,AppGrid,LakeGWConnector,Precip,ET,iStat)
    ...
    !Read the the version number and make sure versions used in Preproccessor and Simulation are the same
    ...
    cLakeBudgetFileName = StripTextUntilCharacter(cLakeBudgetFileName,f_cInlineCommentChar) 
    ...
    CALL LakeDataFile%ReadData(ALine,iStat)  ;  IF (iStat .EQ. -1) RETURN  ;  ALine = ADJUSTL(StripTextUntilCharacter(ALine,f_cInlineCommentChar))  ;  CALL CleanSpecialCharacters(ALine)
    IF (ALine .NE. '') THEN
    ...
    DO indxLake=1,AppLake%NLakes
        ...
        END DO
        AppLake%Lakes(iLake)%cName = StripTextUntilCharacter(ADJUSTL(ALine),f_cInlineCommentChar)
    ...
    CALL LakeDataFile%ReadData(ALine,iStat)     ;  IF (iStat .EQ. -1) RETURN 
    CALL CleanSpecialCharacters(ALine)
    AppLake%cTimeUnitOutflow = ADJUSTL(StripTextUntilCharacter(ALine,f_cInlineCommentChar))
    IF (IsTimeIntervalValid(AppLake%cTimeUnitOutflow) .EQ. 0) THEN
        CALL AppLake%Logger%SetLastMessage('Time unit of lake outflow in the rating tables is not recognized!',f_iFatal,ThisProcedure)
        iStat = -1
        RETURN
    END IF
    ...
    DO indxLake=1,AppLake%NLakes
        ...
        CALL ConvertID_To_Index(ID,iLakeIDs,iLake)
        IF (iLake .EQ. 0) THEN 
            CALL AppLake%Logger%SetLastMessage('Lake ID '//TRIM(IntToText(ID))//' listed for rating tables is not recognized!',f_iFatal,ThisProcedure)
            iStat = -1
            RETURN
        END IF
        ...
        !Make sure lake data was not entered previously
        IF (lProcessed(iLake)) THEN
            CALL AppLake%Logger%SetLastMessage('Rating table for lake '//TRIM(IntToText(ID))//' is entered more than once!',f_iFatal,ThisProcedure)
            iStat = -1
            RETURN
        END IF
        lProcessed(iLake) = .TRUE.
        ...
        END DO
        CALL AppLake%OutflowRatingTables(iLake)%New(AppLake%Logger,iNPoints,rLakeElevs,rLakeOutflows,iStat) 
        IF (iStat .EQ. -1) RETURN
  ...
  ! -------------------------------------------------------------
  SUBROUTINE AppLake_v50_SetAllComponents(AppLake,IsForInquiry,cFileName,cSimWorkingDirectory,cPackageVersion,TimeStep,NTIME,AppGrid,BinFile,LakeGWConnector,Precip,ET,iStat)
    CLASS(AppLake_v50_Type),INTENT(OUT) :: AppLake
    LOGICAL,INTENT(IN)                  :: IsForInquiry
  ...
  ! -------------------------------------------------------------
  SUBROUTINE AppLake_v50_SetAllComponentsWithoutBinFile(AppLake,IsForInquiry,cPPFileName,cSimFileName,cSimWorkingDirectory,cPackageVersion,AppGrid,Stratigraphy,TimeStep,NTIME,StrmLakeConnector,LakeGWConnector,Precip,ET,iStat)
    CLASS(AppLake_v50_Type),INTENT(OUT)   :: AppLake
    LOGICAL,INTENT(IN)                    :: IsForInquiry
