...
! ******************************************************************
  ...
  ! -------------------------------------------------------------
  TYPE,EXTENDS(GenericLandUse_v41_Type) :: NonPondedAg_v41_Type
  ...
  CONTAINS
      ...
      PROCEDURE,PASS   :: ComputeETFromGW_Max => NonPondedAgLandUse_ComputeETFromGW_Max
      PROCEDURE,PASS   :: RewindTSInputFilesToTimeStamp
  END TYPE NonPondedAgLandUse_v41_Type
...
! ******************************************************************
  ...
  SUBROUTINE New(AgLand,IsForInquiry,cProjectNameForDSS,cFileName,cWorkingDirectory,FactCN,AppGrid,iElemIDs,TimeStep,NTimeSteps,cVersion,iStat,pAgLWUseBudRawFile_New,pAgRootZoneBudRawFile_New,iColCropCoeff)
    ...
    IF (ErrorCode+iStat .NE. 0) THEN
        CALL ModuleLogger%SetLastMessage('Error in allocating memory for non-ponded agricultural data!',f_iFatal,ThisProcedure)
        iStat = -1
        RETURN
    END IF
    ...
    AgLand%MaxRootDepth            = 0.0
    AgLand%iColRootDepthFrac       = 0.0
    AgLand%RootDepth_P             = 0.0
    ...
    DO indxCrop=1,NCrops
      ...
      ELSE
        CALL ModuleLogger%SetLastMessage('Too many or too few characters for crop code of crop ' // TRIM(IntToText(indxCrop)) // '!',f_iFatal,ThisProcedure)
        iStat = -1
        RETURN
      END IF
      ...
      DO indxCropP=1,indxCrop-1
        IF (AgLand%CropCodes(indxCrop) .EQ. AgLand%CropCodes(indxCropP)) THEN
            CALL ModuleLogger%SetLastMessage('Crop code '//TRIM(AgLand%CropCodes(indxCrop))//' is defined twice!',f_iFatal,ThisProcedure)
            iStat = -1
            RETURN
        END IF
      END DO
    ...
    IF (NBudgetCrops .GT. 0) THEN
      ...
      IF (ErrorCode .NE. 0) THEN
          CALL ModuleLogger%SetLastMessage('Error in allocating memory for non-ponded crops budget output data!',f_iFatal,ThisProcedure)
          iStat = -1
          RETURN
      END IF
      ...
      DO indxCrop=1,NBudgetCrops
        CALL CropDataFile%ReadData(ALine,iStat)  ;  IF (iStat .EQ. -1) RETURN  ;  cBudgetCropCode = ADJUSTL(StripTextUntilCharacter(ALine,f_cInlineCommentChar))  ;  CALL CleanSpecialCharacters(cBudgetCropCode)
        lCropFound = .FALSE.
        ...
        IF (.NOT. lCropFound) THEN
            CALL ModuleLogger%SetLastMessage(TRIM(cBudgetCropCode)//' for water budget output is not defined as a non-ponded crop!',f_iFatal,ThisProcedure)
            iStat = -1
            RETURN
        END IF
      END DO
    ...
    DO indxElem=1,NElements
        ...
        IF (lProcessed(iElem)) THEN
            ID = iElemIDs(iElem)
            CALL ModuleLogger%SetLastMessage('Curve numbers for non-ponded crops at element '//TRIM(IntToText(ID))//' are defined more than once!',f_iFatal,ThisProcedure)
            iStat = -1
            RETURN
        END IF
    ...
    DO indxElem=1,NElements
        ...
        IF (lProcessed(iElem)) THEN
            ID = iElemIDs(iElem)
            CALL ModuleLogger%SetLastMessage('Evapotranspration column pointers for non-ponded crops at element '//TRIM(IntToText(ID))//' are defined more than once!',f_iFatal,ThisProcedure)
            iStat = -1
        END IF
        lProcessed(iElem)             = .TRUE.
    ...
    !If requested, read crop coefficient data pointer
    IF (PRESENT(iColCropCoeff)) THEN
        ALLOCATE (iColCropCoeff(NCrops,NElements))
        CALL InitColumnPointerData_ForElements(CropDataFile,'crop coefficient column pointers for non-ponded crops',NCrops,iElemIDs,iColCropCoeff,iStat)
    ...
    DO indxElem=1,NElements
        ...
        IF (lProcessed(iElem)) THEN
            ID = iElemIDs(iElem)
            CALL ModuleLogger%SetLastMessage('Water supply requirement column pointers for non-ponded crops at element '//TRIM(IntToText(ID))//' are defined more than once!',f_iFatal,ThisProcedure)
            iStat = -1
            RETURN
        END IF
        lProcessed(iElem)            = .TRUE.
    ...
    DO indxElem=1,NElements
        ...
        IF (lProcessed(iElem)) THEN
            ID = iElemIDs(iElem)
            CALL ModuleLogger%SetLastMessage('Irrigation period column pointers for non-ponded crops at element '//TRIM(IntToText(ID))//' are defined more than once!',f_iFatal,ThisProcedure)
            iStat = -1
            RETURN
        END IF
        lProcessed(iElem)                    = .TRUE.
    ...
    DO indxElem=1,NElements
        ...
        IF (lProcessed(iElem)) THEN
            ID = iElemIDs(iElem)
            CALL ModuleLogger%SetLastMessage('Minimum soil moisture column pointers for non-ponded crops at element '//TRIM(IntToText(ID))//' are defined more than once!',f_iFatal,ThisProcedure)
            iStat = -1
            RETURN
        END IF
        lProcessed(iElem)                  = .TRUE.
    ...
    IF (ALine .NE. '') THEN
        ...
        DO indxElem=1,NElements
            ...
            IF (lProcessed(iElem)) THEN
                ID = iElemIDs(iElem)
                CALL ModuleLogger%SetLastMessage('Irrigation target soil moisture column pointers for non-ponded crops at element '//TRIM(IntToText(ID))//' are defined more than once!',f_iFatal,ThisProcedure)
                iStat = -1
                RETURN
            END IF
            lProcessed(iElem)                     = .TRUE.
    ...
    CALL ReadPointerData(CropDataFile,'return flow factor column pointers for non-ponded crops','elements',NElements,NCrops+1,iElemIDs,DummyIntArray,iStat)  ;  IF (iStat .EQ. -1) RETURN
    lProcessed = .FALSE.
    DO indxElem=1,NElements
        iElem = DummyIntArray(indxElem,1)
        IF (lProcessed(iElem)) THEN
            ID = iElemIDs(iElem)
            CALL ModuleLogger%SetLastMessage('Return flow fraction column pointers for non-ponded crops at element '//TRIM(IntToText(ID))//' are defined more than once!',f_iFatal,ThisProcedure)
            iStat = -1
            RETURN
        END IF
        lProcessed(iElem)                    = .TRUE.
    ...
    CALL ReadPointerData(CropDataFile,'re-use factor column pointers for non-ponded crops','elements',NElements,NCrops+1,iElemIDs,DummyIntArray,iStat)  ;  IF (iStat .EQ. -1) RETURN
    lProcessed = .FALSE.
    DO indxElem=1,NElements
        iElem = DummyIntArray(indxElem,1)
        IF (lProcessed(iElem)) THEN
            ID = iElemIDs(iElem)
            CALL ModuleLogger%SetLastMessage('Re-use flow fraction column pointers for non-ponded crops at element '//TRIM(IntToText(ID))//' are defined more than once!',f_iFatal,ThisProcedure)
            iStat = -1
            RETURN
        END IF
        lProcessed(iElem)                   = .TRUE.
    ...
    IF (ALine .NE. '') THEN
        ...
        DO indxElem=1,NElements
            ...
            IF (lProcessed(iElem)) THEN
                ID = iElemIDs(iElem)
                CALL ModuleLogger%SetLastMessage('Minimum percolation column pointers for non-ponded crops at element '//TRIM(IntToText(ID))//' are defined more than once!',f_iFatal,ThisProcedure)
                iStat = -1
                RETURN
            END IF
            lProcessed(iElem)                   = .TRUE.
    ...
    DO indxElem=1,NElements
        ...
        IF (lProcessed(iElem)) THEN
            ID = iElemIDs(iElem)
            CALL ModuleLogger%SetLastMessage('Initial conditions for non-ponded crops at element '//TRIM(IntToText(ID))//' are defined more than once!',f_iFatal,ThisProcedure)
            iStat = -1
            RETURN
        END IF
        lProcessed(iElem)                  = .TRUE.
...
! ******************************************************************
  ...
  SUBROUTINE Kill(AgLand)
    ...
    !Local variables
    INTEGER                           :: ErrorCode
    TYPE(NonPondedAgLandUse_v41_Type) :: Dummy
    ...
    SELECT TYPE (AgLand)
        TYPE IS (NonPondedAgLandUse_v41_Type)
            AgLand = Dummy
    END SELECT
...
! ******************************************************************
  ...
  SUBROUTINE GetBudget_TSData(AgLand,iBudgetType,iLocationIndex,iCols,cBeginDate,cEndDate,cInterval,rFactLT,rFactAR,rFactVL,rOutputDates,rOutputValues,iDataTypes,inActualOutput,iStat)
    CLASS(NonPondedAgLandUse_v41_Type),TARGET,INTENT(IN) :: AgLand
  ...
  SUBROUTINE GetMaxAndMinNetReturnFlowFrac(AgLand,ReturnFracFile,ReuseFracFile,FirstTimeStep,rMaxFrac,rMinFrac,iStat)
    CLASS(NonPondedAgLandUse_v41_Type),INTENT(IN) :: AgLand
...
! ******************************************************************
  ...
  SUBROUTINE ReadTSData(AgLand,TimeStep,AppGrid,IrigPeriodFile,iElemIDs,rElemAreas,WiltingPoint,FieldCapacity,iStat)
    ...
    !Local variables
    CHARACTER(LEN=ModNameLen+10),PARAMETER :: ThisProcedure = ModName // 'ReadTSData'
...
! ******************************************************************
  ...
  SUBROUTINE PrintRestartData(NonPondedAg,OutFile)
    CLASS(NonPondedAgLandUse_v41_Type),INTENT(IN) :: NonPondedAg
  ...
  SUBROUTINE WriteLWUseFlowsToBudRawFile(AppGrid,RLUArea,RPump,RDeli,RUpstrmElemRunoff,NonPondedAg)
    ...
    TYPE(NonPondedAgLandUse_v41_Type) :: NonPondedAg
    ...
    DO indxCrop=1,NBudgetCrops
      ...
      ElemValue                                  = NonPondedAg%Crops%ETFromGW_Actual(iBudgetCrops(indxCrop),:)
  ...
  SUBROUTINE WriteRootZoneFlowsToBudRawFile(AppGrid,RLUArea,RPump,RDeli,RGenMoistInflow,RUpstrmElemRunoff,ElemPrecip,Area,NonPondedAg)
   ...
   REAL(8),INTENT(IN)                :: Area(AppGrid%NElements,NonPondedAg%NBudgetCrops)
    ...
    !Local variables
    INTEGER                                                             :: NBudgetCrops,indxCrop,indxLast,iBudgetCrops(NonPondedAg%NBudgetCrops),iCrop
    ...
    REAL(8),DIMENSION((AppGrid%NSubregions+1)*NonPondedAg%NBudgetCrops) :: RRunoff,RPrecip,RReuse,RReturn,RSoilMCh,RInfilt,   &
                                                                           RETa,RPerc,Error,RSoilM,RSoilM_P,RGWInflow,RETPot
    ...
    DO indxCrop=1,NBudgetCrops
      ...
      ElemValue                                       = NonPondedAg%Crops%ETFromGW_Actual(iCrop,:) 
...
! ******************************************************************
  ...
  SUBROUTINE Simulate(NonPondedAg,AppGrid,ETData,DeltaT,Precip,GenericMoisture,SoilsData,ElemSupply,ReuseFrac,ReturnFrac,ElemsToGW,SolverData,lLakeElem,iStat,iColCropCoeff)
    ...
    REAL(8)                               :: AchievedConv,Area,ETc(NonPondedAg%NCrops),HydCond,TotalPorosity,fRU, &
                                             ...
                                             ETc_effect,rRunoff,rReturnFlow,rPrecipInfilt,rIrigInfilt,rETa,rETAW, &
                                             rETP,rETOth,rPerc,rReuse,rGMExcess,rETFromGW_Actual,rSoilM_Precip_P, &
                                             rSoilM_AW_P,rSoilM_Oth_P
    ...
    ASSOCIATE (pCrops => NonPondedAg%Crops)
        ...
        DO indxElem=1,AppGrid%NElements
            ...
            DO indxCrop=1,NonPondedAg%NCrops
                ...
                !ET from GW
                rETFromGW_Actual = MIN(ETc(indxCrop)*DeltaT , pCrops%ETFromGW_Max(indxCrop,indxElem))
                ETc_effect       = ETc(indxCrop)*DeltaT - rETFromGW_Actual
  ...
  SUBROUTINE ComputeWaterDemand(NonPondedAg,AppGrid,ETData,DeltaT,Precip,GenericMoisture,SoilsData,SpecifiedDemand,ReuseFrac,ReturnFrac,IrigPeriod,SolverData,lLakeElem,lReadAgWaterDemand,iStat,iColCropCoeff)
    ...
    DO indxElem=1,AppGrid%NElements
        ...
        DO indxCrop=1,NonPondedAg%NCrops              
            ...
            RootDepth_P = NonPondedAg%RootDepth_P(indxCrop)
            GM          = GMElem * RootDepth
            ETc_effect  = ETc(indxCrop)*DeltaT - MIN(ETc(indxCrop)*DeltaT , NonPondedAg%Crops%ETFromGW_Max(indxCrop,indxElem))
            IF (RootDepth .GT. RootDepth_P) THEN
                RootDepthFrac = 1d0 
            ELSE
                RootDepthFrac = RootDepth / RootDepth_P
...
! ******************************************************************
  ...
  ! -------------------------------------------------------------
  SUBROUTINE NonPondedAgLandUse_ComputeETFromGW_Max(AgLand,DepthToGW,Sy,CapillaryRise)
    CLASS(NonPondedAgLandUse_v41_Type) :: AgLand
  ...
  SUBROUTINE SoilMContent_To_Depth(NonPondedAgLand,NElements,iElemIDs,TotalPorosity,iStat)
    CLASS(NonPondedAgLandUse_v41_Type) :: NonPondedAgLand
