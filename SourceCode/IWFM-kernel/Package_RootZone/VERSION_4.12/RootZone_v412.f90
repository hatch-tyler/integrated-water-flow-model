...
! ******************************************************************
  ...
  INTEGER,PARAMETER           :: f_iNLWUseBudColumns                                   = 15  , &
                                 ...
                                 f_iNRootZoneBudColumns                                = 61  , &
                                 f_iNAgRootZoneBudColumns_NonPondedCrops               = 20  , &
                                 f_iNAgRootZoneBudColumns_PondedCrops                  = 23  , &
                                 f_iNRootZoneZBudColumns                               = 104  
  ...
  CHARACTER(LEN=53),PARAMETER :: f_cRootZoneBudgetColumnTitles(f_iNRootZoneBudColumns) = ['Ag. Area'                                               , &
                                                                                          ...
                                                                                          'Ag. Beginning Moisture Storage (+)'                     , &
                                                                                          'Ag. Beginning Pond Storage (+)'                         , &
                                                                                          ...
                                                                                          'Ag. Actual ET (-)'                                      , &
                                                                                          'Ag. Percolation (-)'                                    , &
                                                                                          'Ag. Ending Moisture Storage (-)'                        , &
                                                                                          'Ag. Ending Pond Storage (-)'                            , &
                                                                                          'Ag. Discrepancy (=)'                                    , &
                                                                                          'Urban Area'                                             , &
                                                                                          'Urban Potential ET'                                     , &
                                                                                          'Urban Precipitation'                                    , &
  ...
  CHARACTER(LEN=54),PARAMETER :: f_cRootZoneZBudgetColumnTitles(f_iNRootZoneZBudColumns) = ['Non-ponded Ag. Area'                                    , &
                                                                                            ...
                                                                                            'Rice Beginning Moisture Storage (+)'                     , &
                                                                                            'Rice Beginning Pond Storage (+)'                         , &
                                                                                            ...
                                                                                            'Rice Actual ET (-)'                                     , &
                                                                                            'Rice Percolation (-)'                                   , &
                                                                                            'Rice Ending Moisture Storage (-)'                       , &
                                                                                            'Rice Ending Pond Storage (-)'                           , &
                                                                                            'Rice Discrepancy (=)'                                   , &
                                                                                            'Refuge Area'                                            , &
                                                                                            'Refuge Potential ET'                                    , &
                                                                                            ...
                                                                                            'Refuge Beginning Moisture Storage (+)'                  , &
                                                                                            'Refuge Beginning Pond Storage (+)'                      , &
                                                                                            ...
                                                                                            'Refuge Actual ET (-)'                                   , &
                                                                                            'Refuge Percolation (-)'                                 , &
                                                                                            'Refuge Ending Moisture Storage (-)'                     , &
                                                                                            'Refuge Ending Pond Storage (-)'                         , &
                                                                                            'Refuge Discrepancy (=)'                                 , &
                                                                                            'Urban Area'                                             , &
                                                                                            'Urban Potential ET'                                     , &
                                                                                            'Urban Precipitation'                                    , &
...
! ******************************************************************
  ...
  ! --- NEW ROOT ZONE DATA
  ! -------------------------------------------------------------
  SUBROUTINE RootZone_v412_New(RootZone,IsForInquiry,cProjectNameForDSS,cFileName,cCropCoeffFileName,cWorkingDirectory,AppGrid,TimeStep,NTIME,ET,Precip,iStat,iStrmNodeIDs,iLakeIDs)
    ...
    CHARACTER(LEN=1000)                         :: cALine,NonPondedCropFile,RiceRefugeFile,UrbanDataFile,NVRVFile,AgWaterDemandFile,GenericMoistureFile
    ...
    LOGICAL                                     :: TrackTime,lProcessed(AppGrid%NElements),lLUAreaScaleOutFileDefined
    ...
    !Initialize
    cVersionFull                       = 'v4.12-' // TRIM(IWFMKernelVersion%GetVersion())
    ...
    ALLOCATE (RootZone%ElemSoilsData(NElements)                          , &
              ...
              RootZone%RSoilM_P(NRegion+1,3)                             , &  !Includes pond storage; 2nd Dim: 1 = Ag; 2 = Urban; 3 = Native & Riparian Veg
              RootZone%RSoilM(NRegion+1,3)                               , &  !Includes pond storage; 2nd Dim: 1 = Ag; 2 = Urban; 3 = Native & Riparian Veg
    ...
    CALL RootZone%NonPondedAgRootZone%New(RootZone%Logger,IsForInquiry,cProjectNameForDSS,cAbsPathFileName,cWorkingDirectory,FactCN,AppGrid,iElemIDs,TimeStep,NTIME,TRIM(cVersionFull),iStat,pAgLWUseBudRawFile_New,pNonPondedAgRootZoneBudRawFile_New)
    IF (iStat .EQ. -1) RETURN
    ...
    CALL RootZone%PondedAgRootZone%New(RootZone%Logger,IsForInquiry,.FALSE.,cProjectNameForDSS,cAbsPathFileName,cWorkingDirectory,FactCN,AppGrid,iElemIDs,TimeStep,NTIME,TRIM(cVersionFull),iStat,pAgLWUseBudRawFile_New,pPondedAgRootZoneBudRawFile_New)
    IF (iStat .EQ. -1) RETURN
    ...
    IF (cALine .NE. '') THEN
        CALL EstablishAbsolutePathFileName(TRIM(ADJUSTL(cALine)),cWorkingDirectory,cAbsPathFileName)
        CALL LWUseZoneBudRawFile_New(RootZone%Logger,IsForInquiry,cAbsPathFileName,TimeStep,NTIME,TRIM(cVersionFull),RootZone%Flags,AppGrid,RootZone%LWUZoneBudRawFile,iStat)
        IF (iStat .EQ. -1) RETURN
    ...
    IF (cALine .NE. '') THEN
        CALL EstablishAbsolutePathFileName(TRIM(ADJUSTL(cALine)),cWorkingDirectory,cAbsPathFileName)
        CALL RootZoneZoneBudRawFile_New(RootZone%Logger,IsForInquiry,cAbsPathFileName,TimeStep,NTIME,TRIM(cVersionFull),RootZone%Flags,AppGrid,RootZone%RootZoneZoneBudRawFile,iStat)
        IF (iStat .EQ. -1) RETURN
    ...
    CALL CleanSpecialCharacters(cALine)
    IF (cALine .EQ. '') THEN
        CALL RootZone%Logger%SetLastMessage('Surface flow destinations data file is missing for the Root Zone component!',f_iFatal,ThisProcedure)
        iStat = -1
        RETURN
    ELSE
        CALL EstablishAbsolutePathFileName(TRIM(ADJUSTL(cALine)),cWorkingDirectory,cAbsPathFileName)
  ...
  ! -------------------------------------------------------------
  SUBROUTINE LWUseZoneBudRawFile_New(Logger,IsForInquiry,cFileName,TimeStep,NTIME,cVersion,Flags,AppGrid,ZBudFile,iStat)
    TYPE(MessageLoggerType),TARGET,INTENT(INOUT) :: Logger
    LOGICAL,INTENT(IN)            :: IsForInquiry
  ...
  SUBROUTINE RootZoneBudRawFile_New(IsForInquiry,cProjectNameForDSS,cFileName,TimeStep,NTIME,NRegion,RegionArea,cRegionNames,cDescriptor,cVersion,RawFile,iStat,Logger)
    ...
    CHARACTER(LEN=6)       :: CParts(f_iNRootZoneBudColumns) = ['AREA'   , &   !Ag. Area                                                
                                                                ...
                                                                'VOLUME' , &   !Ag. Beginning Moisture Storage (+)                               
                                                                'VOLUME' , &   !Ag. Beginning Pond Storage (+)                               
                                                                ...
                                                                'VOLUME' , &   !Ag. Ending Moisture Storage (-)                                  
                                                                'VOLUME' , &   !Ag. Ending Pond Storage (-)                                  
    ...
    CHARACTER(LEN=15)      :: FParts(f_iNRootZoneBudColumns) = ['AG_AREA'            ,&      !Ag. Area                                                                            
                                                                ...
                                                                'AG_BGN_MS_STOR'     ,&      !Ag. Beginning Moisture Storage (+)                               
                                                                'AG_BGN_PD_STOR'     ,&      !Ag. Beginning Pond Storage (+)                               
                                                                ...
                                                                'AG_END_MS_STOR'     ,&      !Ag. Ending Moisture Storage (-)                                  
                                                                'AG_END_PD_STOR'     ,&      !Ag. Ending Pond Storage (-)                                  
                                                                ...
                                                                'URB_BGN_STOR'       ,&      !Urban Beginning Storage (+)                             
                                                                ...
                                                                'NRV_BGN_STOR'       ,&      !Native&Riparian Veg. Beginning Storage (+)              
    ...
    ASSOCIATE (pASCIIOutput => OutputData%ASCIIOutput)
        ...
        pASCIIOutput%NTitles  = f_iNTitles
          ...
          pASCIIOutput%cTitles(3)         = ArrangeText('SUBREGION AREA: '//f_cAreaMarker_Budget//' '//f_cAreaUnitMarker_Budget , pASCIIOutput%TitleLen)
          pASCIIOutput%cTitles(4)         = REPEAT('-',pASCIIOutput%TitleLen)
          pASCIIOutput%cTitles(5)         = REPEAT(' ',182)//'Agricultural Area'//REPEAT(' ',297)//'Urban Area'//REPEAT(' ',266)//'Native & Riparian Vegetation Area'
          pASCIIOutput%cTitles(6)         = REPEAT(' ',17)//REPEAT('-',346)//REPEAT(' ',3)//REPEAT('-',300)//REPEAT(' ',3)//REPEAT('-',270)
          pASCIIOutput%lTitlePersist(1:3) = .TRUE.
          pASCIIOutput%lTitlePersist(4:6) = .FALSE.
        pASCIIOutput%cFormatSpec        = ADJUSTL('(A16,1X,23(F14.1,1X),3X,20(F14.1,1X),3X,18(F14.1,1X))')
        pASCIIOutput%NColumnHeaderLines = f_iNColumnHeaderLines
    END ASSOCIATE 
    ...
    ASSOCIATE (pLocation => OutputData%Locations(1))
        ...
        pLocation%cFullColumnHeaders(25) =  'Urban Area ('//f_cAreaUnitMarker_Budget//')'
        pLocation%cFullColumnHeaders(45) =  'Native&Riparian Veg. Area ('//f_cAreaUnitMarker_Budget//')'
        pLocation%iDataColumnTypes  =  [f_iAR_Budget ,&  !Ag area
                                        ...
                                        f_iVLB_Budget,&  !Ag beginning moisture storage
                                        f_iVLB_Budget,&  !Ag beginning pond storage
                                        ...
                                        f_iVLE_Budget,&  !Ag ending moisture storage
                                        f_iVLE_Budget,&  !Ag ending pond storage
        ...
        ASSOCIATE (pColumnHeaders => pLocation%cColumnHeaders           , &
            ...
            pColumnHeaders(:,4) = ''
            pFormatSpecs(1)     = '(A17,A14,A15,A16,20A15,3X,A14,A15,A16,17A15,3X,A14,A15,A16,15A15)'
            pFormatSpecs(2)     = '(A17,A14,A15,A16,20A15,3X,A14,A15,A16,17A15,3X,A14,A15,A16,15A15)'
            pFormatSpecs(3)     = '(A17,A14,A15,A16,20A15,3X,A14,A15,A16,17A15,3X,A14,A15,A16,15A15)'
    ...
    ASSOCIATE (pDSSOutput => OutputData%DSSOutput)
        ...
        DO indxLocation=1,NRegion
            ...
            END DO
        END DO
        pDSSOutput%iDataTypes = [f_iPER_AVER,(f_iPER_CUM,indxCol=1,22),f_iPER_AVER,(f_iPER_CUM,indxCol=1,19),f_iPER_AVER,(f_iPER_CUM,indxCol=1,17)]
    END ASSOCIATE
  ...
  ! -------------------------------------------------------------
  SUBROUTINE RootZoneZoneBudRawFile_New(Logger,IsForInquiry,cFileName,TimeStep,NTIME,cVersion,Flags,AppGrid,ZBudFile,iStat)
    TYPE(MessageLoggerType),TARGET,INTENT(INOUT) :: Logger
    LOGICAL,INTENT(IN)            :: IsForInquiry
    CHARACTER(LEN=*),INTENT(IN)   :: cFileName
    TYPE(TimeStepType),INTENT(IN) :: TimeStep
    ...
    Header%iDataTypes = [f_iAR_ZBudget ,&  !Non-ponded ag area                                          1
                         ...
                         f_iVLB_ZBudget,&  !Rice beginning moisture storage                             32
                         f_iVLB_ZBudget,&  !Rice beginning pond storage                                 33
                         f_iVR_ZBudget ,&  !Rice net gain from land expansion                           34
                         f_iVR_ZBudget ,&  !Rice infiltration                                           35
                         f_iVR_ZBudget ,&  !Rice groundwater inflow                                     36
                         f_iVR_ZBudget ,&  !Rice generic inflow                                         37
                         f_iVR_ZBudget ,&  !Rice pond drain                                             38
                         f_iVR_ZBudget ,&  !Rice actual ET                                              39
                         f_iVR_ZBudget ,&  !Rice perc                                                   40
                         f_iVLE_ZBudget,&  !Rice ending moisture storage                                41
                         f_iVLE_ZBudget,&  !Rice ending pond storage                                    42
                         f_iVR_ZBudget ,&  !Rice discrepancy                                            43
                         f_iAR_ZBudget ,&  !Refuge area                                                 44
                         f_iVR_ZBudget ,&  !Refuge potential ET                                         45
                         f_iVR_ZBudget ,&  !Refuge precipitation                                        46
                         f_iVR_ZBudget ,&  !Refuge runoff                                               47
                         f_iVR_ZBudget ,&  !Refuge prime applied water                                  48
                         f_iVR_ZBudget ,&  !Refuge re-used water                                        49
                         f_iVR_ZBudget ,&  !Refuge return flow                                          50
                         f_iVR_ZBudget ,&  !Refuge surface flow for gw recharge (Ag)                    51
                         f_iVR_ZBudget ,&  !Refuge surface flow for gw recharge (UrbIn)                 52
                         f_iVR_ZBudget ,&  !Refuge surface flow for gw recharge (UrbOut)                53
                         f_iVR_ZBudget ,&  !Refuge surface flow for gw recharge (NVRV)                  54
                         f_iVLB_ZBudget,&  !Refuge beginning moisture storage                           55
                         f_iVLB_ZBudget,&  !Refuge beginning pond storage                               56
                         f_iVR_ZBudget ,&  !Refuge net gain from land expansion                         57
                         f_iVR_ZBudget ,&  !Refuge infiltration                                         58
                         f_iVR_ZBudget ,&  !Refuge groundwater inflow                                   59
                         f_iVR_ZBudget ,&  !Refuge generic inflow                                       60
                         f_iVR_ZBudget ,&  !Refuge pond drain                                           61
                         f_iVR_ZBudget ,&  !Refuge actual ET                                            62
                         f_iVR_ZBudget ,&  !Refuge perc                                                 63
                         f_iVLE_ZBudget,&  !Refuge ending moisture storage                              64
                         f_iVLE_ZBudget,&  !Refuge ending pond storage                                  65
                         f_iVR_ZBudget ,&  !Refuge discrepancy                                          66
                         f_iAR_ZBudget ,&  !Urban area                                                  67
                         f_iVR_ZBudget ,&  !Urban potential ET                                          68
                         f_iVR_ZBudget ,&  !Urban precipitation                                         69
                         f_iVR_ZBudget ,&  !Urban runoff                                                70
                         f_iVR_ZBudget ,&  !Urban prime applied water                                   71
                         f_iVR_ZBudget ,&  !Urban re-used water                                         72
                         f_iVR_ZBudget ,&  !Urban return flow                                           73
                         f_iVR_ZBudget ,&  !Urban surface flow for gw recharge (Ag)                     74
                         f_iVR_ZBudget ,&  !Urban surface flow for gw recharge (UrbIn)                  75
                         f_iVR_ZBudget ,&  !Urban surface flow for gw recharge (UrbOut)                 76
                         f_iVR_ZBudget ,&  !Urban surface flow for gw recharge (NVRV)                   77
                         f_iVLB_ZBudget,&  !Urban beginning storage                                     78
                         f_iVR_ZBudget ,&  !Urban net gain from land expansion                          79
                         f_iVR_ZBudget ,&  !Urban infiltration                                          80
                         f_iVR_ZBudget ,&  !Urban groundwater inflow                                    81
                         f_iVR_ZBudget ,&  !Urban generic inflow                                        82
                         f_iVR_ZBudget ,&  !Urban actual ET                                             83
                         f_iVR_ZBudget ,&  !Urban perc                                                  84
                         f_iVLE_ZBudget,&  !Urban ending storage                                        85
                         f_iVR_ZBudget ,&  !Urban discrepancy                                           86
                         f_iAR_ZBudget ,&  !NV&RV area                                                  87
                         f_iVR_ZBudget ,&  !NV&RV potential ET                                          88
                         f_iVR_ZBudget ,&  !NV&RV precipitation                                         89
                         f_iVR_ZBudget ,&  !NV&RV runoff                                                90
                         f_iVR_ZBudget ,&  !NV&RV surface flow for gw recharge (Ag)                     91
                         f_iVR_ZBudget ,&  !NV&RV surface flow for gw recharge (UrbIn)                  92
                         f_iVR_ZBudget ,&  !NV&RV surface flow for gw recharge (UrbOut)                 93
                         f_iVR_ZBudget ,&  !NV&RV surface flow for gw recharge (NVRV)                   94
                         f_iVLB_ZBudget,&  !NV&RV beginning storage                                     95
                         f_iVR_ZBudget ,&  !NV&RV net gain from land expansion                          96
                         f_iVR_ZBudget ,&  !NV&RV infiltration                                          97
                         f_iVR_ZBudget ,&  !NV&RV groundwater inflow                                    98
                         f_iVR_ZBudget ,&  !NV&RV generic inflow                                        99
                         f_iVR_ZBudget ,&  !NV&RV stream inflow for RV ET                               100
                         f_iVR_ZBudget ,&  !NV&RV actual ET                                             101
                         f_iVR_ZBudget ,&  !NV&RV perc                                                  102
                         f_iVLE_ZBudget,&  !NV&RV ending storage                                        103
                         f_iVR_ZBudget ]   !NV&RV discrepancy                                           104
    Header%cFullDataNames     = f_cRootZoneZBudgetColumnTitles
    Header%cFullDataNames(1)  = TRIM(Header%cFullDataNames(1))  // cArea
    Header%cFullDataNames(21) = TRIM(Header%cFullDataNames(21)) // cArea
    Header%cFullDataNames(44) = TRIM(Header%cFullDataNames(44)) // cArea
    Header%cFullDataNames(67) = TRIM(Header%cFullDataNames(67)) // cArea
    Header%cFullDataNames(87) = TRIM(Header%cFullDataNames(87)) // cArea
    ...
    IF (Flags%lPondedAg_Defined) THEN
        !Rice
        Header%iNDataElems(21:43,:) = AppGrid%NElements
        DO indxElem=1,AppGrid%NElements
            Header%iElemDataColumns(indxElem,21:43,:) = indxElem
        END DO
        IF (.NOT. Flags%lComputeETFromGW) THEN
        ...
        END IF
        IF (.NOT. Flags%lGenericMoistureFile_Defined) THEN
            Header%iNDataElems(37,:)        = 0
            Header%iElemDataColumns(:,37,:) = 0
        END IF
        
        !Refuge
        Header%iNDataElems(44:66,:) = AppGrid%NElements
        DO indxElem=1,AppGrid%NElements
            Header%iElemDataColumns(indxElem,44:66,:) = indxElem
        END DO
        IF (.NOT. Flags%lComputeETFromGW) THEN
        ...
        END IF
        IF (.NOT. Flags%lGenericMoistureFile_Defined) THEN
            Header%iNDataElems(60,:)        = 0
            Header%iElemDataColumns(:,60,:) = 0
        END IF
    ELSE
        Header%iNDataElems(21:66,:)        = 0
        Header%iElemDataColumns(:,21:66,:) = 0
    END IF
    
    !Urban
    IF (Flags%lUrban_Defined) THEN
        Header%iNDataElems(67:86,:) = AppGrid%NElements
        DO indxElem=1,AppGrid%NElements
            Header%iElemDataColumns(indxElem,67:86,:) = indxElem
        END DO
        IF (.NOT. Flags%lComputeETFromGW) THEN
            Header%iNDataElems(81,:)        = 0
            Header%iElemDataColumns(:,81,:) = 0
        END IF
        IF (.NOT. Flags%lGenericMoistureFile_Defined) THEN
            Header%iNDataElems(82,:)        = 0
            Header%iElemDataColumns(:,82,:) = 0
        END IF
    ELSE
        Header%iNDataElems(67:86,:)        = 0
        Header%iElemDataColumns(:,67:86,:) = 0
    END IF
    
    !Native and riparian veg.
    IF (Flags%lNVRV_Defined) THEN
        Header%iNDataElems(87:,:) = AppGrid%NElements
        DO indxElem=1,AppGrid%NElements
            Header%iElemDataColumns(indxElem,87:,:) = indxElem
        END DO
        IF (.NOT. Flags%lComputeETFromGW) THEN
            Header%iNDataElems(98,:)        = 0
            Header%iElemDataColumns(:,98,:) = 0
        END IF
        IF (.NOT. Flags%lGenericMoistureFile_Defined) THEN
            Header%iNDataElems(99,:)        = 0
            Header%iElemDataColumns(:,99,:) = 0
        END IF
    ELSE
        Header%iNDataElems(87:,:)        = 0
        Header%iElemDataColumns(:,87:,:) = 0
    END IF

    !ASCII output titles
    Header%ASCIIOutput%iNTitles         = 5
    Header%ASCIIOutput%iLenColumnTitles = 1593
    ...
    Header%ASCIIOutput%cNumberFormat    = '(A16,20(2X,F13.1),3X,23(2X,F13.1),3X,23(2X,F13.1),3X,20(2X,F13.1),3X,18(2X,F13.1))'
  ...
  ! -------------------------------------------------------------
  ! --- NEW BINARY ROOT ZONE BUDGET FILE FOR POST-PROCESSING OF Non-PONDED AG LANDS
  ! -------------------------------------------------------------
  SUBROUTINE NonPondedAgRootZoneBudRawFile_New(IsForInquiry,cProjectNameForDSS,cFileName,TimeStep,NTIME,NRegion,RegionArea,cRegionNames,cDescriptor,cVersion,RawFile,iStat,Logger)
    LOGICAL,INTENT(IN)            :: IsForInquiry
    CHARACTER(LEN=*),INTENT(IN)   :: cProjectNameForDSS,cFileName,cRegionNames(NRegion)
    TYPE(TimeStepType),INTENT(IN) :: TimeStep
    INTEGER,INTENT(IN)            :: NTIME,NRegion
    ...
    INTEGER                :: indxCol,indxLocation,iCount
    CHARACTER              :: UnitT*10,Text*17,Text1*13
    CHARACTER(LEN=6)       :: CParts(f_iNAgRootZoneBudColumns_NonPondedCrops) = ['AREA'   , &
                                                                                 'VOLUME' , &
                                                                                 'VOLUME' , &
                                                                                 'VOLUME' , &
                                                                                 'VOLUME' , &
                                                                                 ...
                                                                                 'VOLUME' , &
                                                                                 'VOLUME' ]
    CHARACTER(LEN=11)      :: FParts(f_iNAgRootZoneBudColumns_NonPondedCrops) = ['AREA'           ,&
                                                                                 'POT_ET'         ,&   
                                                                                 'PRECIP'         ,&   
                                                                                 'RUNOFF'         ,&   
                                                                                 'PRM_H2O'        ,&
    ...
    ASSOCIATE (pASCIIOutput => OutputData%ASCIIOutput)
        ...
        pASCIIOutput%NTitles  = f_iNTitles
          ...
          pASCIIOutput%lTitlePersist(1:3) = .TRUE.
          pASCIIOutput%lTitlePersist(4)   = .FALSE.
        pASCIIOutput%cFormatSpec        = ADJUSTL('(A16,1X,*(F14.1,1X))')
        pASCIIOutput%NColumnHeaderLines = f_iNColumnHeaderLines
    END ASSOCIATE 
    ...
    !Locations
    ALLOCATE (OutputData%Locations(1)                                                                                 , &
              OutputData%Locations(1)%cFullColumnHeaders(f_iNAgRootZoneBudColumns_NonPondedCrops+1)                   , &
              OutputData%Locations(1)%iDataColumnTypes(f_iNAgRootZoneBudColumns_NonPondedCrops)                       , &
              OutputData%Locations(1)%iColWidth(f_iNAgRootZoneBudColumns_NonPondedCrops+1)                            , &
              OutputData%Locations(1)%cColumnHeaders(f_iNAgRootZoneBudColumns_NonPondedCrops+1,f_iNColumnHeaderLines) , &
              OutputData%Locations(1)%cColumnHeadersFormatSpec(f_iNColumnHeaderLines)               )
    ASSOCIATE (pLocation => OutputData%Locations(1))
        pLocation%NDataColumns       = f_iNAgRootZoneBudColumns_NonPondedCrops
        pLocation%cFullColumnHeaders =  ['Time'                                               , &
                                         'Area ('//f_cAreaUnitMarker_Budget//')'              , &
                                         'Potential ET'                                       , &
                                         'Precipitation'                                      , &
        ...
        pLocation%iDataColumnTypes  =  [f_iAR_Budget ,&  !Ag area
                                        ...
                                        f_iVLE_Budget,&  !Ag ending moisture storage
                                        f_iVR_Budget ]   !Ag discrepancy
        pLocation%iColWidth       = [17,14,15,16,(17,indxCol=1,17)]
        ASSOCIATE (pColumnHeaders => pLocation%cColumnHeaders           , &
                   pFormatSpecs   => pLocation%cColumnHeadersFormatSpec )
            Text                = ArrangeText(TRIM(UnitT),17)
            Text1               = '('//TRIM(f_cAreaUnitMarker_Budget)//')'
            pColumnHeaders(:,1) = ['                 ','              ','               ','                ','               ','       Prime   ','         Reused','          Net  ','  Surface Flow ','  Surface Flow ','  Surface Flow ','  Surface Flow ','     Beginning ',' Net Gain from ','               ','    Groundwater','        Other  ','         Actual','               ','        Ending ','               ']
            pColumnHeaders(:,2) = ['      Time       ','          Area','      Potential','   Precipitation','         Runoff','      Applied  ','         Water ','         Return','     to GW     ','     to GW     ','     to GW     ','     to GW     ',' Moist. Storage',' Land Expansion','   Infiltration','       Inflow  ','        Inflow ','           ET  ','    Percolation',' Moist. Storage','    Discrepancy']
            pColumnHeaders(:,3) = [               Text,           Text1,'         ET    ','                ','               ','       Water   ','               ','          Flow ','     (Ag)      ',' (Urban Indrs) ',' (Urban Outdrs)',' (Native Veg.) ','        (+)    ','       (+)     ','        (+)    ','         (+)   ','          (+)  ','           (-) ','       (-)     ','          (-)  ','        (=)    ']
            pColumnHeaders(:,4) = ''
            pFormatSpecs(1)     = '(A17,A14,A15,A16,17A15)'
            pFormatSpecs(2)     = '(A17,A14,A15,A16,17A15)'
            pFormatSpecs(3)     = '(A17,A14,A15,A16,17A15)'
            pFormatSpecs(4)     = '('//TRIM(IntToText(f_iTitleLen))//'(1H-),'//TRIM(IntToText(f_iNAgRootZoneBudColumns_NonPondedCrops+1))//'A0)'
        END ASSOCIATE
    END ASSOCIATE

    !Data for DSS output  
    ASSOCIATE (pDSSOutput => OutputData%DSSOutput)
        ALLOCATE (pDSSOutput%cPathNames(f_iNAgRootZoneBudColumns_NonPondedCrops*NRegion) , pDSSOutput%iDataTypes(f_iNAgRootZoneBudColumns_NonPondedCrops))
        iCount = 1
        DO indxLocation=1,NRegion
            DO indxCol=1,f_iNAgRootZoneBudColumns_NonPondedCrops
                pDSSOutput%cPathNames(iCount) = '/'//TRIM(UpperCase(cProjectNameForDSS))//'_ROOTZN_BUD/'       //  &  !A part
                                                TRIM(UpperCase(OutputData%cLocationNames(indxLocation)))//'/'  //  &  !B part
                                                TRIM(CParts(indxCol))//'/'                                     //  &  !C part
                                                '/'                                                            //  &  !D part
            ...
            END DO
        END DO
        pDSSOutput%iDataTypes = [f_iPER_AVER,(f_iPER_CUM,indxCol=1,19)]
    END ASSOCIATE
  ...
  ! -------------------------------------------------------------
  ! --- NEW BINARY ROOT ZONE BUDGET FILE FOR POST-PROCESSING OF PONDED AG LANDS
  ! -------------------------------------------------------------
  SUBROUTINE PondedAgRootZoneBudRawFile_New(IsForInquiry,cProjectNameForDSS,cFileName,TimeStep,NTIME,NRegion,RegionArea,cRegionNames,cDescriptor,cVersion,RawFile,iStat,Logger)
    LOGICAL,INTENT(IN)            :: IsForInquiry
    CHARACTER(LEN=*),INTENT(IN)   :: cProjectNameForDSS,cFileName,cRegionNames(NRegion)
    TYPE(TimeStepType),INTENT(IN) :: TimeStep
    INTEGER,INTENT(IN)            :: NTIME,NRegion
    ...
    INTEGER                :: indxCol,indxLocation,iCount
    CHARACTER              :: UnitT*10,Text*17,Text1*13
    CHARACTER(LEN=6)       :: CParts(f_iNAgRootZoneBudColumns_PondedCrops) = ['AREA'   , &
                                                                              'VOLUME' , &
                                                                              'VOLUME' , &
                                                                              'VOLUME' , &
                                                                              'VOLUME' , &
                                                                              ...
                                                                              'VOLUME' , &
                                                                              'VOLUME' ]
    CHARACTER(LEN=11)      :: FParts(f_iNAgRootZoneBudColumns_PondedCrops) = ['AREA'           ,&
                                                                              'POT_ET'         ,&   
                                                                              'PRECIP'         ,&   
                                                                              'RUNOFF'         ,&   
                                                                              'PRM_H2O'        ,&
    ...
    !Locations
    ALLOCATE (OutputData%Locations(1)                                                                              , &
              OutputData%Locations(1)%cFullColumnHeaders(f_iNAgRootZoneBudColumns_PondedCrops+1)                   , &
              OutputData%Locations(1)%iDataColumnTypes(f_iNAgRootZoneBudColumns_PondedCrops)                       , &
              OutputData%Locations(1)%iColWidth(f_iNAgRootZoneBudColumns_PondedCrops+1)                            , &
              OutputData%Locations(1)%cColumnHeaders(f_iNAgRootZoneBudColumns_PondedCrops+1,f_iNColumnHeaderLines) , &
              OutputData%Locations(1)%cColumnHeadersFormatSpec(f_iNColumnHeaderLines)               )
    ASSOCIATE (pLocation => OutputData%Locations(1))
        pLocation%NDataColumns       = f_iNAgRootZoneBudColumns_PondedCrops
        pLocation%cFullColumnHeaders =  ['Time'                                               , &
                                         'Area ('//f_cAreaUnitMarker_Budget//')'              , &
                                         'Potential ET'                                       , &
                                         'Precipitation'                                      , &
        ...
        pLocation%iDataColumnTypes  =  [f_iAR_Budget ,&  !Ag area
                                        ...
                                        f_iVLE_Budget,&  !Ag ending pond storage
                                        f_iVR_Budget ]   !Ag discrepancy
        pLocation%iColWidth       = [17,14,15,16,(17,indxCol=1,18)]
        ASSOCIATE (pColumnHeaders => pLocation%cColumnHeaders           , &
                   pFormatSpecs   => pLocation%cColumnHeadersFormatSpec )
            Text                = ArrangeText(TRIM(UnitT),17)
            Text1               = '('//TRIM(f_cAreaUnitMarker_Budget)//')'
            pColumnHeaders(:,1) = ['                 ','              ','               ','                ','               ','       Prime   ','         Reused','          Net  ','  Surface Flow ','  Surface Flow ','  Surface Flow ','  Surface Flow ','     Beginning ','     Beginning ',' Net Gain from ','               ','    Groundwater','        Other  ','          Pond ','         Actual','               ','        Ending ','        Ending ','               ']
            pColumnHeaders(:,2) = ['      Time       ','          Area','      Potential','   Precipitation','         Runoff','      Applied  ','         Water ','         Return','     to GW     ','     to GW     ','     to GW     ','     to GW     ',' Moist. Storage','   Pond Storage',' Land Expansion','   Infiltration','       Inflow  ','        Inflow ','          Drain','           ET  ','    Percolation',' Moist. Storage','   Pond Storage','    Discrepancy']
            pColumnHeaders(:,3) = [               Text,           Text1,'         ET    ','                ','               ','       Water   ','               ','          Flow ','     (Ag)      ',' (Urban Indrs) ',' (Urban Outdrs)',' (Native Veg.) ','        (+)    ','        (+)    ','       (+)     ','        (+)    ','         (+)   ','          (+)  ','           (-) ','           (-) ','       (-)     ','          (-)  ','          (-)  ','        (=)    ']
            pColumnHeaders(:,4) = ''
            pFormatSpecs(1)     = '(A17,A14,A15,A16,20A15)'
            pFormatSpecs(2)     = '(A17,A14,A15,A16,20A15)'
            pFormatSpecs(3)     = '(A17,A14,A15,A16,20A15)'
            pFormatSpecs(4)     = '('//TRIM(IntToText(f_iTitleLen))//'(1H-),'//TRIM(IntToText(f_iNAgRootZoneBudColumns_PondedCrops+1))//'A0)'
        END ASSOCIATE
    END ASSOCIATE

    !Data for DSS output  
    ASSOCIATE (pDSSOutput => OutputData%DSSOutput)
        ALLOCATE (pDSSOutput%cPathNames(f_iNAgRootZoneBudColumns_PondedCrops*NRegion) , pDSSOutput%iDataTypes(f_iNAgRootZoneBudColumns_PondedCrops))
        iCount = 1
        DO indxLocation=1,NRegion
            DO indxCol=1,f_iNAgRootZoneBudColumns_PondedCrops
                pDSSOutput%cPathNames(iCount) = '/'//TRIM(UpperCase(cProjectNameForDSS))//'_ROOTZN_BUD/'       //  &  !A part
                                                TRIM(UpperCase(OutputData%cLocationNames(indxLocation)))//'/'  //  &  !B part
                                                TRIM(CParts(indxCol))//'/'                                     //  &  !C part
                                                '/'                                                            //  &  !D part
            ...
            END DO
        END DO
        pDSSOutput%iDataTypes = [f_iPER_AVER,(f_iPER_CUM,indxCol=1,22)]
    END ASSOCIATE
...
! ******************************************************************
  ...
  ! -------------------------------------------------------------
  SUBROUTINE RootZone_v412_GetBudget_AnnualFlows_GivenFile(Budget,iBudgetType,iLUType,iSubregionIndex,cBeginDate,cEndDate,rFactVL,rFlows,cFlowNames,iStat,Logger)
  ...
  ! --- (Assumes cBeginDate and cEndDate are adjusted properly)
  ! -------------------------------------------------------------
  SUBROUTINE RootZone_v412_GetBudget_MonthlyFlows_GivenFile(Budget,iBudgetType,iLUType,iSubregionIndex,cBeginDate,cEndDate,rFactVL,rFlows,cFlowNames,iStat,Logger)
    TYPE(BudgetType),INTENT(IN)                        :: Budget      !Assumes Budget file is already open
    CHARACTER(LEN=*),INTENT(IN)                        :: cBeginDate,cEndDate
    INTEGER,INTENT(IN)                                 :: iBudgetType,iLUType,iSubregionIndex
    REAL(8),INTENT(IN)                                 :: rFactVL
  ...
  ! -------------------------------------------------------------
  SUBROUTINE RootZone_v412_GetZBudget_MonthlyFlows_GivenFile(ZBudget,iZBudgetType,ZoneList,iZoneID,iLUType,cBeginDate,cEndDate,rFactVL,rFlows,cFlowNames,iStat,Logger)
  ...
  ! -------------------------------------------------------------
  SUBROUTINE RootZone_v412_GetZBudget_AnnualFlows_GivenFile(ZBudget,iZBudgetType,ZoneList,iZoneID,iLUType,cBeginDate,cEndDate,rFactVL,rFlows,cFlowNames,iStat,Logger)
     TYPE(ZBudgetType),INTENT(IN)             :: ZBudget
     TYPE(ZoneListType),INTENT(IN)            :: ZoneList
     INTEGER,INTENT(IN)                       :: iZBudgetType,iZoneID,iLUType
     CHARACTER(LEN=*),INTENT(IN)              :: cBeginDate,cEndDate
...
! ******************************************************************
  ...
  ! -------------------------------------------------------------
  SUBROUTINE RootZone_v412_ReadTSData(RootZone,AppGrid,TimeStep,Precip,ETData,iStat,RegionLUAreas,iColCropCoeff_NonPondedAg,iColCropCoeff_PondedAg,iColCropCoeff_Urban,iColCropCoeff_NVRV)
    CLASS(RootZone_v412_Type)          :: RootZone
    TYPE(AppGridType),INTENT(IN)       :: AppGrid
    TYPE(TimeStepType),INTENT(IN)      :: TimeStep
    TYPE(PrecipitationType),INTENT(IN) :: Precip
    ...
    CALL RootZone%RootZone_v411_Type%ReadTSData(AppGrid,TimeStep,Precip,ETData,iStat,RegionLUAreas,iColCropCoeff_NonPondedAg,iColCropCoeff_PondedAg,iColCropCoeff_Urban,iColCropCoeff_NVRV)
    IF (iStat .NE. 0) RETURN
                ...
                SELECT CASE (pDestTypes(indx))
                    ...
                    CASE (f_iFlowDest_StrmNode)
                        IF (RootZone%lStrmNodeIDs_Provided) THEN
                            ...
                            IF (pDests(indx) .EQ. 0) THEN
                                CALL RootZone%Logger%SetLastMessage('Stream node ID '//TRIM(IntToText(ID))//' listed in the Surface Flow Destination file in the Root Zone component is not in the model!',f_iFatal,ThisProcedure)
                                iStat = -1
                                RETURN
                            END IF
                        END IF
                    ...
                    CASE (f_iFlowDest_Lake)
                        IF (RootZone%lLakeIDs_Provided) THEN
                            ...
                            IF (pDests(indx) .EQ. 0) THEN
                                CALL RootZone%Logger%SetLastMessage('Lake ID '//TRIM(IntToText(ID))//' listed in the Surface Flow Destination file in the Root Zone component is not in the model!',f_iFatal,ThisProcedure)
                                iStat = -1
                                RETURN
                            END IF
                        END IF
                    ...
                    CASE (f_iFlowDest_GWElement)
                        ...
                        IF (ID .NE. -1) THEN
                            ...
                            IF (pDests(indx) .EQ. 0) THEN
                                CALL RootZone%Logger%SetLastMessage('Element ID '//TRIM(IntToText(ID))//' listed the Surface Flow Destination file in the Root Zone component is not in the model!',f_iFatal,ThisProcedure)
                                iStat = -1
                                RETURN
                            END IF
                        END IF
...
! ******************************************************************
  ...
  ! --- PRINT OUT RESULTS
  ! -------------------------------------------------------------
  SUBROUTINE RootZone_v412_PrintResults(RootZone,AppGrid,ETData,TimeStep,lEndOfSimulation,iColCropCoeff_NonPondedAg,iColCropCoeff_PondedAg,iColCropCoeff_Urban,iColCropCoeff_NVRV)
    CLASS(RootZone_v412_Type)     :: RootZone
    TYPE(AppGridType),INTENT(IN)  :: AppGrid
    TYPE(ETType),INTENT(IN)       :: ETData
    ...
    IF (RootZone%Flags%RootZoneZoneBudRawFile_Defined)   &
        CALL WriteRootZoneFlowsToZoneBudRawFile(RootZone,AppGrid,ETData,                                     &
                                                ...
                                                rAgArea_Refuge,rAgPump_Refuge,rAgDeli_Refuge,rAgETGW_Refuge, &
                                                rUrbArea,rUrbPump,rUrbDeli,                                  &
                                                iColCropCoeff_NonPondedAg,iColCropCoeff_PondedAg,            &
                                                iColCropCoeff_Urban,iColCropCoeff_NVRV                       )
  ...
  SUBROUTINE PrintNonPondedAgResults(RootZone,AppGrid)
    ...
    REAL(8),DIMENSION((AppGrid%NSubregions+1)*RootZone%NonPondedAgRootZone%NBudgetCrops) :: RPump,RDeli,RRunoff,RLUArea,RGenMoistInflow,RReturn,    &
                                                                                            ...
                                                                                            RSrfcInForGW_UrbOut,RSrfcInForGW_NVRV
    ...
    ASSOCIATE (pNonPondedAg => RootZone%NonPondedAgRootZone)
        ...
        IF (pNonPondedAg%lRootZoneBudRawFile_Defined) THEN
            CALL WriteNonPondedAgRootZoneFlowsToBudRawFile(pNonPondedAg%RootZoneBudRawFile            , &
                                                           AppGrid                                    , &
                                                           pNonPondedAg%iBudgetCrops                  , &
                                                           pNonPondedAg%Crops%GenericLandUse_v41_Type , &
                                                           RLUArea                                    , &
  ...
  SUBROUTINE PrintPondedAgResults(RootZone,AppGrid)
    ...
    !Local variables
    INTEGER                                                                            :: iNBudgetCrops,indxLast,indxCrop,indxElem,iCrop,iNSubregions
    REAL(8),DIMENSION(AppGrid%NElements)                                               :: rElemValues
    ...
    ASSOCIATE (pPondedAg => RootZone%PondedAgRootZone)
        ...
        IF (pPondedAg%lRootZoneBudRawFile_Defined) THEN
            DO indxCrop=1,iNBudgetCrops
                ...
                RReturn(indxCrop:indxLast:iNBudgetCrops)     = AppGrid%AccumElemValuesToSubregions(rElemValues)
                RReturn(indxLast+indxCrop)                   = SUM(RReturn(indxCrop:indxLast:iNBudgetCrops))
                rElemValues                                  = pPondedAg%Crops%PondDepth(iCrop,:) * pPondedAg%Crops%Area(iCrop,:)
            END DO
            CALL pPondedAg%ComputeRegionalPondStorage_ForBudgetCrops(AppGrid,lAssignToPreviousTime=.FALSE.)
        END IF
        ...
        IF (pPondedAg%lRootZoneBudRawFile_Defined) THEN
            CALL WritePondedAgRootZoneFlowsToBudRawFile(pPondedAg%RootZoneBudRawFile            , &
                                                        AppGrid                                 , &
                                                        pPondedAg%iBudgetCrops                  , &
                                                        pPondedAg%Crops%GenericLandUse_v41_Type , &
                                                        RLUArea                                 , &
                                                        RootZone%ElemPrecipData%Precip          , &
                                                        RRunoff,RPump,RDeli                     , &
                                                        pPondedAg%Crops%Reuse                   , &
                                                        pPondedAg%RPondStor_ForBudgetCrops      , & 
                                                        pPondedAg%RPondStor_ForBudgetCrops_P    , & 
                                                        RReturn                                 , &
                                                        RSrfcInForGW_Ag                         , &
                                                        RSrfcInForGW_UrbIn                      , &
                                                        RSrfcInForGW_UrbOut                     , &
  ...
  SUBROUTINE WriteRootZoneFlowsToBudRawFile(RootZone,AppGrid,RPump_Ag,RDeli_Ag,RPump_Urb,RDeli_Urb,RLUArea_Ag,RLUArea_Urb)
    ...
    !Pond storage
    IF (RootZone%Flags%lPondedAg_Defined) THEN
        rPondStor_P = RootZone%PondedAgRootZone%rPondStor_P
        rPondStor   = RootZone%PondedAgRootZone%rPondStor
    ELSE
        RPondStor_P = 0.0
        RPondStor   = 0.0
    END IF
    ...
    rDummyArray(12,:) = RootZone%RSoilM_P(:,1) - RPondStor_P                        !Moisture storage at the beginning of the time interval
    rDummyArray(13,:) = RPondStor_P                                                 !Pond storage at the beginning of the time interval
    rDummyArray(14,:) = RSoilMCh_Ag                                                 !Soil moisture change due to expansion/contraction of ag lands
    rDummyArray(15,:) = RInfilt_Ag                                                  !Infiltration on ag lands
    rDummyArray(16,:) = RGWInflow_Ag                                                !Groundwater inflow on ag lands
    rDummyArray(17,:) = RGenericMoist_Ag                                            !Generic moisture inflow to ag lands
    rDummyArray(18,:) = RDrain_Ag                                                   !Rice/refuge pond drainage on ag lands
    rDummyArray(19,:) = RETa_Ag                                                     !ET on ag lands
    rDummyArray(20,:) = RPerc_Ag                                                    !Percolation on ag lands
    rDummyArray(21,:) = RootZone%RSoilM(:,1) - RPondStor                            !Moisture storage at the end of the time interval
    rDummyArray(22,:) = RPondStor                                                   !Pond storage at the end of the time interval
    rDummyArray(23,:) = Error_Ag                                                    !Mass balance error for ag lands
    rDummyArray(24,:) = RLUArea_Urb                                                 !Urban area
    rDummyArray(25,:) = RETPot_Urb                                                  !Potential ET on urban lands
    rDummyArray(26,:) = RPrecip_Urb                                                 !Precipitation on urban lands
    rDummyArray(27,:) = RRunoff_Urb                                                 !Runoff from urban lands
    rDummyArray(28,:) = RDeli_Urb + RPump_Urb                                       !Prime applied water on urban lands prior to re-used water
    rDummyArray(29,:) = RReuse_Urb                                                  !Reused water on urban indoors and outdoors
    rDummyArray(30,:) = RReturn_Urb                                                 !Return flow from urban lands
    rDummyArray(31,:) = RSrfcInForGW_Urb_Ag                                         !Surface flow from ag lands for gw recharge on urban lands
    rDummyArray(32,:) = RSrfcInForGW_Urb_UrbIn                                      !Surface flow from urban indoors for gw recharge on urban lands
    rDummyArray(33,:) = RSrfcInForGW_Urb_UrbOut                                     !Surface flow from urban outdoors for gw recharge on urban lands
    rDummyArray(34,:) = RSrfcInForGW_Urb_NVRV                                       !Surface flow from NVRV for gw recharge on urban lands
    rDummyArray(35,:) = RootZone%RSoilM_P(:,2)                                      !Storage at the beginning of the time interval
    rDummyArray(36,:) = RSoilMCh_Urb                                                !Soil moisture change due to expansion/contraction of urban lands
    rDummyArray(37,:) = RInfilt_Urb                                                 !Infiltration on urban lands
    rDummyArray(38,:) = RGWInflow_Urb                                               !Groundwater inflow on urban lands
    rDummyArray(39,:) = RGenericMoist_Urb                                           !Generic moisture inflow to urban lands
    rDummyArray(40,:) = RETa_Urb                                                    !ET on urban lands
    rDummyArray(41,:) = RPerc_Urb                                                   !Percolation on urban lands
    rDummyArray(42,:) = RootZone%RSoilM(:,2)                                        !Storage at the end of the time interval     
    rDummyArray(43,:) = Error_Urb                                                   !Mass balance error at urban lands
    rDummyArray(44,:) = RLUArea_NV                                                  !Natural area
    rDummyArray(45,:) = RETPot_NV                                                   !Potential ET on natural lands
    rDummyArray(46,:) = RPrecip_NV                                                  !Precipitation on natural lands
    rDummyArray(47,:) = RRunoff_NV                                                  !Total rainfall runoff on natural lands
    rDummyArray(48,:) = RSrfcInForGW_NV_Ag                                          !Surface flow from ag lands for gw recharge on natural lands
    rDummyArray(49,:) = RSrfcInForGW_NV_UrbIn                                       !Surface flow from urban indoorsfor gw recharge on natural lands
    rDummyArray(50,:) = RSrfcInForGW_NV_UrbOut                                      !Surface flow from urban outdoors for gw recharge on natural lands
    rDummyArray(51,:) = RSrfcInForGW_NV_NVRV                                        !Surface flow from NVRV for gw recharge on natural lands
    rDummyArray(52,:) = RootZone%RSoilM_P(:,3)                                      !Storage at the beginning of the time interval
    rDummyArray(53,:) = RSoilMCh_NV                                                 !Soil moisture change due to expansion/contraction of natural lands
    rDummyArray(54,:) = RInfilt_NV                                                  !Infiltration on natural lands
    rDummyArray(55,:) = RGWInflow_NV                                                !Groundwater inflow on natural lands
    rDummyArray(56,:) = RGenericMoist_NV                                            !Generic moisture inflow to natural lands
    rDummyArray(57,:) = RRVETFromStrm_NV                                            !Stream inflow to meet riparian ET
    rDummyArray(58,:) = RETa_NV                                                     !ET on natural lands
    rDummyArray(59,:) = RPerc_NV                                                    !Percolation on natural lands
    rDummyArray(60,:) = RootZone%RSoilM(:,3)                                        !Storage at the end of the time interval          
    rDummyArray(61,:) = Error_NV                                                    !Mass balance error at native and riparian lands
  ...
  ! -------------------------------------------------------------
  ! --- PRINT NON-PONDED-CROP-SPECIFIC ROOT ZONE BUDGET RAW DATA
  ! -------------------------------------------------------------
  SUBROUTINE WriteNonPondedAgRootZoneFlowsToBudRawFile(RootZoneBudRawFile,AppGrid,iBudgetCrops,Crops,RLUArea,rElemPrecip,RRunoff,RPump,RDeli,rCropsReuse,RReturn,RSrfcInForGW_Ag,RSrfcInForGW_UrbIn,RSrfcInForGW_UrbOut,RSrfcInForGW_NVRV,RGenMoistInflow,RInfilt,RPerc,RegionETPot)
    TYPE(BudgetType)                         :: RootZoneBudRawFile
    TYPE(AppGridType),INTENT(IN)             :: AppGrid
    INTEGER,INTENT(IN)                       :: iBudgetCrops(:)
    TYPE(GenericLandUse_v41_Type),INTENT(IN) :: Crops
    REAL(8),INTENT(IN)                       :: RPump(:),RDeli(:),RGenMoistInflow(:),RRunoff(:),RReturn(:),RInfilt(:),RPerc(:),       &
                                                RLUArea(:),rElemPrecip(:),RegionETPot(:,:),rCropsReuse(:,:),                &
                                                RSrfcInForGW_Ag(:),RSrfcInForGW_UrbIn(:),RSrfcInForGW_UrbOut(:),RSrfcInForGW_NVRV(:)
    
    !Local variables
    INTEGER                                                       :: iNBudgetCrops,indxCrop,indxLast,iCrop
    REAL(8)                                                       :: rDummyArray(f_iNAgRootZoneBudColumns_NonPondedCrops,(AppGrid%NSubregions+1)*SIZE(iBudgetCrops)), &
    ...
    rDummyArray(12,:) = RSoilM_P                                        !Moisture storage at the beginning of the time interval
    ...
    rDummyArray(17,:) = RETa                                            !ET on ag lands
    rDummyArray(18,:) = RPerc                                           !Percolation on ag lands
    rDummyArray(19,:) = RSoilM                                          !Moisture storage at the end of the time interval
    rDummyArray(20,:) = Error                                           !Mass balance error for ag lands

    !Print out values to binary file
    CALL RootZoneBudRawFile%WriteData(rDummyArray)

  END SUBROUTINE WriteNonPondedAgRootZoneFlowsToBudRawFile
  
  
  ! -------------------------------------------------------------
  ! --- PRINT PONDED-CROP-SPECIFIC ROOT ZONE BUDGET RAW DATA
  ! -------------------------------------------------------------
  SUBROUTINE WritePondedAgRootZoneFlowsToBudRawFile(RootZoneBudRawFile,AppGrid,iBudgetCrops,Crops,RLUArea,rElemPrecip,RRunoff,RPump,RDeli,rCropsReuse,rPondStor,RPondStor_P,RReturn,RSrfcInForGW_Ag,RSrfcInForGW_UrbIn,RSrfcInForGW_UrbOut,RSrfcInForGW_NVRV,RGenMoistInflow,RInfilt,RPerc,RDrain,RegionETPot)
    TYPE(BudgetType)                         :: RootZoneBudRawFile
    TYPE(AppGridType),INTENT(IN)             :: AppGrid
    INTEGER,INTENT(IN)                       :: iBudgetCrops(:)
    TYPE(GenericLandUse_v41_Type),INTENT(IN) :: Crops
    REAL(8),INTENT(IN)                       :: RPump(:),RDeli(:),RGenMoistInflow(:),RRunoff(:),RReturn(:),RInfilt(:),RPerc(:),       &
                                                ...
                                                RSrfcInForGW_Ag(:),RSrfcInForGW_UrbIn(:),RSrfcInForGW_UrbOut(:),RSrfcInForGW_NVRV(:), &
                                                RPondStor(:),RPondStor_P(:)
    
    !Local variables
    INTEGER                                                       :: iNBudgetCrops,indxCrop,indxLast,iCrop
    REAL(8)                                                       :: rDummyArray(f_iNAgRootZoneBudColumns_PondedCrops,(AppGrid%NSubregions+1)*SIZE(iBudgetCrops)), &
                                                                     rArea(SIZE(Crops%Area,DIM=2),SIZE(Crops%Area,DIM=1))
    REAL(8),DIMENSION(AppGrid%NElements)                          :: rElemValues
    REAL(8),DIMENSION((AppGrid%NSubregions+1)*SIZE(iBudgetCrops)) :: RPrecip,RReuse,RSoilMCh,RETa,RSoilM,RSoilM_P,RGWInflow,RETPot,Error
    ...
    rDummyArray(10,:) = RSrfcInForGW_UrbOut                             !Surface inflow from urban outdoors for gw recharge 
    rDummyArray(11,:) = RSrfcInForGW_NVRV                               !Surface inflow from native&riparian vegitation for gw recharge 
    rDummyArray(12,:) = RSoilM_P - RPondStor_P                          !Moisture storage at the beginning of the time interval
    rDummyArray(13,:) = RPondStor_P                                     !Pond storage at the beginning of the time interval
    rDummyArray(14,:) = RSoilMCh                                        !Soil moisture chnage due to expansion/contraction of ag lands
    rDummyArray(15,:) = RInfilt                                         !Infiltration on ag lands
    rDummyArray(16,:) = RGWInflow                                       !GW inflow to non-ponded ag lands
    rDummyArray(17,:) = RGenMoistInflow                                 !Generic moisture inflow to non-ponded ag lands
    rDummyArray(18,:) = RDrain                                          !Rice/refuge pond drainage on ag lands
    rDummyArray(19,:) = RETa                                            !ET on ag lands
    rDummyArray(20,:) = RPerc                                           !Percolation on ag lands
    rDummyArray(21,:) = RSoilM - RPondStor                              !Moisture storage at the end of the time interval
    rDummyArray(22,:) = RPondStor                                       !Pond storage at the end of the time interval
    rDummyArray(23,:) = Error                                           !Mass balance error for ag lands

    !Print out values to binary file
    CALL RootZoneBudRawFile%WriteData(rDummyArray)

  END SUBROUTINE WritePondedAgRootZoneFlowsToBudRawFile
  ...
  ! -------------------------------------------------------------
  SUBROUTINE WriteRootZoneFlowsToZoneBudRawFile(RootZone,AppGrid,ETData,rAgArea_NP,rAgPump_NP,rAgDeli_NP,rAgETGW_NP,rAgArea_Rice,rAgPump_Rice,rAgDeli_Rice,rAgETGW_Rice,rAgArea_Refuge,rAgPump_Refuge,rAgDeli_Refuge,rAgETGW_Refuge,rUrbArea,rUrbPump,rUrbDeli,iColCropCoeff_NonPondedAg,iColCropCoeff_PondedAg,iColCropCoeff_Urban,iColCropCoeff_NVRV)
    TYPE(RootZone_v412_Type)                          :: RootZone
    TYPE(AppGridType),INTENT(IN)                      :: AppGrid
    TYPE(ETType),INTENT(IN)                           :: ETData
    REAL(8),DIMENSION(AppGrid%NElements,1),INTENT(IN) :: rAgArea_NP,rAgPump_NP,rAgDeli_NP,rAgETGW_NP,rAgArea_Rice,rAgPump_Rice,rAgDeli_Rice,rAgETGW_Rice,rAgArea_Refuge,rAgPump_Refuge,rAgDeli_Refuge,rAgETGW_Refuge,rUrbArea,rUrbPump,rUrbDeli
    ...
    REAL(8),DIMENSION(AppGrid%NElements,1) :: rAgPotET_NP,rAgPrecip_NP,rAgRunoff_NP,rAgAW_NP,rAgReuse_NP,rAgReturn_NP,rAgBeginStor_NP,rAgSoilMCh_NP,rAgInfilt_NP,rAgOthIn_NP,rAgETa_NP,rAgPerc_NP,rAgEndStor_NP,rAgError_NP,                                    &
                                              ...
                                              rAgPotET_Rice,rAgPrecip_Rice,rAgRunoff_Rice,rAgAW_Rice,rAgReuse_Rice,rAgReturn_Rice,rAgBeginStor_Rice,rAgBeginPondStor_Rice,rAgSoilMCh_Rice,rAgInfilt_Rice,rAgOthIn_Rice,rAgDrain_Rice,                           &
                                              rAgETa_Rice,rAgPerc_Rice,rAgEndStor_Rice,rAgEndPondStor_Rice,rAgError_Rice,rAgSrfcInForGW_Ag_Rice,rAgSrfcInForGW_UrbIn_Rice,rAgSrfcInForGW_UrbOut_Rice,rAgSrfcInForGW_NVRV_Rice,                                  &    
                                              rAgPotET_Refuge,rAgPrecip_Refuge,rAgRunoff_Refuge,rAgAW_Refuge,rAgReuse_Refuge,rAgReturn_Refuge,rAgBeginStor_Refuge,rAgBeginPondStor_Refuge,rAgSoilMCh_Refuge,rAgInfilt_Refuge,rAgOthIn_Refuge,rAgDrain_Refuge,   &
                                              rAgETa_Refuge,rAgPerc_Refuge,rAgEndStor_Refuge,rAgEndPondStor_Refuge,rAgError_Refuge, rAgSrfcInForGW_Ag_Refuge,rAgSrfcInForGW_UrbIn_Refuge,rAgSrfcInForGW_UrbOut_Refuge,rAgSrfcInForGW_NVRV_Refuge,               &    
    ...
    IF (RootZone%Flags%lNonPondedAg_Defined) THEN
        ...
        END DO
        IF (PRESENT(iColCropCoeff_NonPondedAg)) THEN
            DO indxElem=1,AppGrid%NElements
                rAgPotET_NP(indxElem,1) = SUM(ETData%GetValues(RootZone%NonPondedAgRootZone%Crops%iColETc(:,indxElem),iColCropCoeff_NonPondedAg(:,indxElem)) * RootZone%NonPondedAgRootZone%Crops%Area(:,indxElem))       !Non-ponded ag potential ET
            END DO
        ELSE
            DO indxElem=1,AppGrid%NElements
    ...
    IF (RootZone%Flags%lPondedAg_Defined) THEN
        ...
        END DO
        IF (PRESENT(iColCropCoeff_PondedAg)) THEN
            DO indxElem=1,AppGrid%NElements
                rAgPotET_Rice(indxElem,1) = SUM(ETData%GetValues(RootZone%PondedAgRootZone%Crops%iColETc(indxB:indxE,indxElem),iColCropCoeff_PondedAg(indxB:indxE,indxElem)) * RootZone%PondedAgRootZone%Crops%Area(indxB:indxE,indxElem))                       !Rice potential ET
            END DO
        ELSE
            DO indxElem=1,AppGrid%NElements
                rAgPotET_Rice(indxElem,1) = SUM(ETData%GetValues(RootZone%PondedAgRootZone%Crops%iColETc(indxB:indxE,indxElem)) * RootZone%PondedAgRootZone%Crops%Area(indxB:indxE,indxElem))                       !Rice potential ET
            END DO
        END IF
        ...
        rAgBeginPondStor_Rice(:,1) = SUM(RootZone%PondedAgRootZone%Crops%rPondStor_P(indxB:indxE,:) , DIM=1)
        ...
        !Rice pond storage at each (rice crop,element); also serves as advancing state of the pond storage in time
        RootZone%PondedAgRootZone%Crops%rPondStor_P(indxB:indxE,:) = RootZone%PondedAgRootZone%Crops%PondDepth(indxB:indxE,:) * RootZone%PondedAgRootZone%Crops%Area(indxB:indxE,:)
        rAgEndPondStor_Rice(:,1)                                   = SUM(RootZone%PondedAgRootZone%Crops%rPondStor_P(indxB:indxE,:) , DIM=1)                
        
        !Correct beginning end ending moisture storage
        rAgBeginStor_Rice = rAgBeginStor_Rice- rAgBeginPondStor_Rice
        rAgEndStor_Rice   = rAgEndStor_Rice   - rAgEndPondStor_Rice
        ...
        IF (PRESENT(iColCropCoeff_PondedAg)) THEN
            DO indxElem=1,AppGrid%NElements
                rAgPotET_Refuge(indxElem,1) = SUM(ETData%GetValues(RootZone%PondedAgRootZone%Crops%iColETc(indxB:indxE,indxElem),iColCropCoeff_PondedAg(indxB:indxE,indxElem)) * RootZone%PondedAgRootZone%Crops%Area(indxB:indxE,indxElem))           !Refuge potential ET
            END DO
        ELSE
            DO indxElem=1,AppGrid%NElements
                rAgPotET_Refuge(indxElem,1) = SUM(ETData%GetValues(RootZone%PondedAgRootZone%Crops%iColETc(indxB:indxE,indxElem)) * RootZone%PondedAgRootZone%Crops%Area(indxB:indxE,indxElem))           !Refuge potential ET
            END DO
        END IF
        ...
        rAgBeginStor_Refuge(:,1)     = SUM((RootZone%PondedAgRootZone%Crops%SoilM_Precip_P_BeforeUpdate(indxB:indxE,:)  &                                                                             !Refuge beginning storage
                                         +RootZone%PondedAgRootZone%Crops%SoilM_AW_P_BeforeUpdate(indxB:indxE,:)        &                                                                             
                                         +RootZone%PondedAgRootZone%Crops%SoilM_Oth_P_BeforeUpdate(indxB:indxE,:)       ) * RootZone%PondedAgRootZone%Crops%Area_P(indxB:indxE,:) , DIM=1)            
        rAgBeginPondStor_Refuge(:,1) = SUM(RootZone%PondedAgRootZone%Crops%rPondStor_P(indxB:indxE,:) , DIM=1)
        ...
        !Refuge pond storage at each (refuge,element); also serves as advancing state of the pond storage in time
        RootZone%PondedAgRootZone%Crops%rPondStor_P(indxB:indxE,:) = RootZone%PondedAgRootZone%Crops%PondDepth(indxB:indxE,:) * RootZone%PondedAgRootZone%Crops%Area(indxB:indxE,:)
        rAgEndPondStor_Refuge(:,1)                                 = SUM(RootZone%PondedAgRootZone%Crops%rPondStor_P(indxB:indxE,:) , DIM=1)                
        
        !Correct beginning end ending moisture storage
        rAgBeginStor_Refuge = rAgBeginStor_Refuge - rAgBeginPondStor_Refuge
        rAgEndStor_Refuge   = rAgEndStor_Refuge   - rAgEndPondStor_Refuge
    END IF
    ...
    IF (RootZone%Flags%lUrban_Defined) THEN
        ...
        rUrbPotET(:,1)     = ETData%GetValues(RootZone%UrbanRootZone%UrbData%iColETc(:,1),iColCropCoeff_Urban) * rUrbArea(:,1)                                          !Urban potential ET
    ...
    IF (RootZone%Flags%lNVRV_Defined) THEN
        ...
        IF (PRESENT(iColCropCoeff_NVRV)) THEN
            DO indxElem=1,Appgrid%NElements            
                rNVRVPotET(indxElem,1) = SUM(ETData%GetValues(RootZone%NVRVRootZone%NVRV%iColETc(:,indxElem),iColCropCoeff_NVRV(:,indxElem)) * RootZone%NVRVRootZone%NVRV%Area(:,indxElem))    !Native and riparian potential ET
            END DO
        ELSE
            DO indxElem=1,Appgrid%NElements            
                rNVRVPotET(indxElem,1) = SUM(ETData%GetValues(RootZone%NVRVRootZone%NVRV%iColETc(:,indxElem)) * RootZone%NVRVRootZone%NVRV%Area(:,indxElem))    !Native and riparian potential ET
            END DO
        END IF
    ...
    IF (RootZone%Flags%lPondedAg_Defined) THEN
        ...
        CALL RootZone%RootZoneZoneBudRawFile%WriteData(NLayers,f_iElemDataType,32,iLayer,rAgBeginStor_Rice)          !Rice beginning moisture storage
        CALL RootZone%RootZoneZoneBudRawFile%WriteData(NLayers,f_iElemDataType,33,iLayer,rAgBeginPondStor_Rice)      !Rice beginning pond storage
        CALL RootZone%RootZoneZoneBudRawFile%WriteData(NLayers,f_iElemDataType,34,iLayer,rAgSoilMCh_Rice)            !Rice change in soil storage from land expansion
        CALL RootZone%RootZoneZoneBudRawFile%WriteData(NLayers,f_iElemDataType,35,iLayer,rAgInfilt_Rice)             !Rice infiltration
        IF (RootZone%Flags%lComputeETFromGW) &                                                                       !Rice groundwater inflow for ET
            CALL RootZone%RootZoneZoneBudRawFile%WriteData(NLayers,f_iElemDataType,36,iLayer,rAgETGW_Rice)              
        IF (RootZone%Flags%lGenericMoistureFile_Defined) &                                                           !Rice other inflow
            CALL RootZone%RootZoneZoneBudRawFile%WriteData(NLayers,f_iElemDataType,37,iLayer,rAgOthIn_Rice)              
        CALL RootZone%RootZoneZoneBudRawFile%WriteData(NLayers,f_iElemDataType,38,iLayer,rAgDrain_Rice)              !Rice pond drain
        CALL RootZone%RootZoneZoneBudRawFile%WriteData(NLayers,f_iElemDataType,39,iLayer,rAgETa_Rice)                !Rice actual ET
        CALL RootZone%RootZoneZoneBudRawFile%WriteData(NLayers,f_iElemDataType,40,iLayer,rAgPerc_Rice)               !Rice perc
        CALL RootZone%RootZoneZoneBudRawFile%WriteData(NLayers,f_iElemDataType,41,iLayer,rAgEndStor_Rice)            !Rice end moisture storage
        CALL RootZone%RootZoneZoneBudRawFile%WriteData(NLayers,f_iElemDataType,42,iLayer,rAgEndPondStor_Rice)        !Rice end pond storage
        CALL RootZone%RootZoneZoneBudRawFile%WriteData(NLayers,f_iElemDataType,43,iLayer,rAgError_Rice)              !Rice error
        
        !Refuge
        CALL RootZone%RootZoneZoneBudRawFile%WriteData(NLayers,f_iElemDataType,44,iLayer,rAgArea_Refuge)               !Refuge area
        CALL RootZone%RootZoneZoneBudRawFile%WriteData(NLayers,f_iElemDataType,45,iLayer,rAgPotET_Refuge)              !Refuge potential ET
        CALL RootZone%RootZoneZoneBudRawFile%WriteData(NLayers,f_iElemDataType,46,iLayer,rAgPrecip_Refuge)             !Refuge precip
        CALL RootZone%RootZoneZoneBudRawFile%WriteData(NLayers,f_iElemDataType,47,iLayer,rAgRunoff_Refuge)             !Refuge runoff
        CALL RootZone%RootZoneZoneBudRawFile%WriteData(NLayers,f_iElemDataType,48,iLayer,rAgAW_Refuge)                 !Refuge prime applied water
        CALL RootZone%RootZoneZoneBudRawFile%WriteData(NLayers,f_iElemDataType,49,iLayer,rAgReuse_Refuge)              !Refuge reuse
        CALL RootZone%RootZoneZoneBudRawFile%WriteData(NLayers,f_iElemDataType,50,iLayer,rAgReturn_Refuge)             !Refuge return
        CALL RootZone%RootZoneZoneBudRawFile%WriteData(NLayers,f_iElemDataType,51,iLayer,rAgSrfcInForGW_Ag_Refuge)     !Refuge surface inflow from ag lands to recharge gw
        CALL RootZone%RootZoneZoneBudRawFile%WriteData(NLayers,f_iElemDataType,52,iLayer,rAgSrfcInForGW_UrbIn_Refuge)  !Refuge surface inflow from urban indoors to recharge gw
        CALL RootZone%RootZoneZoneBudRawFile%WriteData(NLayers,f_iElemDataType,53,iLayer,rAgSrfcInForGW_UrbOut_Refuge) !Refuge surface inflow from urban outdoors to recharge gw
        CALL RootZone%RootZoneZoneBudRawFile%WriteData(NLayers,f_iElemDataType,54,iLayer,rAgSrfcInForGW_NVRV_Refuge)   !Refuge surface inflow from NVRV to recharge gw
        CALL RootZone%RootZoneZoneBudRawFile%WriteData(NLayers,f_iElemDataType,55,iLayer,rAgBeginStor_Refuge)          !Refuge beginning moisture storage
        CALL RootZone%RootZoneZoneBudRawFile%WriteData(NLayers,f_iElemDataType,56,iLayer,rAgBeginPondStor_Refuge)      !Refuge beginning pond storage
        CALL RootZone%RootZoneZoneBudRawFile%WriteData(NLayers,f_iElemDataType,57,iLayer,rAgSoilMCh_Refuge)            !Refuge change in soil storage from land expansion
        CALL RootZone%RootZoneZoneBudRawFile%WriteData(NLayers,f_iElemDataType,58,iLayer,rAgInfilt_Refuge)             !Refuge infiltration
        IF (RootZone%Flags%lComputeETFromGW) &                                                                         !Refuge groundwater inflow for ET
            CALL RootZone%RootZoneZoneBudRawFile%WriteData(NLayers,f_iElemDataType,59,iLayer,rAgETGW_Refuge)              
        IF (RootZone%Flags%lGenericMoistureFile_Defined) &                                                             !Refuge other inflow
            CALL RootZone%RootZoneZoneBudRawFile%WriteData(NLayers,f_iElemDataType,60,iLayer,rAgOthIn_Refuge)              
        CALL RootZone%RootZoneZoneBudRawFile%WriteData(NLayers,f_iElemDataType,61,iLayer,rAgDrain_Refuge)              !Refuge pond drain
        CALL RootZone%RootZoneZoneBudRawFile%WriteData(NLayers,f_iElemDataType,62,iLayer,rAgETa_Refuge)                !Refuge actual ET
        CALL RootZone%RootZoneZoneBudRawFile%WriteData(NLayers,f_iElemDataType,63,iLayer,rAgPerc_Refuge)               !Refuge perc
        CALL RootZone%RootZoneZoneBudRawFile%WriteData(NLayers,f_iElemDataType,64,iLayer,rAgEndStor_Refuge)            !Refuge end moisture storage
        CALL RootZone%RootZoneZoneBudRawFile%WriteData(NLayers,f_iElemDataType,65,iLayer,rAgEndPondStor_Refuge)        !Refuge end pond storage
        CALL RootZone%RootZoneZoneBudRawFile%WriteData(NLayers,f_iElemDataType,66,iLayer,rAgError_Refuge)              !Refuge error
    END IF                                                                                                   
                                                                                                             
    !Print urban data
    IF (RootZone%Flags%lUrban_Defined) THEN                                                                  
        CALL RootZone%RootZoneZoneBudRawFile%WriteData(NLayers,f_iElemDataType,67,iLayer,rUrbArea)               !Urban area
        CALL RootZone%RootZoneZoneBudRawFile%WriteData(NLayers,f_iElemDataType,68,iLayer,rUrbPotET)              !Urban potential ET
        CALL RootZone%RootZoneZoneBudRawFile%WriteData(NLayers,f_iElemDataType,69,iLayer,rUrbPrecip)             !Urban precip
        CALL RootZone%RootZoneZoneBudRawFile%WriteData(NLayers,f_iElemDataType,70,iLayer,rUrbRunoff)             !Urban runoff
        CALL RootZone%RootZoneZoneBudRawFile%WriteData(NLayers,f_iElemDataType,71,iLayer,rUrbAW)                 !Urban prime applied water
        CALL RootZone%RootZoneZoneBudRawFile%WriteData(NLayers,f_iElemDataType,72,iLayer,rUrbReuse)              !Urban reuse
        CALL RootZone%RootZoneZoneBudRawFile%WriteData(NLayers,f_iElemDataType,73,iLayer,rUrbReturn)             !Urban return
        CALL RootZone%RootZoneZoneBudRawFile%WriteData(NLayers,f_iElemDataType,74,iLayer,rUrbSrfcInForGW_Ag)     !Urban surface inflow from ag lands to recharge gw
        CALL RootZone%RootZoneZoneBudRawFile%WriteData(NLayers,f_iElemDataType,75,iLayer,rUrbSrfcInForGW_UrbIn)  !Urban surface inflow from urban indoors to recharge gw
        CALL RootZone%RootZoneZoneBudRawFile%WriteData(NLayers,f_iElemDataType,76,iLayer,rUrbSrfcInForGW_UrbOut) !Urban surface inflow from urban outdoors to recharge gw
        CALL RootZone%RootZoneZoneBudRawFile%WriteData(NLayers,f_iElemDataType,77,iLayer,rUrbSrfcInForGW_NVRV)   !Urban surface inflow from NVRV to recharge gw
        CALL RootZone%RootZoneZoneBudRawFile%WriteData(NLayers,f_iElemDataType,78,iLayer,rUrbBeginStor)          !Urban beginning storage
        CALL RootZone%RootZoneZoneBudRawFile%WriteData(NLayers,f_iElemDataType,79,iLayer,rUrbSoilMCh)            !Urban change in soil storage from land expansion
        CALL RootZone%RootZoneZoneBudRawFile%WriteData(NLayers,f_iElemDataType,80,iLayer,rUrbInfilt)             !Urban infiltration
        IF (RootZone%Flags%lComputeETFromGW) &                                                                   !Urban groundwater inflow for ET
            CALL RootZone%RootZoneZoneBudRawFile%WriteData(NLayers,f_iElemDataType,81,iLayer,rUrbETGW)               
        IF (RootZone%Flags%lGenericMoistureFile_Defined) &                                                       !Urban other inflow
            CALL RootZone%RootZoneZoneBudRawFile%WriteData(NLayers,f_iElemDataType,82,iLayer,rUrbOthIn)               
        CALL RootZone%RootZoneZoneBudRawFile%WriteData(NLayers,f_iElemDataType,83,iLayer,rUrbETa)                !Urban actual ET
        CALL RootZone%RootZoneZoneBudRawFile%WriteData(NLayers,f_iElemDataType,84,iLayer,rUrbPerc)               !Urban perc
        CALL RootZone%RootZoneZoneBudRawFile%WriteData(NLayers,f_iElemDataType,85,iLayer,rUrbEndStor)            !Urban end storage
        CALL RootZone%RootZoneZoneBudRawFile%WriteData(NLayers,f_iElemDataType,86,iLayer,rUrbError)              !Urban error
    END IF                                                                                                   
                                                                                                             
    !Print native and riparian veg data
    IF (RootZone%Flags%lNVRV_Defined) THEN                                                                   
        CALL RootZone%RootZoneZoneBudRawFile%WriteData(NLayers,f_iElemDataType,87,iLayer,rNVRVArea)               !NVRV area
        CALL RootZone%RootZoneZoneBudRawFile%WriteData(NLayers,f_iElemDataType,88,iLayer,rNVRVPotET)              !NVRV potential ET
        CALL RootZone%RootZoneZoneBudRawFile%WriteData(NLayers,f_iElemDataType,89,iLayer,rNVRVPrecip)             !NVRV precip
        CALL RootZone%RootZoneZoneBudRawFile%WriteData(NLayers,f_iElemDataType,90,iLayer,rNVRVRunoff)             !NVRV runoff
        CALL RootZone%RootZoneZoneBudRawFile%WriteData(NLayers,f_iElemDataType,91,iLayer,rNVRVSrfcInForGW_Ag)     !NVRV surface inflow from ag lands to recharge gw
        CALL RootZone%RootZoneZoneBudRawFile%WriteData(NLayers,f_iElemDataType,92,iLayer,rNVRVSrfcInForGW_UrbIn)  !NVRV surface inflow from urban indoors to recharge gw
        CALL RootZone%RootZoneZoneBudRawFile%WriteData(NLayers,f_iElemDataType,93,iLayer,rNVRVSrfcInForGW_UrbOut) !NVRV surface inflow from urban outdoors to recharge gw
        CALL RootZone%RootZoneZoneBudRawFile%WriteData(NLayers,f_iElemDataType,94,iLayer,rNVRVSrfcInForGW_NVRV)   !NVRV surface inflow from NVRV to recharge gw
        CALL RootZone%RootZoneZoneBudRawFile%WriteData(NLayers,f_iElemDataType,95,iLayer,rNVRVBeginStor)          !NVRV beginning storage
        CALL RootZone%RootZoneZoneBudRawFile%WriteData(NLayers,f_iElemDataType,96,iLayer,rNVRVSoilMCh)            !NVRV change in soil storage from land expansion
        CALL RootZone%RootZoneZoneBudRawFile%WriteData(NLayers,f_iElemDataType,97,iLayer,rNVRVInfilt)             !NVRV infiltration
        IF (RootZone%Flags%lComputeETFromGW) &                                                                    !NVRV groundwater inflow for ET
            CALL RootZone%RootZoneZoneBudRawFile%WriteData(NLayers,f_iElemDataType,98,iLayer,rNVRVETGW)                
        IF (RootZone%Flags%lGenericMoistureFile_Defined) &                                                        !NVRV other inflow
            CALL RootZone%RootZoneZoneBudRawFile%WriteData(NLayers,f_iElemDataType,99,iLayer,rNVRVOthIn)                
        CALL RootZone%RootZoneZoneBudRawFile%WriteData(NLayers,f_iElemDataType,100,iLayer,rNVRVStrmInflow)        !NVRV stream inflow for riparian ET
        CALL RootZone%RootZoneZoneBudRawFile%WriteData(NLayers,f_iElemDataType,101,iLayer,rNVRVETa)               !NVRV actual ET
        CALL RootZone%RootZoneZoneBudRawFile%WriteData(NLayers,f_iElemDataType,102,iLayer,rNVRVPerc)              !NVRV perc
        CALL RootZone%RootZoneZoneBudRawFile%WriteData(NLayers,f_iElemDataType,103,iLayer,rNVRVEndStor)           !NVRV end storage
        CALL RootZone%RootZoneZoneBudRawFile%WriteData(NLayers,f_iElemDataType,104,iLayer,rNVRVError)             !NVRV error
    END IF
