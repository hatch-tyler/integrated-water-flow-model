...
MODULE RootZone_v40
  ...
  USE Class_NonPondedAgLandUse_v40    , ONLY: NonPondedAgLandUse_v40_Type                     
  USE Class_PondedAgLandUse_v40       , ONLY: PondedAgLandUse_v40_Type                    , &
                                              f_iNPondedCrops
  USE Class_UrbanLandUse_v40          , ONLY: UrbanLandUse_v40_Type 
  USE Class_NativeRiparianLandUse_v40 , ONLY: NativeRiparianLandUse_v40_Type                  
...
! ******************************************************************
  ...
  TYPE,EXTENDS(BaseRootZoneType) :: RootZone_v40_Type
    ...
    TYPE(NonPondedAgLandUse_v40_Type)    :: NonPondedAgRootZone                                    !Non-ponded ag database
    TYPE(PondedAgLandUse_v40_Type)       :: PondedAgRootZone                                       !Rice/refuge database
    TYPE(UrbanLandUse_v40_Type)          :: UrbanRootZone                                          !Urban database
    TYPE(NativeRiparianLandUse_v40_Type) :: NVRVRootZone                                           !Native/riparian database
  ...
  CONTAINS
    ...
    PROCEDURE,PASS   :: GetBudget_AnnualFlows_GivenRootZone   => RootZone_v40_GetBudget_AnnualFlows_GivenRootZone
    PROCEDURE,NOPASS :: GetBudget_AnnualFlows_GivenFile       => RootZone_v40_GetBudget_AnnualFlows_GivenFile
    ...
    PROCEDURE,PASS   :: GetElementRunoff                      => RootZone_v40_GetElementRunoff
    PROCEDURE,PASS   :: GetElementAppliedWater                => RootZone_v40_GetElementAppliedWater
    PROCEDURE,PASS   :: GetElementAppliedWater_ForLUType      => RootZone_v40_GetElementAppliedWater_ForLUType
    PROCEDURE,PASS   :: GetElementReturnFlow                  => RootZone_v40_GetElementReturnFlow
    PROCEDURE,PASS   :: GetElementReturnFlow_ForLUType        => RootZone_v40_GetElementReturnFlow_ForLUType
    PROCEDURE,PASS   :: GetElementPrecipInfilt                => RootZone_v40_GetElementPrecipInfilt
    PROCEDURE,PASS   :: GetElementInfilt                      => RootZone_v40_GetElementInfilt
    PROCEDURE,PASS   :: GetElementActualET                    => RootZone_v40_GetElementActualET
    PROCEDURE,PASS   :: GetElementPondDrain                   => RootZone_v40_GetElementPondDrain
    PROCEDURE,PASS   :: GetElementPondDrain_ForLUType         => RootZone_v40_GetElementPondDrain_ForLUType
    ...
    PROCEDURE,NOPASS :: GetLandUseAreasForTimePeriod          => RootZone_v40_GetLandUseAreasForTimePeriod
    ...
    PROCEDURE,PASS   :: GetElementPerc                        => RootZone_v40_GetElementPerc
...
! ******************************************************************
  ...
  SUBROUTINE RootZone_v40_New(RootZone,IsForInquiry,cProjectNameForDSS,cFileName,cCropCoeffFileName,cWorkingDirectory,AppGrid,TimeStep,NTIME,ET,Precip,iStat,iStrmNodeIDs,iLakeIDs)
    ...
    !Initialize
    cVersionFull           = 'v4.0-' // TRIM(IWFMKernelVersion%GetVersion())
    ...
    GenericMoistureFile = StripTextUntilCharacter(GenericMoistureFile,f_cInlineCommentChar) 
    ...
    AgWaterDemandFile = StripTextUntilCharacter(AgWaterDemandFile,f_cInlineCommentChar) 
    ...
    CALL RootZoneParamFile%ReadData(ALine,iStat)  ;  IF (iStat .EQ. -1) RETURN
    CALL CleanSpecialCharacters(ALine)
    RootZone%VarTimeUnit = ADJUSTL(StripTextUntilCharacter(ALine,f_cInlineCommentChar))
    
    !Backward compatibility: Check if the user entered KPonded values at all
    CALL RootZoneParamFile%ReadData(ALine,iStat)  ;  IF (iStat .EQ. -1) RETURN
    CALL CleanSpecialCharacters(ALine)  ;  ALine = ADJUSTL(StripTextUntilCharacter(ALine,f_cInlineCommentChar)) 
...
! ******************************************************************
  ...
  ! -------------------------------------------------------------
  SUBROUTINE RootZone_v40_GetBudget_MonthlyFlows_GivenRootZone(RootZone,iBudgetType,iLUType,iSubregionIndex,cBeginDate,cEndDate,rFactVL,rFlows,cFlowNames,iStat)
    CLASS(RootZone_v40_Type),TARGET,INTENT(IN) :: RootZone
    ...
    !Get the values
    CALL RootZone_v40_GetBudget_MonthlyFlows_GivenFile(pBudget,iBudgetType,iLUType,iSubregionIndex,cBeginDate,cEndDate,rFactVL,rFlows,cFlowNames,iStat) 
  ...
  ! --- (Assumes cBeginDate and cEndDate are adjusted properly)
  ! -------------------------------------------------------------
  SUBROUTINE RootZone_v40_GetBudget_MonthlyFlows_GivenFile(Budget,iBudgetType,iLUType,iSubregionIndex,cBeginDate,cEndDate,rFactVL,rFlows,cFlowNames,iStat,Logger)
  ...
  ! --- (Assumes cBeginDate and cEndDate are adjusted properly)
  ! -------------------------------------------------------------
  SUBROUTINE RootZone_v40_GetBudget_AnnualFlows_GivenRootZone(RootZone,iBudgetType,iLUType,iSubregionIndex,cBeginDate,cEndDate,rFactVL,rFlows,cFlowNames,iStat)
    CLASS(RootZone_v40_Type),TARGET,INTENT(IN) :: RootZone
    CHARACTER(LEN=*),INTENT(IN)                :: cBeginDate,cEndDate
    INTEGER,INTENT(IN)                         :: iBudgetType,iLUType,iSubregionIndex  
    REAL(8),INTENT(IN)                         :: rFactVL
    ...
    !Get the values
    CALL RootZone_v40_GetBudget_AnnualFlows_GivenFile(pBudget,iBudgetType,iLUType,iSubregionIndex,cBeginDate,cEndDate,rFactVL,rFlows,cFlowNames,iStat) 
    
  END SUBROUTINE RootZone_v40_GetBudget_AnnualFlows_GivenRootZone
  ...
  ! --- (Assumes cBeginDate and cEndDate are adjusted properly)
  ! -------------------------------------------------------------
  SUBROUTINE RootZone_v40_GetBudget_AnnualFlows_GivenFile(Budget,iBudgetType,iLUType,iSubregionIndex,cBeginDate,cEndDate,rFactVL,rFlows,cFlowNames,iStat,Logger)
    TYPE(BudgetType),INTENT(IN)                        :: Budget      !Assumes Budget file is already open
    CHARACTER(LEN=*),INTENT(IN)                        :: cBeginDate,cEndDate
    INTEGER,INTENT(IN)                                 :: iBudgetType,iLUType,iSubregionIndex
    REAL(8),INTENT(IN)                                 :: rFactVL
    ...
    !Local variables
    CHARACTER(LEN=ModNameLen+44) :: ThisProcedure = ModName // 'RootZone_v40_GetBudget_AnnualFlows_GivenFile'
    INTEGER,TARGET               :: iDimActual,iNTimeSteps,                         &
                                    iReadCols_LWU_Ag(5) = [3,4,5,6,7],              &
                                    iReadCols_LWU_Urb(5) = [12,13,14,15,16],        &
                                    iReadCols_RZ_Ag(8) = [9,10,11,12,13,14,15,16],  &
  ...
  END SUBROUTINE RootZone_v40_GetBudget_AnnualFlows_GivenFile

  
  ! -------------------------------------------------------------
  ! --- GET BUDGET TIME SERIES DATA FOR A SET OF COLUMNS 
  ! -------------------------------------------------------------
  SUBROUTINE RootZone_v40_GetBudget_TSData(RootZone,iBudgetType,iSubregionIndex,iCols,cBeginDate,cEndDate,cInterval,rFactLT,rFactAR,rFactVL,rOutputDates,rOutputValues,iDataTypes,inActualOutput,iStat)
    CLASS(RootZone_v40_Type),INTENT(IN) :: RootZone
  ...
  ! -------------------------------------------------------------
  SUBROUTINE RootZone_v40_GetElementRunoff(RootZone,iElemRegions,rRunoff)
    CLASS(RootZone_v40_Type),INTENT(IN) :: RootZone
    ...
    IF (RootZone%Flags%lNVRV_Defined) rRunoff = rRunoff + RootZone%NVRVRootZone%NativeVeg%Runoff(:,1) + RootZone%NVRVRootZone%NativeVeg%Runoff(:,1)
    
  END SUBROUTINE RootZone_v40_GetElementRunoff
  
  
  ! -------------------------------------------------------------
  ! --- GET APPLIED WATER AT ALL ELEMENTS
  ! -------------------------------------------------------------
  SUBROUTINE RootZone_v40_GetElementAppliedWater(RootZone,iElemRegions,rAW)
    CLASS(RootZone_v40_Type),INTENT(IN) :: RootZone
    INTEGER,INTENT(IN)                  :: iElemRegions(:)  !Not used in this version
  ...
  END SUBROUTINE RootZone_v40_GetElementAppliedWater
  
  
  ! -------------------------------------------------------------
  ! --- GET APPLIED WATER AT ALL ELEMENTS FOR A SPECIFIED LAND USE TYPE
  ! -------------------------------------------------------------
  RECURSIVE SUBROUTINE RootZone_v40_GetElementAppliedWater_ForLUType(RootZone,iElemRegions,iLUType,rAW)
    CLASS(RootZone_v40_Type),INTENT(IN) :: RootZone
    INTEGER,INTENT(IN)                  :: iElemRegions(:)  !Not used in this version
    ...
    SELECT CASE (iLUType)
        ...
        CASE (f_iLandUse_Refuge)
            IF (RootZone%Flags%lPondedAg_Defined) THEN
                rDemandFrac = SUM(RootZone%PondedAgRootZone%Crops%ElemDemandFrac_Ag(4:5,:), DIM=1)          !Ratio of rice demand to the total ag demand in the element
                rAW         = (RootZone%ElemSupply%Pumping_Ag + RootZone%ElemSupply%Diversion_Ag) * rDemandFrac
            ELSE
                rAW = 0.0
            END IF
  ...
  END SUBROUTINE RootZone_v40_GetElementAppliedWater_ForLUType
  
  
  ! -------------------------------------------------------------
  ! --- GET RETURN FLOW AT ALL ELEMENTS
  ! -------------------------------------------------------------
  SUBROUTINE RootZone_v40_GetElementReturnFlow(RootZone,iElemRegions,rReturn)
    CLASS(RootZone_v40_Type),INTENT(IN) :: RootZone
    INTEGER,INTENT(IN)                  :: iElemRegions(:)  !Not used in this version
  ...
  END SUBROUTINE RootZone_v40_GetElementReturnFlow
  
  
  ! -------------------------------------------------------------
  ! --- GET RETURN FLOW AT ALL ELEMENTS FOR A SPECIFIED LAND USE TYPE
  ! -------------------------------------------------------------
  RECURSIVE SUBROUTINE RootZone_v40_GetElementReturnFlow_ForLUType(RootZone,iElemRegions,iLUType,rReturn)
    CLASS(RootZone_v40_Type),INTENT(IN) :: RootZone
    INTEGER,INTENT(IN)                  :: iElemRegions(:)  !Not used in this version
    INTEGER,INTENT(IN)                  :: iLUType
    REAL(8),INTENT(OUT)                 :: rReturn(:)
    ...
    SELECT CASE (iLUType)
        ...
        CASE (f_iLandUse_Rice)
            IF (RootZone%Flags%lPondedAg_Defined) THEN
                rReturn = SUM(RootZone%PondedAgRootZone%Crops%ReturnFlow(1:3,:) , DIM=1) 
            ELSE
                rReturn = 0.0
            END IF
        ...
        CASE (f_iLandUse_Refuge)
            IF (RootZone%Flags%lPondedAg_Defined) THEN
                rReturn = SUM(RootZone%PondedAgRootZone%Crops%ReturnFlow(4:5,:) , DIM=1) 
            ELSE
                rReturn = 0.0
            END IF
  ...
  END SUBROUTINE RootZone_v40_GetElementReturnFlow_ForLUType
  ...
  SUBROUTINE RootZone_v40_GetElementPrecipInfilt(RootZone,ElemRegion,PrecipInfilt)
    ...
    IF (RootZone%Flags%lNVRV_Defined) &
        PrecipInfilt = PrecipInfilt + RootZone%NVRVRootZone%NativeVeg%PrecipInfilt(:,1)   * RootZone%NVRVRootZone%NativeVeg%Area(:,1)    &
                                    + RootZone%NVRVRootZone%RiparianVeg%PrecipInfilt(:,1) * RootZone%NVRVRootZone%RiparianVeg%Area(:,1)
  ...
  ! -------------------------------------------------------------
  SUBROUTINE RootZone_v40_GetElementInfilt(RootZone,iElemRegion,rInfilt)
    CLASS(RootZone_v40_Type),INTENT(IN) :: RootZone
    ...
    !From native and riparian veg
    IF (RootZone%Flags%lNVRV_Defined) &
        rInfilt = rInfilt + RootZone%NVRVRootZone%NativeVeg%PrecipInfilt(:,1) + RootZone%NVRVRootZone%RiparianVeg%PrecipInfilt(:,1) 
    
  END SUBROUTINE RootZone_v40_GetElementInfilt
  ...
  SUBROUTINE RootZone_v40_GetElementActualET(RootZone,ElemRegion,ET)
    ...
    !From native and riparian veg
    IF (RootZone%Flags%lNVRV_Defined) &
        ET = ET + RootZone%NVRVRootZone%NativeVeg%ETa(:,1) + RootZone%NVRVRootZone%RiparianVeg%ETa(:,1) 
  ...
  ! -------------------------------------------------------------
  SUBROUTINE RootZone_v40_GetElementPondDrain(RootZone,iElemRegions,rDrain)
    CLASS(RootZone_v40_Type),INTENT(IN) :: RootZone
    INTEGER,INTENT(IN)                  :: iElemRegions(:)  !Not used in this version
  ...
  END SUBROUTINE RootZone_v40_GetElementPondDrain
  
  
  ! -------------------------------------------------------------
  ! --- GET POND DRAIN AT ALL ELEMENTS FOR A SPECIFIED LAND USE TYPE
  ! -------------------------------------------------------------
  SUBROUTINE RootZone_v40_GetElementPondDrain_ForLUType(RootZone,iElemRegions,iLUType,rDrain)
    CLASS(RootZone_v40_Type),INTENT(IN) :: RootZone
    INTEGER,INTENT(IN)                  :: iElemRegions(:)  !Not used in this version
    INTEGER,INTENT(IN)                  :: iLUType
    REAL(8),INTENT(OUT)                 :: rDrain(:)
    ...
    SELECT CASE(iLUType)
        ...
        CASE (f_iLandUse_Rice)    
            rDrain = SUM(RootZone%PondedAgRootZone%Crops%Drain(1:3,:) , DIM=1)
            
        CASE (f_iLandUse_Refuge)    
            rDrain = SUM(RootZone%PondedAgRootZone%Crops%Drain(4:5,:) , DIM=1)
            
        CASE DEFAULT 
            rDrain = 0.0
    END SELECT
    
  END SUBROUTINE RootZone_v40_GetElementPondDrain_ForLUType
  ...
  ! -------------------------------------------------------------
  SUBROUTINE RootZone_v40_GetLandUseAreasForTimePeriod(cRootZoneMainFileName,cWorkingDirectory,cBeginDate,cEndDate,TimeStep,AppGrid,iLUType,iLU,rLUAreas,iStat,Logger)
    ...
    !Local variables
    CHARACTER(LEN=ModNameLen+41)         :: ThisProcedure = ModName // 'RootZone_v40_GetLandUseAreasForTimePeriod'
    ...
    TYPE(NonPondedAgLandUse_v40_Type)    :: DummyNonPondedAg
    TYPE(PondedAgLandUse_v40_Type)       :: DummyPondedAg
    TYPE(UrbanLandUse_v40_Type)          :: DummyUrban
    TYPE(NativeRiparianLandUse_v40_Type) :: DummyNVRV
    ...
    !Read the version number and skip 3 lines of input data (total of 4 lines)
    DO indx=1,4
    ...
    SELECT CASE (iLUType)
        ...
        CASE (f_iLandUse_PondedAg)
            CALL DummyPondedAg%GetAreasForTimePeriod(cPondedCropFileName,cWorkingDirectory,cBeginDate,cEndDate,TimeStep,AppGrid,iLU,rLUAreas,iStat)
 ...
 10 CALL RootZoneMainFile%Kill() 
  ...
  END SUBROUTINE RootZone_v40_GetLandUseAreasForTimePeriod
  ...
  SUBROUTINE RootZone_v40_GetElementNativeVegAreas(RootZone,Areas)
    ...
    IF (RootZone%Flags%lNVRV_Defined) THEN
        Areas = RootZone%NVRVRootZone%NativeVeg%Area(:,1)
    ELSE
        Areas = 0.0
    END IF
  ...
  SUBROUTINE RootZone_v40_GetElementRiparianVegAreas(RootZone,Areas)
    ...
    IF (RootZone%Flags%lNVRV_Defined) THEN
        Areas = RootZone%NVRVRootZone%RiparianVeg%Area(:,1)
    ELSE
        Areas = 0.0
    END IF
  ...
  SUBROUTINE RootZone_v40_GetSubregionAgAreas(RootZone,AppGrid,Areas)
    ...
    Areas = RegionalLUARea(AppGrid,RootZone,f_iLandUse_Ag)
  ...
  SUBROUTINE RootZone_v40_GetSubregionUrbanAreas(RootZone,AppGrid,Areas)
    ...
    Areas = RegionalLUARea(AppGrid,RootZone,f_iLandUse_Urb)
  ...
  ! -------------------------------------------------------------
  SUBROUTINE RootZone_v40_GetElementSoilMVolume(RootZone,AppGrid,lBeginSoilM,SoilM)
    CLASS(RootZone_v40_Type),TARGET,INTENT(IN) :: RootZone
    TYPE(AppGridType),INTENT(IN)               :: AppGrid   
    ...
    IF (RootZone%Flags%lNVRV_Defined) THEN
        !Native veg
        IF (lBeginSoilM) THEN
            pSoilM_Precip => RootZone%NVRVRootZone%NativeVeg%SoilM_Precip_P
            pSoilM_AW     => RootZone%NVRVRootZone%NativeVeg%SoilM_AW_P
            pSoilM_Oth    => RootZone%NVRVRootZone%NativeVeg%SoilM_Oth_P
        ELSE
            pSoilM_Precip => RootZone%NVRVRootZone%NativeVeg%SoilM_Precip
            pSoilM_AW     => RootZone%NVRVRootZone%NativeVeg%SoilM_AW
            pSoilM_Oth    => RootZone%NVRVRootZone%NativeVeg%SoilM_Oth
        END IF
        SoilM = SoilM + (pSoilM_Precip(:,1) + pSoilM_AW(:,1) + pSoilM_Oth(:,1)) * RootZone%NVRVRootZone%NativeVeg%Area(:,1)
        
        !Riparian veg
        IF (lBeginSoilM) THEN
            pSoilM_Precip => RootZone%NVRVRootZone%RiparianVeg%SoilM_Precip_P
            pSoilM_AW     => RootZone%NVRVRootZone%RiparianVeg%SoilM_AW_P
            pSoilM_Oth    => RootZone%NVRVRootZone%RiparianVeg%SoilM_Oth_P
        ELSE
            pSoilM_Precip => RootZone%NVRVRootZone%RiparianVeg%SoilM_Precip
            pSoilM_AW     => RootZone%NVRVRootZone%RiparianVeg%SoilM_AW
            pSoilM_Oth    => RootZone%NVRVRootZone%RiparianVeg%SoilM_Oth
        END IF
        SoilM = SoilM + (pSoilM_Precip(:,1) + pSoilM_AW(:,1) + pSoilM_Oth(:,1)) * RootZone%NVRVRootZone%RiparianVeg%Area(:,1)
    END IF
  ...
  ! -------------------------------------------------------------
  SUBROUTINE RootZone_v40_GetElementPerc(RootZone,iElemRegions,rPerc)
    CLASS(RootZone_v40_Type),INTENT(IN) :: RootZone
    ...
    !Native and riparian vegetation areas
    IF (RootZone%Flags%lNVRV_Defined) rPerc = rPerc + RootZone%NVRVRootZone%NativeVeg%Perc(:,1)   + RootZone%NVRVRootZone%RiparianVeg%Perc(:,1)  &
                                                    + RootZone%NVRVRootZone%NativeVeg%PercCh(:,1) + RootZone%NVRVRootZone%RiparianVeg%PercCh(:,1) 
    
  END SUBROUTINE RootZone_v40_GetElementPerc
  ...
  FUNCTION RootZone_v40_GetPercElement(RootZone,iElem,AppGrid) RESULT(Perc)
    ...
    !Native and riparian vegetation areas
    IF (RootZone%Flags%lNVRV_Defined) Perc = Perc + RootZone%NVRVRootZone%NativeVeg%Perc(iElem,1)   + RootZone%NVRVRootZone%RiparianVeg%Perc(iElem,1)  &
  ...
  ! -------------------------------------------------------------
  SUBROUTINE RootZone_v40_GetFlowsToStreams(RootZone,AppGrid,DirectRunoff,ReturnFlow,PondDrain,RiparianET)
    CLASS(RootZone_v40_Type),INTENT(IN) :: RootZone
  ...
  ! -------------------------------------------------------------
  SUBROUTINE RootZone_v40_GetFlowsToLakes(RootZone,AppGrid,DirectRunoff,ReturnFlow,PondDrain)
    CLASS(RootZone_v40_Type),INTENT(IN) :: RootZone
    TYPE(AppGridType),INTENT(IN)        :: AppGrid                                     !Not used in this version
    REAL(8),INTENT(OUT)                 :: DirectRunoff(:),ReturnFlow(:),PondDrain(:)
...
! ******************************************************************
  ...
  SUBROUTINE RootZone_v40_SetLakeElemFlag(RootZone,iLakeElem)
    ...
    END IF
    IF (RootZone%Flags%lNVRV_Defined) THEN
      RootZone%NVRVRootZone%NativeVeg%SoilM_Precip(iLakeElem,1)     = 0.0
      RootZone%NVRVRootZone%NativeVeg%SoilM_AW(iLakeElem,1)         = 0.0
      RootZone%NVRVRootZone%NativeVeg%SoilM_Precip_P(iLakeElem,1)   = 0.0
      RootZone%NVRVRootZone%NativeVeg%SoilM_AW_P(iLakeElem,1)       = 0.0
      RootZone%NVRVRootZone%RiparianVeg%SoilM_Precip(iLakeElem,1)   = 0.0
      RootZone%NVRVRootZone%RiparianVeg%SoilM_AW(iLakeElem,1)       = 0.0
      RootZone%NVRVRootZone%RiparianVeg%SoilM_Precip_P(iLakeElem,1) = 0.0
      RootZone%NVRVRootZone%RiparianVeg%SoilM_AW_P(iLakeElem,1)     = 0.0
    END IF
...
! ******************************************************************
  ...
  SUBROUTINE RootZone_v40_ReadTSData(RootZone,AppGrid,TimeStep,Precip,ETData,iStat,RegionLUAreas,iColCropCoeff_NonPondedAg,iColCropCoeff_PondedAg,iColCropCoeff_Urban,iColCropCoeff_NVRV)
    ...
    CALL ReadReturnFlowFractions(RootZone%Logger,TimeStep,RootZone%ReturnFracFile,lReturnFracUpdated,iStat)
    IF (iStat .EQ. -1) RETURN

    !Read re-use fractions
    CALL ReadReuseFractions(RootZone%Logger,TimeStep,RootZone%ReuseFracFile,lReuseFracUpdated,iStat)
    IF (iStat .EQ. -1) RETURN
    ...
    IF (PRESENT(RegionLUAreas)) THEN
        ...
        END IF
        IF (RootZone%Flags%lNVRV_Defined) THEN
            RootZone%NVRVRootZone%NativeVeg%Area(:,1)   = ExIntAreas(NLandUse+1,:)
            RootZone%NVRVRootZone%RiparianVeg%Area(:,1) = ExIntAreas(NLandUse+2,:)  
        END IF
    ...
    IF (ETData%IsUpdated()) THEN
        ...
        IF (RootZone%Flags%lNVRV_Defined) THEN
            CALL ComputeRegionalETPot(AppGrid,ETData,RootZone%NVRVRootZone%NativeVeg%iColETc(:,1),RootZone%NVRVRootZone%NativeVeg%Area(:,1),RootZone%NVRVRootZone%RegionETPot_NV)
            CALL ComputeRegionalETPot(AppGrid,ETData,RootZone%NVRVRootZone%RiparianVeg%iColETc(:,1),RootZone%NVRVRootZone%RiparianVeg%Area(:,1),RootZone%NVRVRootZone%RegionETPot_RV)
        END IF
    END IF
    ...
    ASSOCIATE (pNPCrops => RootZone%NonPondedAgRootZone%Crops , &
        ...
        DO indxElem=1,NElements
            ...
            IF (RootZone%Flags%lNVRV_Defined) THEN
                IF (pNV%Area(indxElem,1) .EQ. 0.0) THEN
                    pNV%SoilM_Precip_P(indxElem,1) = 0.0
                    pNV%SoilM_Precip(indxElem,1)   = 0.0
                END IF  
                IF (pRV%Area(indxElem,1) .EQ. 0.0) THEN
                    pRV%SoilM_Precip_P(indxElem,1) = 0.0
                    pRV%SoilM_Precip(indxElem,1)   = 0.0
                END IF   
            END IF   
        END DO
    END ASSOCIATE    
...
! ******************************************************************
  ...
  SUBROUTINE RootZone_v40_PrintResults(RootZone,AppGrid,ETData,TimeStep,lEndOfSimulation,iColCropCoeff_NonPondedAg,iColCropCoeff_PondedAg,iColCropCoeff_Urban,iColCropCoeff_NVRV)
    ...
    ASSOCIATE (pFlags            => RootZone%Flags                                , &
      ...
      IF (pFlags%LWUseBudRawFile_Defined .OR. pFlags%RootZoneBudRawFile_Defined) THEN
        RPump_Ag              = RegionalPumping(AppGrid,RootZone,f_iLandUse_Ag)
        RPump_Urb             = RegionalPumping(AppGrid,RootZone,f_iLandUse_Urb)
        RDeli_Ag              = RegionalDeliveries(AppGrid,RootZone,f_iLandUse_Ag)
        RDeli_Urb             = RegionalDeliveries(AppGrid,RootZone,f_iLandUse_Urb)
        ...
        RLUArea_Ag            = RegionalLUArea(AppGrid,RootZone,f_iLandUse_Ag)
        RLUArea_Urb           = RegionalLUArea(AppGrid,RootZone,f_iLandUse_Urb)
        RLUArea_NV            = RegionalLUArea(AppGrid,RootZone,f_iLandUse_NVRV)
        IF (pFlags%lGenericMoistureFile_Defined) THEN
            RGenericMoist_Ag  = RegionalGenericMoistInflow(AppGrid,RootZone,f_iLandUse_Ag)
            RGenericMoist_Urb = RegionalGenericMoistInflow(AppGrid,RootZone,f_iLandUse_Urb)
            RGenericMoist_NV  = RegionalGenericMoistInflow(AppGrid,RootZone,f_iLandUse_NVRV)
        ELSE
    ...
    ASSOCIATE (pOutFile     => RootZone%FinalMoistureOutfile , &
      ...
      IF (pFlags%lPondedAg_Defined) THEN
        ...
        DO indxElem=1,NElements
          ...
          DO indxCrop=1,f_iNPondedCrops
            WRITE (cArrayP(indxCrop+1),'(F8.6)') (pPondedAg%Crops%SoilM_Precip(indxCrop,indxElem) + pPondedAg%Crops%SoilM_AW(indxCrop,indxElem) + pPondedAg%Crops%SoilM_Oth(indxCrop,indxElem)) / pPondedAg%RootDepth(indxCrop)
          END DO
          CALL pOutFile%WriteData(cArrayP,FormatSpec=FormatSpec)
        END DO
      END IF
      ...
      IF (pFlags%lUrban_Defined) THEN
        ...
        !Values
        DO indxElem=1,SIZE(pUrban%UrbData%SMax , DIM=1)
          cArrayU    = ''
      ...
      IF (pFlags%lNVRV_Defined) THEN
        ...
        !Values
        DO indxElem=1,SIZE(pNVRV%NativeVeg%SMax , Dim=1)
          cArrayNVRV    = ''
          cArrayNVRV(1) = ADJUSTR(TRIM(IntToText(iElemIDs(indxElem))))
          WRITE (cArrayNVRV(2),'(F8.6)') (pNVRV%NativeVeg%SoilM_Precip(indxElem,1) + pNVRV%NativeVeg%SoilM_AW(indxElem,1) + pNVRV%NativeVeg%SoilM_Oth(indxElem,1)) / pNVRV%RootDepth_Native
          WRITE (cArrayNVRV(3),'(F8.6)') (pNVRV%RiparianVeg%SoilM_Precip(indxElem,1) + pNVRV%RiparianVeg%SoilM_AW(indxElem,1) + pNVRV%RiparianVeg%SoilM_Oth(indxElem,1)) / pNVRV%RootDepth_Riparian
          CALL pOutFile%WriteData(cArrayNVRV,FormatSpec=FormatSpec)
        END DO
      END IF
  ...
  SUBROUTINE WriteRootZoneFlowsToBudRawFile(AppGrid,RPump_Ag,RDeli_Ag,RGenericMoist_Ag,RPump_Urb,RDeli_Urb,RGenericMoist_Urb,RUpstrmElemRunoff_Ag,RUpstrmElemRunoff_Urb,RUpstrmElemRunoff_NV,RLUArea_Ag,RLUArea_Urb,RLUArea_NV,RGenericMoist_NV,RootZone)
    ...
    ASSOCIATE (pFlags => RootZone%Flags)
      ...
      IF (pFlags%lNonPondedAg_Defined .OR. pFlags%lPondedAg_Defined) THEN
          CALL RegionalETPot(AppGrid,RootZone,f_iLandUse_Ag,RETPot_Ag)
          RPrecip_Ag  = RegionalPrecip(AppGrid,RootZone,f_iLandUse_Ag)
          RRunoff_Ag  = RegionalRunoff(AppGrid,RootZone,f_iLandUse_Ag)
          RReuse_Ag   = RegionalReuse(AppGrid,RootZone,f_iLandUse_Ag)
          RReturn_Ag  = RegionalReturn(AppGrid,RootZone,f_iLandUse_Ag)
          RSoilMCh_Ag = RegionalSoilMChange(AppGrid,RootZone,f_iLandUse_Ag) 
          RInfilt_Ag  = RegionalInfiltration(AppGrid,RootZone,f_iLandUse_Ag)
          RDrain_Ag   = RegionalDrain(AppGrid,RootZone,f_iLandUse_Ag)
          RETa_Ag     = RegionalETa(AppGrid,RootZone,f_iLandUse_Ag) 
          RPerc_Ag    = RegionalPerc_ForLU(AppGrid,RootZone,f_iLandUse_Ag)
          Error_Ag    = RootZone%RSoilM_P(:,1) + RSoilMCh_Ag + RInfilt_Ag + RGenericMoist_Ag - RDrain_Ag - RETa_Ag - RPerc_Ag - RootZone%RSoilM(:,1)
      END IF
            
      !Urban
      IF (pFlags%lUrban_Defined) THEN
          CALL RegionalETPot(AppGrid,RootZone,f_iLandUse_Urb,RETPot_Urb)
          RPrecip_Urb  = RegionalPrecip(AppGrid,RootZone,f_iLandUse_Urb)
          RRunoff_Urb  = RegionalRunoff(AppGrid,RootZone,f_iLandUse_Urb)
          RReuse_Urb   = RegionalReuse(AppGrid,RootZone,f_iLandUse_Urb)
          RReturn_Urb  = RegionalReturn(AppGrid,RootZone,f_iLandUse_Urb)
          RSoilMCh_Urb = RegionalSoilMChange(AppGrid,RootZone,f_iLandUse_Urb) 
          RInfilt_Urb  = RegionalInfiltration(AppGrid,RootZone,f_iLandUse_Urb)
          RETa_Urb     = RegionalETa(AppGrid,RootZone,f_iLandUse_Urb)
      ...
      IF (pFlags%lNVRV_Defined) THEN
          CALL RegionalETPot(AppGrid,RootZone,f_iLandUse_NVRV,RETPot_NV)
          RPrecip_NV  = RegionalPrecip(AppGrid,RootZone,f_iLandUse_NVRV)       
          RRunoff_NV  = RegionalRunoff(AppGrid,RootZone,f_iLandUse_NVRV)
          RSoilMCh_NV = RegionalSoilMChange(AppGrid,RootZone,f_iLandUse_NVRV) 
          RInfilt_NV  = RegionalInfiltration(AppGrid,RootZone,f_iLandUse_NVRV)
          RETa_NV     = RegionalETa(AppGrid,RootZone,f_iLandUse_NVRV)
    ...
    DummyArray(16,:) = RootZone%RSoilM(:,1)                                        !Storage at the end of the time interval
    ...
    DummyArray(26,:) = RootZone%RSoilM_P(:,2)                                      !Storage at the beginning of the time interval
    ...
    DummyArray(32,:) = RootZone%RSoilM(:,2)                                        !Storage at the end of the time interval     
    ...
    DummyArray(39,:) = RootZone%RSoilM_P(:,3)                                      !Storage at the beginning of the time interval
    ...
    DummyArray(45,:) = RootZone%RSoilM(:,3)                                        !Storage at the end of the time interval          
  ...
  SUBROUTINE WriteLWUseFlowsToBudRawFile(AppGrid,RLUArea_Ag,RLUArea_Urb,RPump_Ag,RPump_Urb,RDeli_Ag,RDeli_Urb,RUpstrmElemRunoff_Ag,RUpstrmElemRunoff_Urb,RootZone)
    ...
    IF (RootZone%Flags%lNonPondedAg_Defined .OR. RootZone%Flags%lPondedAg_Defined) THEN
      ...
      RDemand_Ag      = RegionalDemand(AppGrid,RootZone,f_iLandUse_Ag)
    ...
    IF (RootZone%Flags%lUrban_Defined) THEN
      RDemand_Urb      = RegionalDemand(AppGrid,RootZone,f_iLandUse_Urb)
      ...
      DO indxIter=1,pSolverData%IterMax
          ...
          IF (RootZone%Flags%lUrban_Defined) THEN 
              ...
              CALL RootZone%UrbanRootZone%Simulate(AppGrid                   , &
                                                   ...
                                                   iStat                     )
              IF (iStat .EQ. -1) RETURN
              SurfaceFlow = RootZone%UrbanRootZone%UrbData%Runoff(:,1) + RootZone%UrbanRootZone%UrbData%ReturnFlowIn(:,1) + RootZone%UrbanRootZone%UrbData%ReturnFlowOut(:,1)
              CALL FlowToElements(SurfaceFlow              , &
                                  AppGrid                  , &
                                  RootZone                 , &
                                  f_iLandUse_NVRV          , &
          ...
          IF (RootZone%Flags%lNVRV_Defined) THEN
              ...
              CALL pNVRV%Simulate(AppGrid                   , &
                                  ...
                                  iStat                     )
              IF (iStat .EQ. -1) RETURN
              SurfaceFlow = pNVRV%NativeVeg%Runoff(:,1) + pNVRV%RiparianVeg%Runoff(:,1)
              CALL FlowToElements(SurfaceFlow              , &
                                  AppGrid                  , &
                                  RootZone                 , &
                                  f_iLandUse_NVRV          , &
...
! ******************************************************************
  ...
  ! -------------------------------------------------------------
  FUNCTION RegionalDemand(AppGrid,RootZone,iLUType) RESULT(RDemand)
    TYPE(AppGridType),INTENT(IN)       :: AppGrid
  ...
  ! -------------------------------------------------------------
  FUNCTION RegionalReuse(AppGrid,RootZone,iLUType) RESULT(RReuse)
    TYPE(AppGridType),INTENT(IN)       :: AppGrid
    TYPE(RootZone_v40_Type),INTENT(IN) :: RootZone
    INTEGER,INTENT(IN)                 :: iLUType
  ...
  ! -------------------------------------------------------------
  FUNCTION RegionalPrecip(AppGrid,RootZone,iLUType) RESULT(RPrecip)
    TYPE(AppGridType),INTENT(IN)       :: AppGrid
    TYPE(RootZone_v40_Type),INTENT(IN) :: RootZone
    INTEGER,INTENT(IN)                 :: iLUType
    ...
    SELECT CASE (iLUType)
      ...
      !Native and riparian vegetation
      CASE (f_iLandUse_NVRV)
        ElemValue = RootZone%ElemPrecipData%Precip * (RootZone%NVRVRootZone%NativeVeg%Area(:,1) + RootZone%NVRVRootZone%RiparianVeg%Area(:,1))
  ...
  ! -------------------------------------------------------------
  FUNCTION RegionalGenericMoistInflow(AppGrid,RootZone,iLUType) RESULT(RGenericMoist)
    TYPE(AppGridType),INTENT(IN)       :: AppGrid
    TYPE(RootZone_v40_Type),INTENT(IN) :: RootZone
    INTEGER,INTENT(IN)                 :: iLUType
      ...
      CASE (f_iLandUse_NVRV)
        IF (RootZone%Flags%lNVRV_Defined) THEN
          ASSOCIATE (pNVRVRootZone => RootZone%NVRVRootZone)
            ElemValue = (RootZone%GenericMoistureData%rGenericMoisture(1,:) * pNVRVRootZone%RootDepth_Native   - pNVRVRootZone%NativeVeg%GMExcess(:,1))   * pNVRVRootZone%NativeVeg%Area(:,1)    &
                      + (RootZone%GenericMoistureData%rGenericMoisture(1,:) * pNVRVRootZone%RootDepth_Riparian - pNVRVRootZone%RiparianVeg%GMExcess(:,1)) * pNVRVRootZone%RiparianVeg%Area(:,1)
          END ASSOCIATE
        END IF
  ...
  ! -------------------------------------------------------------
  FUNCTION RegionalSoilMChange(AppGrid,RootZone,iLUType) RESULT(RSoilMCh)
    TYPE(AppGridType),INTENT(IN)       :: AppGrid
    TYPE(RootZone_v40_Type),INTENT(IN) :: RootZone
    INTEGER,INTENT(IN)                 :: iLUType
    ...
    SELECT CASE (iLUType)
      ...
      !Native and riparian vegetation
      CASE (f_iLandUse_NVRV)
        ElemValue = RootZone%NVRVRootZone%NativeVeg%SoilMCh(:,1) + RootZone%NVRVRootZone%RiparianVeg%SoilMCh(:,1)
  ...
  ! -------------------------------------------------------------
  FUNCTION RegionalETa(AppGrid,RootZone,iLUType) RESULT(RETa)
    TYPE(AppGridType),INTENT(IN)       :: AppGrid
    TYPE(RootZone_v40_Type),INTENT(IN) :: RootZone
    INTEGER,INTENT(IN)                 :: iLUType
    ...
    SELECT CASE (iLUType)
      ...
      !Native and riparian vegetation
      CASE (f_iLandUse_NVRV)
        ElemValue = RootZone%NVRVRootZone%NativeVeg%ETa(:,1) + RootZone%NVRVRootZone%RiparianVeg%ETa(:,1)
  ...
  FUNCTION RegionalPerc_ForLU(AppGrid,RootZone,iLUType) RESULT(RPerc)
    ...
    SELECT CASE (iLUType)
      ...
      !Native and riparian vegetation
      CASE (f_iLandUse_NVRV)
        ElemValue =  RootZone%NVRVRootZone%NativeVeg%Perc(:,1) + RootZone%NVRVRootZone%NativeVeg%PercCh(:,1)  &
  ...
  FUNCTION RegionalInfiltration(AppGrid,RootZone,iLUType) RESULT(RInfilt)
    ...
    SELECT CASE (iLUType)
      ...
      !Native and riparian vegetation
      CASE (f_iLandUse_NVRV)
        ElemValue =  RootZone%NVRVRootZone%NativeVeg%PrecipInfilt(:,1) + RootZone%NVRVRootZone%RiparianVeg%PrecipInfilt(:,1) 
  ...
  ! -------------------------------------------------------------
  FUNCTION RegionalLUArea(AppGrid,RootZone,iLUType) RESULT(RLUArea)
    TYPE(AppGridType),INTENT(IN)       :: AppGrid
    TYPE(RootZone_v40_Type),INTENT(IN) :: RootZone
    INTEGER,INTENT(IN)                 :: iLUType
    ...
    SELECT CASE (iLUType)
      ...
      CASE (f_iLandUse_NVRV)
        ...
        ElemValue = RootZone%NVRVRootZone%NativeVeg%Area(:,1) + RootZone%NVRVRootZone%RiparianVeg%Area(:,1)
        ...
        CASE (f_iLandUse_NVRV)
          IF (RootZone%Flags%lNVRV_Defined) THEN
            ASSOCIATE (pNV => RootZone%NVRVRootZone%NativeVeg    , &
              ...
              DO indxRegionElem=1,pAppSubregion%NRegionElements
                iElem = pAppSubregion%RegionElements(indxRegionElem)
                Area = Area + pNV%Area(iElem,1) + pRV%Area(iElem,1)
              END DO
            END ASSOCIATE
          END IF
  ...
  FUNCTION RegionalRunoff(AppGrid,RootZone,iLUType) RESULT(RRunoff)
    ...
    SELECT CASE (iLUType)
      ...
      !Native and riparian vegetation
      CASE (f_iLandUse_NVRV)
        ElemValue = RootZone%NVRVRootZone%NativeVeg%Runoff(:,1) + RootZone%NVRVRootZone%RiparianVeg%Runoff(:,1)
  ...
  ! -------------------------------------------------------------
  FUNCTION RegionalPumping(AppGrid,RootZone,iLUType) RESULT(RPump)
    TYPE(AppGridType),INTENT(IN)       :: AppGrid
    TYPE(RootZone_v40_Type),INTENT(IN) :: RootZone
    INTEGER,INTENT(IN)                 :: iLUType
  ...
  ! -------------------------------------------------------------
  FUNCTION RegionalDeliveries(AppGrid,RootZone,iLUType) RESULT(RDeli)
    TYPE(AppGridType),INTENT(IN)       :: AppGrid
    TYPE(RootZone_v40_Type),INTENT(IN) :: RootZone
    INTEGER,INTENT(IN)                 :: iLUType
  ...
  FUNCTION RegionalMoistStorage(AppGrid,RootZone) RESULT(RSoilM)
    ...
    ASSOCIATE (pFlags       => RootZone%Flags                     , &
      ...
      IF (pFlags%lNVRV_Defined) THEN
        ElemValues           = (pNV%SoilM_Precip(:,1) + pNV%SoilM_AW(:,1) + pNV%SoilM_Oth(:,1)) * pNV%Area(:,1)  &
                             + (pRV%SoilM_Precip(:,1) + pRV%SoilM_AW(:,1) + pRV%SoilM_Oth(:,1)) * pRV%Area(:,1)
  ...
  ! -------------------------------------------------------------
  SUBROUTINE RegionalETPot(AppGrid,RootZone,iLUType,RETp)
    TYPE(AppGridType),INTENT(IN)       :: AppGrid
    TYPE(RootZone_v40_Type),INTENT(IN) :: RootZone
    INTEGER,INTENT(IN)                 :: iLUType
  ...
  SUBROUTINE ComputeUpstrmElemRunoffToLandUse(AppGrid,UpstrmElemRunoff,RootZone,iLandUseType,InRunoff)
    ...
    SELECT CASE (iLandUseType)
        ...
        CASE (f_iLandUse_NVRV)
            !$OMP PARALLEL DO DEFAULT(SHARED) PRIVATE(indxElem)
            DO indxElem=1,NElements
                InRunoff(1,indxElem) = UpstrmElemRunoff(indxElem) * (RootZone%NVRVRootZone%NativeVeg%Area(indxElem,1)+RootZone%NVRVRootZone%RiparianVeg%Area(indxElem,1)) / ElemArea(indxElem) 
            END DO
            !$OMP END PARALLEL DO
  ...
  SUBROUTINE ComputeDemandSupplyRelatedFracs(AppGrid,RootZone)
    ...
    !Local variables
    INTEGER :: indxElem,NElements,iNCrops,indxRegion,iElem,iNPondedCrops
    ...
    ASSOCIATE (pRatioElemToRegion_Ag  => RootZone%Ratio_ElemSupplyToRegionSupply_Ag     , &
      ...
      DO indxElem=1,NElements
        ElemDemand = 0.0
        IF (pFlags%lNonPondedAg_Defined) ElemDemand_Ag(indxElem) = SUM(pCrops%Demand(:,indxElem)) 
        IF (pFlags%lPondedAg_Defined)    ElemDemand_Ag(indxElem) = ElemDemand_Ag(indxElem) + SUM(pPondedCrops%Demand(:,indxElem))
        ElemDemand = ElemDemand_Ag(indxElem)
        IF (pFlags%lUrban_Defined)       ElemDemand              = ElemDemand + pUrban%Demand(indxElem,1)
        ...
        IF (ElemDemand .EQ. 0.0) THEN
          ...
          IF (pFlags%lNonPondedAg_Defined) ElemDemandFrac(1:iNCrops)                       = pCrops%Area(:,indxElem)
          IF (pFlags%lPondedAg_Defined)    ElemDemandFrac(iNCrops+1:iNCrops+iNPondedCrops) = pPondedCrops%Area(:,indxElem)
          IF (pFlags%lUrban_Defined)       ElemDemandFrac(iNCrops+iNPondedCrops+1)         = pUrban%Area(indxElem,1)
        ELSE
          !Use demand fractions if total demand is greater than zero
          IF (pFlags%lNonPondedAg_Defined) ElemDemandFrac(1:iNCrops)                       = pCrops%Demand(:,indxElem)
          IF (pFlags%lPondedAg_Defined)    ElemDemandFrac(iNCrops+1:iNCrops+iNPondedCrops) = pPondedCrops%Demand(:,indxElem)
          IF (pFlags%lUrban_Defined)       ElemDemandFrac(iNCrops+iNPondedCrops+1)         = pUrban%Demand(indxElem,1)
        END IF
        CALL NormalizeArray(ElemDemandFrac)
        ElemDemandFrac_Ag = SUM(ElemDemandFrac(1:iNCrops+iNPondedCrops)) 
        IF (pFlags%lNonPondedAg_Defined) pCrops%ElemDemandFrac(:,indxElem)       = ElemDemandFrac(1:iNCrops)
        IF (pFlags%lPondedAg_Defined)    pPondedCrops%ElemDemandFrac(:,indxElem) = ElemDemandFrac(iNCrops+1:iNCrops+iNPondedCrops) 
        IF (pFlags%lUrban_Defined)       pUrban%ElemDemandFrac(indxElem,1)       = ElemDemandFrac(iNCrops+iNPondedCrops+1)
        IF (ElemDemandFrac_Ag .EQ. 0.0) THEN
            ...
            END IF 
        ELSE
            IF (pFlags%lNonPondedAg_Defined) pCrops%ElemDemandFrac_Ag(:,indxElem)       = ElemDemandFrac(1:iNCrops) / ElemDemandFrac_Ag
            IF (pFlags%lPondedAg_Defined)    pPondedCrops%ElemDemandFrac_Ag(:,indxElem) = ElemDemandFrac(iNCrops+1:iNCrops+iNPondedCrops) / ElemDemandFrac_Ag
        END IF
      END DO
      
      !Compute fractions to distribute regional supply to elements
      IF (pFlags%lNonPondedAg_Defined  .OR. pFlags%lPondedAg_Defined) THEN
        RegionalDemand_Ag  = RegionalDemand(AppGrid,RootZone,f_iLandUse_Ag)
      ELSE
        RegionalDemand_Ag = 0.0
      END IF
      IF (pFlags%lUrban_Defined) THEN
        RegionalDemand_Urb = RegionalDemand(AppGrid,RootZone,f_iLandUse_Urb)
      ELSE
  ...
  SUBROUTINE RedistributeMoist(NElements,DeltaT,RootZone)
    ...
    INTEGER                                        :: indxElem,indxLU,indxUrban_In,indxUrban_Out,indxNV, &
                                                      indxRV,iNCrops,NLandsExt,iNPondedCrops
    ...
    lNVRV_Defined        = RootZone%Flags%lNVRV_Defined
    iNCrops              = RootZone%NonPondedAgRootZone%NCrops
    iNPondedCrops     = 0  ; IF (lPondedAg_Defined) iNPondedCrops = f_iNPondedCrops
    NLandsExt            = RootZone%NLands + 1
    indxUrban_In         = iNCrops + iNPondedCrops + 1
    indxUrban_Out        = iNCrops + iNPondedCrops + 2
    indxNV               = iNCrops + iNPondedCrops + 3
    indxRV               = iNCrops + iNPondedCrops + 4
    Area                 = 0.0
    Area_P               = 0.0
    SM_Precip            = 0.0
    SM_AW                = 0.0
    ...
    AreaExpand           = 0.0
    AreaReduced          = 0.0
    RootDepth            = 0.0   ;   IF (lNonPondedAg_Defined) RootDepth(1:iNCrops) = RootZone%NonPondedAgRootZone%RootDepth
                                     IF (lPondedAg_Defined) RootDepth(iNCrops+1:iNCrops+iNPondedCrops) = RootZone%PondedAgRootZone%RootDepth
                                     IF (lUrban_Defined) THEN
    ...
    ASSOCIATE (pNonPondedCrops => RootZone%NonPondedAgRootZone%Crops  , &
      ...
      DO indxElem=1,NElements
          ...
          IF (lNonPondedAg_Defined) THEN
            Area(1:iNCrops)      = pNonPondedCrops%Area(:,indxElem)
            Area_P(1:iNCrops)    = pNonPondedCrops%Area_P(:,indxElem)
            SM_Precip(1:iNCrops) = pNonPondedCrops%SoilM_Precip_P(:,indxElem)
            SM_AW(1:iNCrops)     = pNonPondedCrops%SoilM_AW_P(:,indxElem)
            SM_Oth(1:iNCrops)    = pNonPondedCrops%SoilM_Oth_P(:,indxElem)
          END IF 
          IF (lPondedAg_Defined) THEN
            Area(iNCrops+1:iNCrops+iNPondedCrops)      = pPondedCrops%Area(:,indxElem)
            Area_P(iNCrops+1:iNCrops+iNPondedCrops)    = pPondedCrops%Area_P(:,indxElem)
            SM_Precip(iNCrops+1:iNCrops+iNPondedCrops) = pPondedCrops%SoilM_Precip_P(:,indxElem)
            SM_AW(iNCrops+1:iNCrops+iNPondedCrops)     = pPondedCrops%SoilM_AW_P(:,indxElem)
            SM_Oth(iNCrops+1:iNCrops+iNPondedCrops)    = pPondedCrops%SoilM_Oth_P(:,indxElem)
          END IF 
          IF (lUrban_Defined) THEN
          ...
          END IF
          IF (lNVRV_Defined) THEN
            Area(indxNV)      = pNV%Area(indxElem,1)
            Area(indxRV)      = pRV%Area(indxElem,1)
            Area_P(indxNV)    = pNV%Area_P(indxElem,1)
            Area_P(indxRV)    = pRV%Area_P(indxElem,1)
            SM_Precip(indxNV) = pNV%SoilM_Precip_P(indxElem,1)
            SM_Precip(indxRV) = pRV%SoilM_Precip_P(indxElem,1)
            SM_AW(indxNV)     = pNV%SoilM_AW_P(indxElem,1)        !Although there is no irrigtaion for native and riparian veg, they
            SM_AW(indxRV)     = pRV%SoilM_AW_P(indxElem,1)        !  can inherit moisture due to irrigtaion when their area expands into ag and urban lands
            SM_Oth(indxNV)    = pNV%SoilM_Oth_P(indxElem,1)
            SM_Oth(indxRV)    = pRV%SoilM_Oth_P(indxElem,1)
          END IF 
      ...
      IF (lNonPondedAg_Defined) THEN
        pNonPondedCrops%SoilM_Precip = SoilM_Precip(1:iNCrops,:) 
        pNonPondedCrops%SoilM_AW     = SoilM_AW(1:iNCrops,:) 
        pNonPondedCrops%SoilM_Oth    = SoilM_Oth(1:iNCrops,:) 
        pNonPondedCrops%SoilMCh      = SoilMCh_Precip(1:iNCrops,:) + SoilMCh_AW(1:iNCrops,:) + SoilMCh_Oth(1:iNCrops,:)
        pNonPondedCrops%PercCh       = PercCh(1:iNCrops,:)
      END IF
      IF (lPondedAg_Defined) THEN
        pPondedCrops%SoilM_Precip = SoilM_Precip(iNCrops+1:iNCrops+iNPondedCrops,:) 
        pPondedCrops%SoilM_AW     = SoilM_AW(iNCrops+1:iNCrops+iNPondedCrops,:)
        pPondedCrops%SoilM_Oth    = SoilM_Oth(iNCrops+1:iNCrops+iNPondedCrops,:)
        pPondedCrops%SoilMCh      = SoilmCh_Precip(iNCrops+1:iNCrops+iNPondedCrops,:) + SoilMCh_AW(iNCrops+1:iNCrops+iNPondedCrops,:) + SoilMCh_Oth(iNCrops+1:iNCrops+iNPondedCrops,:)
        pPondedCrops%PercCh       = PercCh(iNCrops+1:iNCrops+iNPondedCrops,:)
      END IF
      IF (lUrban_Defined) THEN
      ...
      END IF
      IF (lNVRV_Defined) THEN
        pNV%SoilM_Precip(:,1) = SoilM_Precip(indxNV,:)
        pNV%SoilM_AW(:,1)     = SoilM_AW(indxNV,:)
        pNV%SoilM_Oth(:,1)    = SoilM_Oth(indxNV,:)
        pNV%SoilMCh(:,1)      = SoilmCh_Precip(indxNV,:) + SoilmCh_AW(indxNV,:) + SoilMCh_Oth(indxNV,:)
        pNV%PercCh(:,1)       = PercCh(indxNV,:)
        pRV%SoilM_Precip(:,1) = SoilM_Precip(indxRV,:)
        pRV%SoilM_AW(:,1)     = SoilM_AW(indxRV,:)
        pRV%SoilM_Oth(:,1)    = SoilM_Oth(indxRV,:)
        pRV%SoilMCh(:,1)      = SoilmCh_Precip(indxRV,:) + SoilMCh_AW(indxRV,:) + SoilMCh_Oth(indxRV,:)
        pRV%PercCh(:,1)       = PercCh(indxRV,:)
      END IF
  ...
  SUBROUTINE AdvanceStateLocal(RootZone,lAdvanceArea)
    ...
    IF (RootZone%Flags%lNVRV_Defined) THEN
        ...
        IF (lAdvanceArea) THEN
            RootZone%NVRVRootZone%NativeVeg%Area_P       = RootZone%NVRVRootZone%NativeVeg%Area
            RootZone%NVRVRootZone%RiparianVeg%Area_P     = RootZone%NVRVRootZone%RiparianVeg%Area
        END IF
    END IF
  ...
  SUBROUTINE RootZone_v40_RegionalReturnFlow_Urb(RootZone,AppGrid,RReturnFlow)
    ...
    !Compute sub-regional return flow
    IF (RootZone%Flags%lUrban_Defined) THEN
        RReturnFlow(1:NRegion) = AppGrid%AccumElemValuesToSubregions(RootZone%UrbanRootZone%UrbData%ReturnFlowIn+RootZone%UrbanRootZone%UrbData%ReturnFlowOut)
        RReturnFlow(NRegion+1) = SUM(RReturnFlow(1:NRegion))
    ELSE
        RReturnFlow = 0.0
    END IF
  ...
  SUBROUTINE ProcessLandUseAreas(AppGrid,TimeStep,RootZone,iStat)
    ...
    NAllCrops     = NNonPondCrops
    IF (RootZone%Flags%lPondedAg_Defined) NAllCrops = NAllCrops + f_iNPondedCrops
    ...
    DO indxElem=1,NElements
      ...
      IF (RootZone%Flags%lNonPondedAg_Defined) LUArea(1:NNonPondCrops,indxElem)           = RootZone%NonPondedAgRootZone%Crops%Area(:,indxElem)
      IF (RootZone%Flags%lPondedAg_Defined)    LUArea(NNonPondCrops+1:NAllCrops,indxElem) = RootZone%PondedAgRootZone%Crops%Area(:,indxElem)
      IF (RootZone%Flags%lUrban_Defined)       LUArea(NAllCrops+1,indxElem)               = RootZone%UrbanRootZone%UrbData%Area(indxElem,1)
      IF (RootZone%Flags%lNVRV_Defined) THEN
          LUArea(NAllCrops+2,indxElem) = RootZone%NVRVRootZone%NativeVeg%Area(indxElem,1)
          LUArea(NAllCrops+3,indxElem) = RootZone%NVRVRootZone%RiparianVeg%Area(indxElem,1)
      END IF
      ...
      IF (ALL(LUArea(:,indxElem) .LE. 0.0)) THEN
          iElemID = AppGrid%AppElement(indxElem)%ID
          IF (ASSOCIATED(RootZone%Logger)) THEN
              CALL RootZone%Logger%SetLastMessage('Total land use area is zero at element ' // TRIM(IntToText(iElemID)) // '!',f_iFatal,ThisProcedure)
          ELSE
              CALL RootZone%Logger%SetLastMessage('Total land use area is zero at element ' // TRIM(IntToText(iElemID)) // '!',f_iFatal,ThisProcedure)
          END IF
          iStat = -1
  ...
  SUBROUTINE CheckTSDataPointers(RootZone,iElemIDs,Precip,ET,iStat)
    ...
    ASSOCIATE (pElemPrecipData => RootZone%ElemPrecipData      , &
        ...
        END IF
          ...
          !Native 
          IF (lNVRV_Defined) THEN
              IF (iETColMax .LT. pNVRV%NativeVeg%iColETc(indxElem,1)) THEN
        ...
        END IF
          ...
          !Riparian 
              IF (iETColMax .LT. pNVRV%RiparianVeg%iColETc(indxElem,1)) THEN
