...
!***********************************************************************
MODULE Class_PondedAgLandUse_v40
  !$ USE OMP_LIB
...
! ******************************************************************
  ...
  ! -------------------------------------------------------------
  PRIVATE
  PUBLIC :: PondedAgv40_SetModuleLogger , &
            PondedAgLandUse_v40_Type  ,  &
            f_iNPondedCrops
  ...
  ! -------------------------------------------------------------
  TYPE,EXTENDS(GenericLandUseType) :: PondedAg_v40_Type
  ...
  CHARACTER(LEN=ModNameLen),PARAMETER :: ModName    = 'Class_PondedAgLandUse_v40::'
...
! ******************************************************************
  ...
  ! -------------------------------------------------------------
  SUBROUTINE New(PondLand,Logger,IsForInquiry,cProjectNameForDSS,cFileName,cWorkingDirectory,FactCN,AppGrid,iElemIDs,TimeStep,NTimeSteps,cVersion,iStat)
    ...
    IF (ErrorCode+iStat .NE. 0) THEN
            CALL ModuleLogger%SetLastMessage('Error in allocating memory for ponded agricultural data!',f_iFatal,ThisProcedure)
        iStat = -1
        RETURN
    END IF
    ...
    IF (NBudgetCrops .GT. 0) THEN
      ...
      IF (ErrorCode .NE. 0) THEN
              CALL ModuleLogger%SetLastMessage('Error in allocating memory for ponded crops budget output data!',f_iFatal,ThisProcedure)
          iStat = -1
          RETURN
      END IF
      ...
      DO indxCrop=1,NBudgetCrops
        CALL RiceRefugeDataFile%ReadData(ALine,iStat)  ;  IF (iStat .EQ. -1) RETURN  ;  cBudgetCropCode = ADJUSTL(StripTextUntilCharacter(ALine,f_cInlineCommentChar))  ;  CALL CleanSpecialCharacters(cBudgetCropCode)
        lCropFound = .FALSE.
        ...
        IF (.NOT. lCropFound) THEN
                CALL ModuleLogger%SetLastMessage(TRIM(cBudgetCropCode)//' for water budget output is not defined as a ponded crop!',f_iFatal,ThisProcedure)
            iStat = -1
            RETURN
        END IF
      END DO
    ...
    CALL RiceRefugeDataFile%ReadData(ALine,iStat)  ;  IF (iStat .EQ. -1) RETURN  ;  ALine = StripTextUntilCharacter(ALine,f_cInlineCommentChar)  ;  CALL CleanSpecialCharacters(ALine)
    IF (NBudgetCrops .GT. 0) THEN
      IF (ALine .NE. '') THEN
          CALL EstablishAbsolutePathFileName(TRIM(ADJUSTL(ALine)),cWorkingDirectory,cAbsPathFileName)
    ...
    DO indxElem=1,NElements
        ...
        IF (lProcessed(iElem)) THEN
            ID = iElemIDs(iElem)
                CALL ModuleLogger%SetLastMessage('Curve numbers for ponded crops at element '//TRIM(IntToText(ID))//' are defined more than once!',f_iFatal,ThisProcedure)
            iStat = -1
            RETURN
        END IF
    ...
    DO indxElem=1,NElements
        ...
        IF (lProcessed(iElem)) THEN
            ID = iElemIDs(iElem)
                CALL ModuleLogger%SetLastMessage('Evapotranspiration column pointers for ponded crops at element '//TRIM(IntToText(ID))//' are defined more than once!',f_iFatal,ThisProcedure)
            iStat = -1
            RETURN
        END IF
        lProcessed(iElem)               = .TRUE.
    ...
    DO indxElem=1,NElements
        ...
        IF (lProcessed(iElem)) THEN
            ID = iElemIDs(iElem)
                CALL ModuleLogger%SetLastMessage('Water supply requirement column pointers for ponded crops at element '//TRIM(IntToText(ID))//' are defined more than once!',f_iFatal,ThisProcedure)
            iStat = -1
            RETURN
        END IF
        lProcessed(iElem)              = .TRUE.
    ...
    DO indxElem=1,NElements
        ...
        IF (lProcessed(iElem)) THEN
            ID = iElemIDs(iElem)
                CALL ModuleLogger%SetLastMessage('Irrigation period column pointers for ponded crops at element '//TRIM(IntToText(ID))//' are defined more than once!',f_iFatal,ThisProcedure)
            iStat = -1
            RETURN
        END IF
        lProcessed(iElem)                      = .TRUE.
    ...
    DO indxElem=1,NElements
        ...
        IF (lProcessed(iElem)) THEN
            ID = iElemIDs(iElem)
                CALL ModuleLogger%SetLastMessage('Ponding depth column pointers for ponded crops at element '//TRIM(IntToText(ID))//' are defined more than once!',f_iFatal,ThisProcedure)
            iStat = -1
            RETURN
        END IF
        lProcessed(iElem)                                           = .TRUE.
    ...
    DO indxElem=1,NElements
        ...
        IF (lProcessed(iElem)) THEN
            ID = iElemIDs(iElem)
                CALL ModuleLogger%SetLastMessage('Non-flooded rice decomposition water column pointers for ponded crops at element '//TRIM(IntToText(ID))//' are defined more than once!',f_iFatal,ThisProcedure)
            iStat = -1
            RETURN
        END IF
        lProcessed(iElem)                        = .TRUE.
    ...
    DO indxElem=1,NElements
        ...
        IF (lProcessed(iElem)) THEN
            ID = iElemIDs(iElem)
                CALL ModuleLogger%SetLastMessage('Return flow depth column pointers for ponded crops at element '//TRIM(IntToText(ID))//' are defined more than once!',f_iFatal,ThisProcedure)
            iStat = -1
            RETURN
        END IF
        lProcessed(iElem)                  = .TRUE.
    ...
    DO indxElem=1,NElements
        ...
        IF (lProcessed(iElem)) THEN
            ID = iElemIDs(iElem)
                CALL ModuleLogger%SetLastMessage('Re-use flow depth column pointers for ponded crops at element '//TRIM(IntToText(ID))//' are defined more than once!',f_iFatal,ThisProcedure)
            iStat = -1
            RETURN
        END IF
        lProcessed(iElem)                 = .TRUE.
    ...
    DO indxElem=1,NElements
        ...
        IF (lProcessed(iElem)) THEN
            ID = iElemIDs(iElem)
                CALL ModuleLogger%SetLastMessage('Initial conditions for ponded crops at element '//TRIM(IntToText(ID))//' are defined more than once!',f_iFatal,ThisProcedure)
            iStat = -1
            RETURN
        END IF
        lProcessed(iElem)                    = .TRUE.
...
! ******************************************************************
  ...
  SUBROUTINE ReadTotalElemArea(PondLand,iElem,lForInquiry,cReadBeginDateAndTime,cReadEndDateAndTime,nActualOutput,ElemAgLandUse,rOutputDates,iStat)
    ...
    DO indxCrop=1,PondLand%NCrops
        ...
        CALL PondLand%LandUseDataFile%ReadTSData(iElem,iColumn,iPathNameIndex,cReadBeginDateAndTime,cReadEndDateAndTime,nActualOutput,rData,rOutputDates,ErrorCode,iStat)  ;  IF (iStat .EQ. -1) RETURN
...
! ******************************************************************
  ...
  SUBROUTINE Simulate(PondedAg,AppGrid,ETData,DeltaT,Precip,GenericMoisture,SoilsData,HydCondPonded,ElemSupply,ElemsToGW,SolverData,lLakeElem,iStat)
    ...
    REAL(8)                     :: AchievedConv,Area,ETc(f_iNPondedCrops),HydCond,TotalPorosity,       &
                                   ...
                                   SoilM_Array(3),Infilt(3),ETPartition(3),rRunoff,rReturnFlow,        &
                                   rPrecipInfilt,rIrigInfilt,rDrain,rETa,rETP,rETOth,rPerc,rReuse,     &
                                   rGMExcess,rSoilM_Precip_P,rSoilM_AW_P,rSoilM_Oth_P,rETAW
            ...
            DO indxCrop=1,f_iNPondedCrops
                ...
                IF (lReadAgWaterDemand) THEN
                    IF (PondedAg%iColAgDemand(indxCrop,indxElem) .GT. 0) THEN
                        pCrops%DemandRaw(indxCrop,indxElem) = SpecifiedDemand(PondedAg%iColAgDemand(indxCrop,indxElem))
                        pCrops%Demand(indxCrop,indxElem)    = pCrops%DemandRaw(indxCrop,indxElem)
                        CYCLE
                    END IF
                END IF
                ...
                IF (IrigPeriod(pCrops%iColIrigPeriod(indxCrop,indxElem)) .EQ. f_iNoIrigPeriod) CYCLE
        
                !Initialize
                RootDepth         = PondedAg%RootDepth(indxCrop)
                PondDepth         = PondedAg%PondDepthFile%rValues(pCrops%iColPondDepth(indxCrop,indxElem))
                ReturnFlow        = PondedAg%OperationFlowsFile%rValues(pCrops%iColReturn(indxCrop,indxElem)) * DeltaT
                Reuse             = PondedAg%OperationFlowsFile%rValues(pCrops%iColReuse(indxCrop,indxElem)) * DeltaT
                TotalPorosityCrop = TotalPorosity * RootDepth
                FieldCapacityCrop = FieldCapacity * RootDepth
                GM                = GMElem * RootDepth
                SoilM             = pCrops%SoilM_Precip_P(indxCrop,indxElem) + pCrops%SoilM_AW_P(indxCrop,indxElem) + pCrops%SoilM_Oth_P(indxCrop,indxElem)
                ...
                CALL PondedCropDemand(PrecipD                                    ,  &
                                      ...
                                      FieldCapacityCrop                          ,  & 
                                      SoilM                                      ,  & 
                                      pCrops%DemandRaw(indxCrop,indxElem)        ,  &
                                      pCrops%Demand(indxCrop,indxElem)           ) 
        
                !Convert demand related data to volumetric rate and store in persistent arrays
                pCrops%DemandRaw(indxCrop,indxElem) = pCrops%DemandRaw(indxCrop,indxElem) / DeltaT * Area
                pCrops%Demand(indxCrop,indxElem)    = pCrops%Demand(indxCrop,indxElem)    / DeltaT * Area
            END DO
        END DO
        !$OMP END DO
        !$OMP END PARALLEL
