...
CONTAINS
  ...
  SUBROUTINE PRINT_SCREEN(PNAME_IN,Version)
    ...
    IF (ASSOCIATED(ModuleLogger)) THEN
        CALL ModuleLogger%LogMessage(f_L,f_iMessage,'',iDestination=f_iSCREEN,cFmt='(8X,A)')
    ELSE
        CALL DefaultLogger%LogMessage(f_L,f_iMessage,'',iDestination=f_iSCREEN,cFmt='(8X,A)')
    END IF
