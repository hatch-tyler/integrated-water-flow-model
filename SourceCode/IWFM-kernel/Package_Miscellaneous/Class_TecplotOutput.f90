...
! ******************************************************************
  ...
  ! --- PRINT-OUT RESULTS
  ! -------------------------------------------------------------
  SUBROUTINE PrintResults(TPOutput,rValues,rFactor,cFormat,TimeStep)
    CLASS(TecplotOutputType)      :: TPOutput
    ...
    DO indx=1,SIZE(rValues)/100+1 
        WRITE (Text,cFormat) (rValuesTemp(indx1)*rFactor,indx1=(indx-1)*100+1,MIN(indx*100,SIZE(rValues)))
        CALL TPOutput%OutFile%WriteData(Text)
    END DO
