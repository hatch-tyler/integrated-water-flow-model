...
! ******************************************************************
  ...
  SUBROUTINE NodeArea_ForArrayList(Grid,NodeList,iNodeIDs,Area,iStat)
    ...
    !Local variables
    CHARACTER(LEN=f_iModNameLen+21) :: ThisProcedure = f_cModName // 'NodeArea_ForArrayList'
  ...
  SUBROUTINE NodeArea_WithBeginEndIndex(Grid,iNodeIDs,iBeginIndex,iEndIndex,Area,iStat)
    ...
    !Local variables
    CHARACTER(LEN=f_iModNameLen+26) :: ThisProcedure = f_cModName // 'NodeArea_WithBeginEndIndex'
    INTEGER                         :: indx,indxElem,indxVertex,indxList,NodeNumber,ID
    REAL(8)                         :: DummyReal(1)
...
! ******************************************************************
  ...
  SUBROUTINE ElementArea_WithBeginEndIndex(Grid,iElemIDs,iBeginIndex,iEndIndex,Area,iStat)
    ...
    !Local variables
    CHARACTER(LEN=f_iModNameLen+29) :: ThisProcedure = f_cModName // 'ElementArea_WithBeginEndIndex'
    ...
    DO ElemNo=iBeginIndex,iEndIndex
      ...
      ELSE
        Area(ElemNo) = 0.0
        DO indxNode=1,4
          Area(ElemNo) = Area(ElemNo)+QUAD_INTGRL(indxNode,0,XP,YP,f_iNPGauss,QUAD_FUNC_AREA)
        END DO
      END IF
  ...
  SUBROUTINE ElementArea_ForArrayList(Grid,ElemList,iElemIDs,Area,iStat)
    ...
    !Local variables
    CHARACTER(LEN=f_iModNameLen+24) :: ThisProcedure = f_cModName // 'ElementArea_ForArrayList'
    ...
    DO indx=1,SIZE(ElemList)
      ...
      ELSE
        Area(indx) = 0.0
        DO indxNode=1,4
          Area(indx) = Area(indx) + QUAD_INTGRL(indxNode,0,XP,YP,f_iNPGauss,QUAD_FUNC_AREA)
        END DO
      END IF
  ...
  SUBROUTINE ElementVertexArea(Grid,indxElem,VertexList,Area,iStat)
    ...
    !Local variables
    CHARACTER(LEN=f_iModNameLen+17) :: ThisProcedure = f_cModName // 'ElementVertexArea'
    ...
    ELSE
      DO indxVertex=1,SIZE(VertexList)
        Area(indxVertex) = QUAD_INTGRL(VertexList(indxVertex),0,XP,YP,f_iNPGauss,QUAD_FUNC_AREA)
  ...
  SUBROUTINE ElementVertexAreaFraction(Grid,indxElem,iElemIDs,VertexList,Fract,iStat)
    ...
    !Local variables
    CHARACTER(LEN=f_iModNameLen+25),PARAMETER :: ThisProcedure = f_cModName // 'ElementVertexAreaFraction'
    REAL(8)                                   :: Area(1)
  ...
  SUBROUTINE Compute_DELShpI_DELShpJ(Grid,ElemNo,iElemIDs,Integral,ElemArea,iStat)
    ...
    !Local variables
    CHARACTER(LEN=f_iModNameLen+23) :: ThisProcedure = f_cModName // 'Compute_DELShpI_DELShpJ'
    ...
    ELSE
      DO I=1,4
        DO J=I+1,4
          indx = indx+1
          Integral(indx) = QUAD_INTGRL(I,J,XP,YP,f_iNPGauss,QUAD_FUNC_DIFF)
        END DO
      END DO
  ...
  SUBROUTINE Compute_Rot_DELShpI_DELShpJ(Grid,iElemIndex,iElemIDs,Integral,ElemArea,iStat)
    ...
    !Local variables
    CHARACTER(LEN=f_iModNameLen+27) :: ThisProcedure = f_cModName // 'Compute_Rot_DELShpI_DELShpJ'
    INTEGER                         :: NVertex,NIntegral,indx,I,J,Vertex(4)
    REAL(8)                         :: LocalElemArea,XP(4),YP(4),Area(1)
    ...
    ELSE
      DO I=1,4
        DO J=I+1,4
          indx = indx+1
          Integral(indx) = QUAD_INTGRL(I,J,XP(1:NVertex),YP(1:NVertex),f_iNPGauss,QUAD_FUNC_ROT)
        END DO
      END DO
...
! ******************************************************************
  ...
  SUBROUTINE TRI_INTGRL(I,J,XP,YP,AREA,Integral,iStat)
    ...
    !Local variables
    CHARACTER(LEN=f_iModNameLen+10) :: ThisProcedure = f_cModName // 'TRI_INTGRL'
    INTEGER                         :: K
  ...
  SUBROUTINE TRI_ROT(I,J,XP,YP,AREA,Rot,iStat)
    ...
    !Local variables
    CHARACTER(LEN=f_iModNameLen+7) :: ThisProcedure = f_cModName // 'TRI_ROT'
    INTEGER                        :: K
 ...
 ! -------------------------------------------------------------
  ...
  REAL(8) FUNCTION QUAD_SHAPE(XI,ETA,NSHP)
    ...
    IF (NSHP.EQ.1) THEN
      QUAD_SHAPE=0.25d0*(XI-1.0)*(ETA-1d0)
    ELSEIF (NSHP.EQ.2) THEN
      QUAD_SHAPE=0.25d0*(XI+1.0)*(1d0-ETA)
    ELSEIF (NSHP.EQ.3) THEN
      QUAD_SHAPE=0.25d0*(XI+1.0)*(ETA+1d0)
    ELSEIF (NSHP.EQ.4) THEN
      QUAD_SHAPE=0.25d0*(1.0-XI)*(ETA+1d0)
    END IF
  ...
  REAL(8) FUNCTION DXI_QUAD_SHAPE(ETA,NSHP)
    ...
    IF (NSHP.EQ.1) THEN
      DXI_QUAD_SHAPE=0.25d0*(ETA-1d0)
    ELSEIF (NSHP.EQ.2) THEN
      DXI_QUAD_SHAPE=0.25d0*(1d0-ETA)
    ELSEIF (NSHP.EQ.3) THEN
      DXI_QUAD_SHAPE=0.25d0*(ETA+1d0)
    ELSEIF (NSHP.EQ.4) THEN
      DXI_QUAD_SHAPE=-0.25d0*(ETA+1d0)
  ...
  REAL(8) FUNCTION DETA_QUAD_SHAPE(XI,NSHP)
    ...
    IF (NSHP.EQ.1) THEN
      DETA_QUAD_SHAPE=0.25d0*(XI-1d0)
    ELSEIF (NSHP.EQ.2) THEN
      DETA_QUAD_SHAPE=-0.25d0*(XI+1d0)
    ELSEIF (NSHP.EQ.3) THEN
      DETA_QUAD_SHAPE=0.25d0*(XI+1d0)
    ELSEIF (NSHP.EQ.4) THEN
      DETA_QUAD_SHAPE=0.25d0*(1d0-XI)
    END IF
  ...
  REAL(8) FUNCTION DX_QUAD_SHAPE(XI,ETA,XP,YP,NSHP)
    ...
    DX_QUAD_SHAPE = 1d0/DET_JACOB(XI,ETA,XP,YP) *             &
  ...
  REAL(8) FUNCTION DY_QUAD_SHAPE(XI,ETA,XP,YP,NSHP)
    ...
    DY_QUAD_SHAPE = 1d0/DET_JACOB(XI,ETA,XP,YP) *              &
