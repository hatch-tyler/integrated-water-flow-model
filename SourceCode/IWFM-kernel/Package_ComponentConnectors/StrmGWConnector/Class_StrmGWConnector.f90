...
! ******************************************************************
  ...
  ! --- *** Note: + flow means loosing stream
  ! -------------------------------------------------------------
  SUBROUTINE Simulate_JFNK(Connector,iNNodes,rGWHeads,rStrmHeads,rAvailableFlows,lUpdateStrmEqns,Matrix,WetPerimeterFunction,rMaxElevs)
    CLASS(StrmGWConnectorType)                      :: Connector
    INTEGER,INTENT(IN)                              :: iNNodes
    REAL(8),INTENT(IN)                              :: rGWHeads(:),rStrmHeads(:),rAvailableFlows(:)
    LOGICAL,INTENT(IN)                              :: lUpdateStrmEqns
    ...
    IF (Connector%lDefined) THEN
        CALL Connector%Me%Simulate_JFNK(iNNodes,rGWHeads,rStrmHeads,rAvailableFlows,lUpdateStrmEqns,Matrix,WetPerimeterFunction,rMaxElevs)
    END IF
    
  END SUBROUTINE Simulate_JFNK
