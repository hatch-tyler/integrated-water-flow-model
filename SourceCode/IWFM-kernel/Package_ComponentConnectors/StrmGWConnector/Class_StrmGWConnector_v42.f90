...
! ******************************************************************
  ...
  CONTAINS
      ...
      PROCEDURE,PASS :: Simulate_JFNK                            => StrmGWConnector_v42_Simulate_JFNK
      ...
      PROCEDURE,PASS :: ComputeStrmGWFlow_GivenStrmFlow          => StrmGWConnector_v42_ComputeStrmGWFlow_GivenStrmFlow
...
! ******************************************************************
  ...
  SUBROUTINE StrmGWConnector_v42_CompileConductance(Connector,InFile,AppGrid,Stratigraphy,NStrmNodes,iStrmNodeIDs,BottomElevs,rLength,iStat,rWetPerimeter)
    ...
    Connector%TimeUnitConductance = ADJUSTL(StripTextUntilCharacter(ALine,f_cInlineCommentChar))
    ...
    DO indxNode=1,NStrmNodes
        ...
        IF (nGWNodes .GT. 1) THEN
            CALL InFile%ReadData(ALine,iStat)  ;  IF (iStat .EQ. -1) RETURN
            CALL CleanSpecialCharacters(ALine)
            ALine = TRIM(StripTextUntilCharacter(ALine,f_cInlineCommentChar))
        ...
        IF (iGWNode .EQ. 0) THEN
            CALL Connector%Logger%SetLastMessage('Groundwater node '//TRIM(IntToText(iGWNodeID))//' listed for stream node '//TRIM(IntToText(iStrmNodeID))//' for stream bed parameters is not in the model!',f_iFatal,ThisProcedure)
            iStat = -1
            RETURN
        END IF
        ...
        DO indx=2,Connector%GWNodeList(iNode)%nGWNodes
            ...
            IF (lWPForEachGWNode) THEN
                ...
                IF (iStat .NE. 0) THEN
                    ...
                    CALL Connector%Logger%SetLastMessage(MessageArray(1:3),f_iFatal,ThisProcedure)
                    iStat = -1
                    RETURN
                END IF
  ...
  ! -------------------------------------------------------------
  SUBROUTINE StrmGWConnector_v42_Simulate(Connector,iNNodes,rGWHeads,rStrmHeads,rAvailableFlows,lUpdateStrmEqns,Matrix,WetPerimeterFunction,rMaxElevs)
    CLASS(StrmGWConnector_v42_Type)                 :: Connector
  ...
  ! -------------------------------------------------------------
  ! --- SIMULATE STREAM-GW INTERACTION FOR JFNK METHOD (UPDATE RHS VECTOR ONLY)
  ! --- *** Note: + flow means losing stream
  ! -------------------------------------------------------------
  SUBROUTINE StrmGWConnector_v42_Simulate_JFNK(Connector,iNNodes,rGWHeads,rStrmHeads,rAvailableFlows,lUpdateStrmEqns,Matrix,WetPerimeterFunction,rMaxElevs)
    CLASS(StrmGWConnector_v42_Type)                 :: Connector
    INTEGER,INTENT(IN)                              :: iNNodes
    REAL(8),INTENT(IN)                              :: rGWHeads(:),rStrmHeads(:),rAvailableFlows(:)
    LOGICAL,INTENT(IN)                              :: lUpdateStrmEqns
    ...
    !Local variables
    INTEGER           :: iGWNode,iNodes_RHS(500),indxGW,indx,nGWNodes,iOffset,indxStrm
    REAL(8)           :: Conductance,rUpdateRHS(500),rGWHead,rStrmGWFlow,rStrmHead,rNodeAvailableFlow
    INTEGER,PARAMETER :: iCompIDs_RHS(500)   = [f_iStrmComp , (f_iGWComp,indx=2,500)] 
  ...
  END SUBROUTINE StrmGWConnector_v42_Simulate_JFNK
  ...
  ! -------------------------------------------------------------
  FUNCTION StrmGWConnector_v42_ComputeStrmGWFlow_GivenStrmFlow(Connector,iStrmNode,rStrmFlow,rStrmHead,rGWHeads,WetPerimeterFunction,rMaxElev) RESULT(rStrmGWFlow)
    CLASS(StrmGWConnector_v42_Type),INTENT(IN)      :: Connector
    INTEGER,INTENT(IN)                              :: iStrmNode
    ...
    !Local variables
    INTEGER :: nGWNodes,indxGW,iOffset
    REAL(8) :: rConductance,rStrmGWFlowWork,rGWHead,rNodeFlow
    
    !Initialize
    rStrmGWFlow = 0.0
    iOffset     = SUM(Connector%GWNodeList(1:iStrmNode-1)%nGWNodes)
    ...
    ASSOCIATE (pData => Connector%GWNodeList(iStrmNode))
        ...
        DO indxGW=1,nGWNodes
            ...
            rConductance = pData%Conductance(indxGW)
            IF (rConductance .EQ. 0.0) CYCLE

            !GW head 
            rGWHead= rGWHeads(iOffset+indxGW)
            
            !Stream flow associated with gw node
            rNodeFlow = rStrmFlow * pData%rDistFrac(indxGW)
            
            !Stream-gw interaction
            rStrmGWFlowWork = rConductance * (rStrmHead - MAX(rGWHead,pData%rDisconnectElev(indxGW)))
            rStrmGWFlow     = rStrmGWFlow + MIN(rNodeFlow , rStrmGWFlowWork)
        END DO
    END ASSOCIATE

  END FUNCTION StrmGWConnector_v42_ComputeStrmGWFlow_GivenStrmFlow
