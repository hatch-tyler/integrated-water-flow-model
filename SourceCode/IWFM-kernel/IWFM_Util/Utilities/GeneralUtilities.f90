...
MODULE GeneralUtilities
  ...
  ! Inline comment character for IWFM input files
  ! Restored to '/' with smart detection in StripTextUntilCharacter
...
! ******************************************************************
  ...
  SUBROUTINE ReplaceString(InString,StringToReplace,StringToReplaceWith,iStat)
    ...
    IF (LEN(StringToReplace) .NE. LEN(StringToReplaceWith)) THEN
        CALL DefaultLogger%SetLastMessage('String to be replaced with must have the same length as the replacing string!',f_iFatal,ThisProcedure)
        iStat = -1
        RETURN
    END IF
  ...
  ! -------------------------------------------------------------
  ! --- CHECK IF A STRING STARTING WITH '/' IS A DSS PATHNAME
  ! --- DSS pathnames have format /A/B/C/D/E/F/ (7 slashes)
  ! -------------------------------------------------------------
  FUNCTION IsDSSPathname(Text) RESULT(IsDSS)
    CHARACTER(LEN=*),INTENT(IN) :: Text
    LOGICAL                     :: IsDSS

    !Local variables
    INTEGER :: i,iSlashCount
  ...
  END FUNCTION IsDSSPathname


  ! -------------------------------------------------------------
  ! --- FIND THE POSITION OF AN INLINE COMMENT '/' DELIMITER
  ! --- Rules: 1) Must be preceded by whitespace (or position 1)
  ! ---        2) Must NOT be start of a DSS pathname
  ! -------------------------------------------------------------
  FUNCTION FindInlineCommentPosition(TextIn,Back) RESULT(Location)
    CHARACTER(LEN=*),INTENT(IN) :: TextIn
    LOGICAL,INTENT(IN)          :: Back
    INTEGER                     :: Location

    !Local variables
    INTEGER :: i,iStart,iEnd,iStep,iLen

    Location = 0
    iLen = LEN_TRIM(TextIn)
    IF (iLen == 0) RETURN

    !Set scan direction
    IF (Back) THEN
      iStart = iLen ; iEnd = 1 ; iStep = -1
    ELSE
      iStart = 1 ; iEnd = iLen ; iStep = 1
    END IF
    ...
    DO i=iStart,iEnd,iStep
      ...
      IF (IsDSSPathname(TextIn(i:iLen))) CYCLE

      !This is the comment delimiter
      Location = i
      EXIT
    END DO

  END FUNCTION FindInlineCommentPosition
  ...
  FUNCTION StripTextUntilCharacter(TextIn,WhatCharacter,Back) RESULT(TextOut)
    ...
    IF (WhatCharacter == f_cInlineCommentChar) THEN
      !Smart inline comment detection for '/'
      Location=FindInlineCommentPosition(TextIn,LocalBack)
    ELSE
      !Standard single-character search for '.', '_', '(', '!', etc.
  ...
  FUNCTION GetStartLocation(String,ColumnNumber) RESULT(Location)
    ...
    WorkString=StripTextUntilCharacter(WorkString,f_cInlineCommentChar,Back=.TRUE.)
...
! ******************************************************************
  ...
  SUBROUTINE HandleAllocError(iErrorCode,cSendingProcedure,iStat)
    ...
    IF (iErrorCode .NE. 0) THEN
        CALL DefaultLogger%SetLastMessage('Error in allocating memory in '//TRIM(ADJUSTL(cSendingProcedure)),f_iFatal,ThisProcedure)
        iStat = -1
    ELSE
        iStat = 0
    END IF
 ...
 4      a(j) = va 
  ...
  SUBROUTINE GetRealArrayData(SourceArray,Array,cProcess,iStat)
    ...
    IF (iSizeA - iRewound*iSizeSA .GT. 0) THEN
        CALL DefaultLogger%SetLastMessage('Sizes of source array and destination arrays are not matched properly!',f_iFatal,cProcess)
        iStat = -1
        RETURN
    END IF
  ...
  SUBROUTINE GetRealArrayDataFromString(SourceString,rArray,cProcess,iStat)
    ...
    SourceString = ADJUSTL(StripTextUntilCharacter(SourceString,f_cInlineCommentChar,Back=.TRUE.))
    ...
    DO indx=1,SIZE(rArray)
      ...
      IF (iLoc .EQ. 0) THEN
          CALL DefaultLogger%SetLastMessage('Error in data entry for '//TRIM(cProcess)//'!',f_iFatal,ThisProcedure)
          iStat = -1
          RETURN
      END IF
