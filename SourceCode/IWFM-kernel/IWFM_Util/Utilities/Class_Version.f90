...
! ******************************************************************
  ...
  ! -------------------------------------------------------------
  ! --- VERSION DATA TYPE
  ! --- Version format is (for Git style) 
  ! ---   When tag exists        : <tag> (<8 characters of hash>)
  ! ---   When tag does not exist: <branch> (<8 characters of hash>)
  ! -------------------------------------------------------------
  TYPE VersionType
    PRIVATE
  ...
  CONTAINS
    GENERIC,PUBLIC          :: New  => Version_New_FromComponents_GitStyle  , &
                                       Version_New_FromComponents_SVNStyle  , &
                                       Version_New_FromFullString
    PROCEDURE,NOPASS,PUBLIC :: Version_New_FromComponents_GitStyle
    PROCEDURE,NOPASS,PUBLIC :: Version_New_FromComponents_SVNStyle
...
! ******************************************************************
  ...
  ! -------------------------------------------------------------
  ! --- CONSTRUCTOR 1, GIT STYLE
  ! -------------------------------------------------------------
  FUNCTION Version_New_FromComponents_GitStyle(cTag,cBranch,cHash) RESULT(Version)
    CHARACTER(LEN=*),INTENT(IN) :: cTag,cBranch,cHash
    TYPE(VersionType)           :: Version
    
    !Local variables
    INTEGER   :: iLenVersion
    CHARACTER :: cVersion*300
    
    !If tag exists...
    IF (LEN_TRIM(cTag) .GT. 0) THEN
        cVersion = TRIM(cTag) // '-' // cHash(1:8)
    ELSE
    !Otherwise...
        cVersion = TRIM(cBranch) // '-' // cHash(1:8) 
    END IF
    
    !Initialize the version
    iLenVersion         = LEN_TRIM(cVersion)
    Version%iLenVersion = iLenVersion   
    ALLOCATE(CHARACTER(iLenVersion) :: Version%cVersion)
    Version%cVersion    = TRIM(cVersion)
    
  END FUNCTION Version_New_FromComponents_GitStyle
  
  
  ! -------------------------------------------------------------
  ! --- CONSTRUCTOR 1, SVN STYLE
  ! -------------------------------------------------------------
  FUNCTION Version_New_FromComponents_SVNStyle(iLenVersion,cVersion,cRevision) RESULT(Version)
  ...
  END FUNCTION Version_New_FromComponents_SVNStyle
