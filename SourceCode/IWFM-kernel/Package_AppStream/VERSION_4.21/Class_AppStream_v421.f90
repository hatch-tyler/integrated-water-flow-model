...
! ******************************************************************
  ...
  CONTAINS
      ...
      PROCEDURE,PASS :: ComputeRHS                     => AppStream_v421_ComputeRHS
...
! ******************************************************************
  ...
  ! -------------------------------------------------------------
  SUBROUTINE AppStream_v421_SetDynamicComponent(AppStream,IsForInquiry,cFileName,cWorkingDirectory,cPackageVersion,TimeStep,NTIME,iLakeIDs,AppGrid,Stratigraphy,ETData,StrmLakeConnector,StrmGWConnector,iStat)
    ...
    cVersionFull   = '4.21-' // TRIM(cPackageVersion)
  ...
  SUBROUTINE AppStream_v421_SetAllComponents(AppStream,IsForInquiry,cFileName,cSimWorkingDirectory,cPackageVersion,TimeStep,NTIME,iLakeIDs,AppGrid,Stratigraphy,ETData,BinFile,StrmLakeConnector,StrmGWConnector,iStat)
    ...
    CALL AppStream_v421_SetDynamicComponent(AppStream,IsForInquiry,cFileName,cSimWorkingDirectory,cPackageVersion,TimeStep,NTIME,iLakeIDs,AppGrid,Stratigraphy,ETData,StrmLakeConnector,StrmGWConnector,iStat)
    IF (iStat .EQ. -1) RETURN
  ...
  SUBROUTINE AppStream_v421_SetAllComponentsWithoutBinFile(AppStream,IsForInquiry,IsRoutedStreams,cPPFileName,cSimFileName,cSimWorkingDirectory,cPackageVersion,AppGrid,Stratigraphy,ETData,TimeStep,NTIME,iLakeIDs,StrmLakeConnector,StrmGWConnector,iStat)
    ...
    CALL AppStream_v421_SetDynamicComponent(AppStream,IsForInquiry,cSimFileName,cSimWorkingDirectory,cPackageVersion,TimeStep,NTIME,iLakeIDs,AppGrid,Stratigraphy,ETData,StrmLakeConnector,StrmGWConnector,iStat)
    IF (iStat .EQ. -1) RETURN
...
! ******************************************************************
  ...
  SUBROUTINE ReadPreprocessedData(AppStream,BinFile,iStat)
    ...
    DO indxNode=1,AppStream%NStrmNodes
        CALL AppStream%RatingTable_WetPerimeter(indxNode)%New(ModuleLogger,BinFile,iStat)
        IF (iStat .EQ. -1) RETURN
    END DO
  ...
  SUBROUTINE ReadStreamConfigData(DataFile,AppGrid,Stratigraphy,iGWNodeIDs,StrmGWConnector,StrmLakeConnector,AppStream,iStat)
    ...
    INTEGER,ALLOCATABLE          :: iGWNodes_Strm(:),iGWNodes_Work(:)
            ...
            DO indxNode=1,NNodes
                ...
                IF (iNData .GT. 1) THEN
                    ...
                    IF (MOD(iNData,2) .NE. 0) THEN
                        CALL ModuleLogger%SetLastMessage('Check that each groundwater node listed for stream node '//TRIM(IntToText(iStrmNodeID))//' is also given a rank for inundation order.',f_iFatal,ThisProcedure)
                        iStat = -1
                        RETURN
                    END IF
                    ...
                    DO indx=2,nGWNodes
                        ...
                        IF (iGWNodes(indx) .EQ. 0) THEN
                            CALL ModuleLogger%SetLastMessage('Groundwater node '//TRIM(IntToText(iGWNodeID))//' listed in stream reach '//TRIM(IntToText(pReach%ID))//' ('//TRIM(pReach%cName)//') for stream node '//TRIM(IntToText(iStrmNodeID))//' is not in the model!',f_iFatal,ThisProcedure)
                            iStat = -1
                            RETURN
                        END IF
                    ...
                    IF (nGWNodes .GT. 1) THEN
                        ...
                        IF (.NOT. ALL(lGoodNode(1:nGWNodes))) THEN
                            CALL ModuleLogger%SetLastMessage('Groundwater nodes listed for stream node '//TRIM(IntToText(iStrmNodeID))//' are not aligned along element faces!',f_iFatal,ThisProcedure)
                            iStat = -1
                            RETURN
                        END IF
                    END IF
                ...
                CALL StrmGWConnector%AddGWNodesToStrmNode(421,AppStream%NStrmNodes,indxStrmNode,iGWNodes(1:nGWNodes),iLayers(1:nGWNodes),iRanks(1:nGWNodes),iStat)
                IF (iStat .EQ. -1) RETURN
  ...
  SUBROUTINE ReadStreamRatingTables(iGWNodeIDs,NRTB,Stratigraphy,StrmGWConnector,DataFile,AppStream,iStat)
    ...
    DO indxNode=1,AppStream%NStrmNodes
        ...
        CALL AppStream%RatingTable_WetPerimeter(iNode)%New(ModuleLogger,NRTB,HRTB,WPTB,iStat)  ;  IF (iStat .EQ. -1) RETURN
...
! ******************************************************************
  ...
  ! -------------------------------------------------------------
  SUBROUTINE AppStream_v421_Simulate(AppStream,GWHeads,GWReturnFlow,Runoff,ReturnFlow,PondDrain,TributaryFlow,DrainInflows,RiparianET,ETData,RiparianETFrac,StrmGWConnector,StrmLakeConnector,Matrix)
    CLASS(AppStream_v421_Type)  :: AppStream
    ...
    INTEGER                                 :: indxNode,indx,iNode,indxReach,ErrorCode,iNodeIDs_Connect(1),NNodes,NDiver,        &
                                               iAreaCol,iEvapCol
    REAL(8)                                 :: rInflow,rOutflow,Bypass_Recieved,rUpdateValues(1),rWetPerimeter,rArea,            &
                                               rEvapPot,rEvapAct,rValue,rBypassOut,rRipET
        ...
        DO indxNode=AppStream%Reaches(indxReach)%UpstrmNode,AppStream%Reaches(indxReach)%DownstrmNode
            ...
            ELSE
                ...
                IF (iAreaCol .EQ. 0) THEN
                    rWetPerimeter = MAX(0.0 , AppStream%RatingTable_WetPerimeter(indxNode)%Evaluate(AppStream%State(indxNode)%Head))
    ...
    CALL StrmGWConnector%Simulate(NNodes,HRG,AppStream%State%Head,rNetInflows,.TRUE.,Matrix,AppStream%RatingTable_WetPerimeter)
  ...
  ! -------------------------------------------------------------
  ! --- CALCULATE EFFECT OF STREAM FLOWS ON RHS VECTOR ONLY
  ! -------------------------------------------------------------
  SUBROUTINE AppStream_v421_ComputeRHS(AppStream,GWHeads,GWReturnFlow,Runoff,ReturnFlow,PondDrain,TributaryFlow,DrainInflows,RiparianET,ETData,RiparianETFrac,StrmGWConnector,StrmLakeConnector,Matrix)
    CLASS(AppStream_v421_Type)  :: AppStream
    REAL(8),INTENT(IN)          :: GWHeads(:,:),GWReturnFlow(:),Runoff(:),ReturnFlow(:),PondDrain(:),TributaryFlow(:),DrainInflows(:),RiparianET(:)
    TYPE(ETType),INTENT(IN)     :: ETData
    REAL(8),INTENT(OUT)         :: RiparianETFrac(:)
    ...
    REAL(8)                                 :: rInflow,rOutflow,Bypass_Recieved,rWetPerimeter,rArea,rEvapPot,rEvapAct,     &
                                               rBypassOut,rRipET
    ...
    !Simulate stream-gw interaction
    CALL StrmGWConnector%Simulate_JFNK(NNodes,HRG,AppStream%State%Head,rNetInflows,.TRUE.,Matrix,AppStream%RatingTable_WetPerimeter)
    
    !Clear memory
    DEALLOCATE (iStrmIDs , iLakeIDs , STAT=ErrorCode)
        
  END SUBROUTINE AppStream_v421_ComputeRHS
