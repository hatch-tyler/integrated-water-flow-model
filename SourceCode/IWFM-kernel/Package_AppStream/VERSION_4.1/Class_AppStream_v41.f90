...
! ******************************************************************
  ...
  ! -------------------------------------------------------------
  TYPE,EXTENDS(BaseAppStreamType) :: AppStream_v41_Type
    PRIVATE
    TYPE(StrmNode_v41_Type),ALLOCATABLE :: Nodes(:)
  CONTAINS
    PROCEDURE,PASS :: SetStaticComponent             => AppStream_v41_SetStaticComponent
    ...
    PROCEDURE,PASS :: SetStrmFlow                    => AppStream_v41_SetStrmFlow
    ...
    PROCEDURE,PASS :: GetDiversionRechargeZone       => AppStream_v41_GetDiversionRechargeZone
    PROCEDURE,PASS :: GetStrmGWFlow_GivenStrmFlow    => AppStream_v41_GetStrmGWFlow_GivenStrmFlow
    ...
    PROCEDURE,PASS :: ConvertTimeUnit                => AppStream_v41_ConvertTimeUnit
    PROCEDURE,PASS :: ConvertFlowToElev              => AppStream_v41_ConvertFlowToElev
    PROCEDURE,PASS :: Simulate                       => AppStream_v41_Simulate
    PROCEDURE,PASS :: ComputeRHS                     => AppStream_v41_ComputeRHS
  END TYPE AppStream_v41_Type
  ...
  CHARACTER(LEN=ModNameLen),PARAMETER :: ModName         = 'Class_AppStream_v41::'
...
CONTAINS
  ...
  ! -------------------------------------------------------------
  SUBROUTINE AppStream_v41_SetModuleLogger(Logger)
    TYPE(MessageLoggerType), TARGET, INTENT(INOUT) :: Logger
    ModuleLogger => Logger
  END SUBROUTINE AppStream_v41_SetModuleLogger
...
! ******************************************************************
  ...
  SUBROUTINE AppStream_v41_SetStaticComponent(AppStream,cFileName,AppGrid,Stratigraphy,IsRoutedStreams,StrmGWConnector,StrmLakeConnector,iStat)
    ...
    IF (NRTB .LE. 1) THEN
        CALL ModuleLogger%SetLastMessage('Number of data points in stream rating tables should be greater than 1!',f_iFatal,ThisProcedure)
        iStat = -1
        RETURN
    END IF
    ...
    IF (ErrorCode .NE. 0) THEN
        CALL ModuleLogger%SetLastMessage('Error allocating memory for stream configuration data!',f_iFatal,ThisProcedure)
        iStat = -1
        RETURN
    END IF
  ...
  END SUBROUTINE AppStream_v41_SetStaticComponent
  
  
  ! -------------------------------------------------------------
  ! --- INSTANTIATE DYNAMIC PART OF STREAM DATA (GENERALLY CALLED IN SIMULATION)
  ! -------------------------------------------------------------
  SUBROUTINE AppStream_v41_SetDynamicComponent(AppStream,IsForInquiry,cFileName,cWorkingDirectory,cPackageVersion,TimeStep,NTIME,iLakeIDs,AppGrid,Stratigraphy,ETData,StrmLakeConnector,StrmGWConnector,iStat)
    ...
    cVersionFull   = '4.1-' // TRIM(cPackageVersion)
    ...
    CALL ReadVersion(MainFile,'STREAM',cVersionSim,iStat,AppStream%Logger)
    IF (iStat .EQ. -1) RETURN
    IF (TRIM(cVersionSim) .NE. '4.1') THEN
        MessageArray(1) = 'Stream Component versions used in Pre-Processor and Simulation must match!'
        MessageArray(2) = 'Version number in Pre-Processor = 4.1' 
    ...
    IF (lRoutedStreams) THEN
        CALL AppStream%AppDiverBypass%New(AppStream%Logger,IsForInquiry,DiverSpecFileName,BypassSpecFileName,DiverFileName,DiverDetailBudFileName,cWorkingDirectory,cVersionFull,NTIME,TimeStep,AppStream%NStrmNodes,iStrmNodeIDs,iLakeIDs,AppStream%Reaches,AppGrid,StrmLakeConnector,iStat)
        IF (iStat .EQ. -1) RETURN
    END IF
    ...
    IF (lRoutedStreams) THEN
        IF (ReachBudRawFileName .NE. '') THEN
            ...
            ELSE
                ...
                BudHeader = PrepareStreamBudgetHeader(AppStream%NReaches,AppStream%iPrintReachBudgetOrder,iReachIDs,iStrmNodeIDs,NTIME,TimeStep,cVersionFull,cReachNames=AppStream%Reaches%cName)
    ...
    CALL AppStream%StrmNodeBudget%New(AppStream%Logger,lRoutedStreams,IsForInquiry,cWorkingDirectory,iReachIDs,iStrmNodeIDs,NTIME,TimeStep,cVersionFull,PrepareStreamBudgetHeader,MainFile,iStat)
    IF (iStat .EQ. -1) RETURN
  ...
  END SUBROUTINE AppStream_v41_SetDynamicComponent
  
  
  ! -------------------------------------------------------------
  ! --- INSTANTIATE COMPLETE STREAM DATA
  ! -------------------------------------------------------------
  SUBROUTINE AppStream_v41_SetAllComponents(AppStream,IsForInquiry,cFileName,cSimWorkingDirectory,cPackageVersion,TimeStep,NTIME,iLakeIDs,AppGrid,Stratigraphy,ETData,BinFile,StrmLakeConnector,StrmGWConnector,iStat)
    CLASS(AppStream_v41_Type),INTENT(OUT) :: AppStream
    LOGICAL,INTENT(IN)                    :: IsForInquiry
    ...
    !Local variables
    CHARACTER(LEN=ModNameLen+30) :: ThisProcedure = ModName // 'AppStream_v41_SetAllComponents'
    ...
    CALL AppStream_v41_SetDynamicComponent(AppStream,IsForInquiry,cFileName,cSimWorkingDirectory,cPackageVersion,TimeStep,NTIME,iLakeIDs,AppGrid,Stratigraphy,ETData,StrmLakeConnector,StrmGWConnector,iStat)
    IF (iStat .EQ. -1) RETURN
  ...
  END SUBROUTINE AppStream_v41_SetAllComponents
  
  
  ! -------------------------------------------------------------
  ! --- INSTANTIATE COMPLETE STREAM DATA WITHOUT INTERMEDIATE BINARY FILE
  ! -------------------------------------------------------------
  SUBROUTINE AppStream_v41_SetAllComponentsWithoutBinFile(AppStream,IsForInquiry,IsRoutedStreams,cPPFileName,cSimFileName,cSimWorkingDirectory,cPackageVersion,AppGrid,Stratigraphy,ETData,TimeStep,NTIME,iLakeIDs,StrmLakeConnector,StrmGWConnector,iStat)
    CLASS(AppStream_v41_Type),INTENT(OUT) :: AppStream
    ...
    !Local variables
    CHARACTER(LEN=ModNameLen+44) :: ThisProcedure = ModName // 'AppStream_v41_SetAllComponentsWithoutBinFile'
    ...
    CALL AppStream_v41_SetStaticComponent(AppStream,cPPFileName,AppGrid,Stratigraphy,IsRoutedStreams,StrmGWConnector,StrmLakeConnector,iStat)
    IF (iStat .EQ. -1) RETURN
    
    !Instantiate the dynamic component of the AppStream data
    CALL AppStream_v41_SetDynamicComponent(AppStream,IsForInquiry,cSimFileName,cSimWorkingDirectory,cPackageVersion,TimeStep,NTIME,iLakeIDs,AppGrid,Stratigraphy,ETData,StrmLakeConnector,StrmGWConnector,iStat)
    IF (iStat .EQ. -1) RETURN
  ...
  END SUBROUTINE AppStream_v41_SetAllComponentsWithoutBinFile
...
! ******************************************************************
  ...
  ! -------------------------------------------------------------
  SUBROUTINE AppStream_v41_Kill(AppStream)
    CLASS(AppStream_v41_Type) :: AppStream
  ...
  END SUBROUTINE AppStream_v41_Kill
...
! ******************************************************************
  ...
  ! -------------------------------------------------------------
  SUBROUTINE AppStream_v41_SetStrmFlow(AppStream,iStrmNode,rFlow)
    CLASS(AppStream_v41_Type) :: AppStream
  ...
  END SUBROUTINE AppStream_v41_SetStrmFlow
...
! ******************************************************************
  ...
  ! -------------------------------------------------------------
  FUNCTION AppStream_v41_GetStrmGWFlow_GivenStrmFlow(AppStream,iStrmNode,rStrmFlow,rGWHeads,StrmGWConnector) RESULT(rStrmGWFlow)
    ...
    rStrmGWFlow = StrmGWConnector%ComputeStrmGWFlow_GivenStrmFlow(iStrmNode,rStrmFlow,rStrmHead,rGWHeadsAtStrmNodes,WetPerimeterFunction=AppStream%Nodes(iStrmNode)%RatingTable_WetPerimeter)
    
  END FUNCTION AppStream_v41_GetStrmGWFlow_GivenStrmFlow
  ...
  SUBROUTINE AppStream_v41_GetStageFlowRatingTable(AppStream,iNode,Stage,Flow)
    CLASS(AppStream_v41_Type),TARGET,INTENT(IN) :: AppStream
    INTEGER,INTENT(IN)                          :: iNode
    REAL(8),INTENT(OUT)                         :: Stage(:),Flow(:)
    
    !Local variables
    TYPE(StrmNode_v41_Type),POINTER :: pStrmNode
  ...
  END SUBROUTINE AppStream_v41_GetStageFlowRatingTable


  ! -------------------------------------------------------------
  ! --- GET NUMBER OF RATING TABLE POINTS AT A STREAM NODE
  ! -------------------------------------------------------------
  PURE FUNCTION AppStream_v41_GetNRatingTablePoints(AppStream,iStrmNode) RESULT(N)
    CLASS(AppStream_v41_Type),INTENT(IN) :: AppStream
    INTEGER,INTENT(IN)                   :: iStrmNode
    INTEGER                              :: N
  ...
  END FUNCTION AppStream_v41_GetNRatingTablePoints


  ! -------------------------------------------------------------
  ! --- GET BOTTOM ELEVATIONS
  ! -------------------------------------------------------------
  PURE FUNCTION AppStream_v41_GetBottomElevations(AppStream) RESULT(BottomElev)
    CLASS(AppStream_v41_Type),INTENT(IN) :: AppStream
    REAL(8)                              :: BottomElev(AppStream%NStrmNodes)
    
    BottomElev = AppStream%Nodes%BottomElev
    
  END FUNCTION AppStream_v41_GetBottomElevations
  ...
  ! -------------------------------------------------------------
  SUBROUTINE AppStream_v41_GetDiversionRechargeZone(AppStream,cStrmSimMainFileName,cWorkingDirectory,iElemIDs,iDiver,iElems,rFracs,iStat)
     CLASS(AppStream_v41_Type),INTENT(IN) :: AppStream
...
10  CALL vMainFile%Kill()
  ...
  END SUBROUTINE AppStream_v41_GetDiversionRechargeZone
...
! ******************************************************************
  ...
  SUBROUTINE ReadPreprocessedData(AppStream,BinFile,iStat)
    ...
    IF (ErrorCode .NE. 0) THEN
        CALL ModuleLogger%SetLastMessage('Error allocating memory for stream data!',f_iFatal,ThisProcedure)
        iStat = -1
        RETURN
    END IF
  ...
  SUBROUTINE ReadStreamConfigData(DataFile,AppGrid,Stratigraphy,iGWNodeIDs,StrmGWConnector,StrmLakeConnector,AppStream,iStat)
    ...
    CALL StrmGWConnector%New(41,iGWNodes,iLayers,iStat)
    IF (iStat .EQ. -1) RETURN
  ...
  SUBROUTINE ReadStreamRatingTables(iGWNodeIDs,NRTB,Stratigraphy,StrmGWConnector,DataFile,AppStream,iStat)
    ...
    AppStream%TimeUnitRatingTableFlow = ADJUSTL(StripTextUntilCharacter(ALine,f_cInlineCommentChar))
...
! ******************************************************************
  ...
  ! -------------------------------------------------------------
  SUBROUTINE AppStream_v41_WritePreprocessedData(AppStream,OutFile)
    CLASS(AppStream_v41_Type),INTENT(IN) :: AppStream
  ...
  END SUBROUTINE AppStream_v41_WritePreprocessedData
  ...
  END SUBROUTINE AppStream_v41_WriteDataToTextFile
...
! ******************************************************************
  ...
  ! -------------------------------------------------------------
  SUBROUTINE AppStream_v41_ConvertFlowToElev(AppStream)
    CLASS(AppStream_v41_Type) :: AppStream
  ...
  END SUBROUTINE AppStream_v41_ConvertFlowToElev

  
  ! -------------------------------------------------------------
  ! --- CONVERT TIME UNIT OF STREAMS RELATED ENTITIES
  ! -------------------------------------------------------------
  SUBROUTINE AppStream_v41_ConvertTimeUnit(AppStream,NewUnit)
    CLASS(AppStream_v41_Type)   :: AppStream
  ...
  END SUBROUTINE AppStream_v41_ConvertTimeUnit
  
  
  ! -------------------------------------------------------------
  ! --- CALCULATE STREAM FLOWS
  ! -------------------------------------------------------------
  SUBROUTINE AppStream_v41_Simulate(AppStream,GWHeads,GWReturnFlow,Runoff,ReturnFlow,PondDrain,TributaryFlow,DrainInflows,RiparianET,ETData,RiparianETFrac,StrmGWConnector,StrmLakeConnector,Matrix)
    CLASS(AppStream_v41_Type)   :: AppStream
    ...
    REAL(8),DIMENSION(AppStream%NStrmNodes) :: dFlow_dStage,Inflows,rUpdateRHS,HRG,rNetInflows,rSpills_Received
    ...
    CALL StrmGWConnector%Simulate(NNodes,HRG,AppStream%State%Head,rNetInflows,.TRUE.,Matrix,AppStream%Nodes%RatingTable_WetPerimeter)  
    
    !Clear memory
    DEALLOCATE (iStrmIDs , iLakeIDs , STAT=ErrorCode)
        
  END SUBROUTINE AppStream_v41_Simulate
  
  
  ! -------------------------------------------------------------
  ! --- CALCULATE EFFECT OF STREAM FLOWS FOR ON RHS VECTOR ONLY
  ! -------------------------------------------------------------
  SUBROUTINE AppStream_v41_ComputeRHS(AppStream,GWHeads,GWReturnFlow,Runoff,ReturnFlow,PondDrain,TributaryFlow,DrainInflows,RiparianET,ETData,RiparianETFrac,StrmGWConnector,StrmLakeConnector,Matrix)
    CLASS(AppStream_v41_Type)   :: AppStream
    REAL(8),INTENT(IN)          :: GWHeads(:,:),GWReturnFlow(:),Runoff(:),ReturnFlow(:),PondDrain(:),TributaryFlow(:),DrainInflows(:),RiparianET(:)
    TYPE(ETType),INTENT(IN)     :: ETData
    REAL(8),INTENT(OUT)         :: RiparianETFrac(:)
    ...
    REAL(8)                                 :: rInflow,rOutflow,Bypass_Recieved,rBypassOut,rRipET,rArea,rWetPerimeter,rEvapPot,rEvapAct
    REAL(8),DIMENSION(AppStream%NStrmNodes) :: Inflows,rUpdateRHS,HRG,rNetInflows,rSpills_Received
    INTEGER,ALLOCATABLE                     :: iStrmIDs(:),iLakeIDs(:)
    ...
    !Simulate stream-gw interaction
    CALL StrmGWConnector%Simulate_JFNK(NNodes,HRG,AppStream%State%Head,rNetInflows,.TRUE.,Matrix,AppStream%Nodes%RatingTable_WetPerimeter)  
    
    !Clear memory
    DEALLOCATE (iStrmIDs , iLakeIDs , STAT=ErrorCode)
        
  END SUBROUTINE AppStream_v41_ComputeRHS
