...
! ******************************************************************
  ...
  TYPE,EXTENDS(BaseAppStreamType) :: AppStream_v42_Type
    ...
    REAL(8),ALLOCATABLE            :: rWetPerimeter(:)
  CONTAINS
    ...
    PROCEDURE,PASS :: SetStrmFlow                    => AppStream_v42_SetStrmFlow
    PROCEDURE,PASS :: GetStrmNodeIDs                 => AppStream_v42_GetStrmNodeIDs
    PROCEDURE,PASS :: GetStrmNodeID                  => AppStream_v42_GetStrmNodeID
    PROCEDURE,PASS :: GetStrmNodeIndex               => AppStream_v42_GetStrmNodeIndex
    ...
    PROCEDURE,PASS :: GetDiversionRechargeZone       => AppStream_v42_GetDiversionRechargeZone
    PROCEDURE,PASS :: GetStrmGWFlow_GivenStrmFlow    => AppStream_v42_GetStrmGWFlow_GivenStrmFlow
    ...
    PROCEDURE,PASS :: ComputeRHS                     => AppStream_v42_ComputeRHS
...
! ******************************************************************
  ...
  ! -------------------------------------------------------------
  SUBROUTINE AppStream_v42_SetAllComponentsWithoutBinFile(AppStream,IsForInquiry,IsRoutedStreams,cPPFileName,cSimFileName,cSimWorkingDirectory,cPackageVersion,AppGrid,Stratigraphy,ETData,TimeStep,NTIME,iLakeIDs,StrmLakeConnector,StrmGWConnector,iStat)
    CLASS(AppStream_v42_Type),INTENT(OUT) :: AppStream
...
! ******************************************************************
  ...
  ! -------------------------------------------------------------
  SUBROUTINE AppStream_v42_SetStrmFlow(AppStream,iStrmNode,rFlow)
    CLASS(AppStream_v42_Type) :: AppStream
  ...
  END SUBROUTINE AppStream_v42_SetStrmFlow
...
! ******************************************************************
  ...
  ! -------------------------------------------------------------
  FUNCTION AppStream_v42_GetStrmGWFlow_GivenStrmFlow(AppStream,iStrmNode,rStrmFlow,rGWHeads,StrmGWConnector) RESULT(rStrmGWFlow)
    ...
    !Local variables
    REAL(8)             :: rStrmHead
    REAL(8),ALLOCATABLE :: rGWHeadsAtStrmNodes(:)
    
    !Retrieve gw heads at stream nodes
    ALLOCATE (rGWHeadsAtStrmNodes(StrmGWConnector%GetnTotalGWNodes()))
  ...
  END FUNCTION AppStream_v42_GetStrmGWFlow_GivenStrmFlow
  
    
  ! -------------------------------------------------------------
  ! --- GET STREAM NODE IDS
  ! -------------------------------------------------------------
  PURE SUBROUTINE AppStream_v42_GetStrmNodeIDs(AppStream,iStrmNodeIDs)
    CLASS(AppStream_v42_Type),INTENT(IN) :: AppStream
    INTEGER,INTENT(OUT)                  :: iStrmNodeIDs(:)
    
    iStrmNodeIDs = AppStream%Nodes%ID
    
  END SUBROUTINE AppStream_v42_GetStrmNodeIDs


  ! -------------------------------------------------------------
  ! --- GET STREAM NODE ID GIVEN INDEX
  ! -------------------------------------------------------------
  PURE FUNCTION AppStream_v42_GetStrmNodeID(AppStream,indx) RESULT(iStrmNodeID)
    CLASS(AppStream_v42_Type),INTENT(IN) :: AppStream
  ...
  END FUNCTION AppStream_v42_GetStrmNodeID


  ! -------------------------------------------------------------
  ! --- GET STREAM NODE INDEX GIVEN ID
  ! -------------------------------------------------------------
  PURE FUNCTION AppStream_v42_GetStrmNodeIndex(AppStream,ID) RESULT(Index)
    CLASS(AppStream_v42_Type),INTENT(IN) :: AppStream
  ...
  END FUNCTION AppStream_v42_GetStrmNodeIndex
  ...
  ! -------------------------------------------------------------
  FUNCTION AppStream_v42_GetNUpstrmNodes(AppStream,iStrmNode) RESULT(iNNodes)
    CLASS(AppStream_v42_Type),INTENT(IN)  :: AppStream
    INTEGER,INTENT(IN)                    :: iStrmNode
    INTEGER                               :: iNNodes
  ...
  END FUNCTION AppStream_v42_GetNUpstrmNodes
  ...
  ! -------------------------------------------------------------
  SUBROUTINE AppStream_v42_GetDiversionRechargeZone(AppStream,cStrmSimMainFileName,cWorkingDirectory,iElemIDs,iDiver,iElems,rFracs,iStat)
     CLASS(AppStream_v42_Type),INTENT(IN) :: AppStream
...
10  CALL vMainFile%Kill()
  ...
  END SUBROUTINE AppStream_v42_GetDiversionRechargeZone
...
! ******************************************************************
  ...
  SUBROUTINE ReadStreamConfigData(DataFile,AppGrid,Stratigraphy,iGWNodeIDs,StrmGWConnector,StrmLakeConnector,AppStream,iStat)
    ...
    INTEGER                      :: indxReach,DummyIntArray3(3),indxNode,DummyIntArray2(2),     &
                                    iDestNode,NNodes,iGWNodes(100),iGWNode_Work,iLoc,indx,      &
                                    iLayers(100),nGWNodes,iStrmNodeID,indxStrmNode,iReachID,    &
                                    indxNode1,indxReach1
    CHARACTER                    :: ALine*2000
    INTEGER,ALLOCATABLE          :: iGWNodes_Work(:),iGWNodes_Strm(:)
        ...
        ASSOCIATE (pReach => AppStream%Reaches(indxReach))
            ...
            DO indxNode=1,NNodes
                ...
                DO indx=1,99
                    ...
                    nGWNodes = nGWNodes + 1
                    READ (ALine,*) iGWNode_Work  ;  iLoc = FirstLocation(' ',ALine)  ;  ALine = ALine(iLoc+1:)
                    CALL ConvertID_To_Index(iGWNode_Work,iGWNodeIDs,iGWNodes(nGWNodes))
                    IF (iGWNodes(nGWNodes) .EQ. 0) THEN
                        CALL ModuleLogger%SetLastMessage('Groundwater node '//TRIM(IntToText(iGWNode_Work))//' listed in stream reach '//TRIM(IntToText(pReach%ID))//' ('//TRIM(pReach%cName)//') for stream node '//TRIM(IntToText(iStrmNodeID))//' is not in the model!',f_iFatal,ThisProcedure)
                        iStat = -1
                        RETURN
                    END IF
    ...
    ALLOCATE (iGWNodes_Strm(AppStream%NStrmNodes)) 
    DO indxNode=1,AppStream%NStrmNodes
        CALL StrmGWConnector%GetGWNodesAtStrmNode(indxNode,iGWNodes_Work,iStat)  
        IF (iStat .EQ. -1) RETURN
        iGWNodes_Strm(indxNode) = iGWNodes_Work(1)
    END DO
  ...
  SUBROUTINE ReadFractionsForGW(DataFile,iStrmNodeIDs,StrmGWConnector,iStat)
    ...
    REAL(8)                                :: rDummy(2)
    INTEGER,ALLOCATABLE                    :: iStrmNodes(:),iGWNodes(:)
    ...
    DO indx=1,iNStrmNodes
        ...
        IF (iStrmNodes(indx) .EQ. 0) THEN
            CALL ModuleLogger%SetLastMessage('Stream node ID '//TRIM(IntToText(iStrmNode))//' listed for partial stream-aquifer interaction is not in the model!',f_iFatal,ThisProcedure)
            iStat = -1
            RETURN
        END IF
    END DO
    
    !Make sure stream nodes listed do not have more than one gw node associated with them
    DO indx=1,iNStrmNodes
        iStrmNode = iStrmNodes(indx)
        CALL StrmGWConnector%GetGWNodesAtStrmNode(iStrmNode,iGWNodes,iStat)  ;  IF (iStat .EQ. -1) RETURN
        IF (SIZE(iGWNodes) .GT. 1) THEN
            MessageArray(1) = 'Stream nodes that have more than one groundwater node associated with them cannot'
            MessageArray(2) = 'be assigned fractions less than 1.0 to apply the stream-aquifer interaction to the'
            MessageArray(3) = 'corresponding groundwater node.'
            MessageArray(4) = 'Stream node ' // TRIM(IntTotext(iStrmNode)) // ' has ' // TRIM(IntTotext(SIZE(iGWNodes))) // ' groundwater nodes associated with it.'
            CALL ModuleLogger%SetLastMessage(MessageArray(1:4),f_iFatal,ThisProcedure)
            iStat = -1
            RETURN
        END IF
...
! ******************************************************************
  ...
  ! -------------------------------------------------------------
  SUBROUTINE AppStream_v42_WriteDataToTextFile(AppStream,iGWNodeIDs,UNITLTOU,FACTLTOU,Stratigraphy,StrmGWConnector,iStat)
    CLASS(AppStream_v42_Type),INTENT(IN) :: AppStream
    INTEGER,INTENT(IN)                   :: iGWNodeIDs(:)
    ...
    !Local variables
    INTEGER             :: indxReach,indxNode,iGWNode,iLayer,ErrorCode,indx,iGWNodeID,iStrmNodeIDs(AppStream%NStrmNodes)
    ...
    DO indxReach=1,AppStream%NReaches
        DO indxNode=AppStream%Reaches(indxReach)%UpstrmNode,AppStream%Reaches(indxReach)%DownstrmNode
            ...
            DO indx=1,SIZE(iGWNodes)
                ...
                ELSE
                    WRITE (ALine,'(13X,I6,5F10.1)')              iGWNodeID                                 , &
                                                                 GSElev*FACTLTOU                           , &
                                                                 StrmBottom*FACTLTOU                       , &
                                                                 DELZ*FACTLTOU                             , &
                                                                 AquiferBottom*FACTLTOU                    , &
...
! ******************************************************************
  ...
  ! -------------------------------------------------------------
  SUBROUTINE AppStream_v42_Simulate(AppStream,GWHeads,GWReturnFlow,Runoff,ReturnFlow,PondDrain,TributaryFlow,DrainInflows,RiparianET,ETData,RiparianETFrac,StrmGWConnector,StrmLakeConnector,Matrix)
    CLASS(AppStream_v42_Type)   :: AppStream
    ...
    INTEGER                                 :: indxNode,indx,iNode,indxReach,ErrorCode,iNodeIDs_Connect(1),NNodes,NDiver, &
                                               iAreaCol,iEvapCol
    REAL(8)                                 :: rInflow,rOutflow,rBypass_Received,rUpdateValues(1),rValue,rBypassOut,      &
                                               rRipET,rEvapAct,rEvapPot,rArea
    ...
    DO indxReach=1,AppStream%NReaches
        DO indxNode=AppStream%Reaches(indxReach)%UpstrmNode,AppStream%Reaches(indxReach)%DownstrmNode
            ...
            !Received bypass
            rBypass_Received = AppStream%AppDiverBypass%GetBypassReceived_AtADestination(f_iFlowDest_StrmNode,indxNode)
            ...
            rInflow = Inflows(indxNode)                                       &    !Inflow as defined by the user
                    ...
                    + rBypass_Received                                        &    !Received by-pass flows 
  ...
  ! -------------------------------------------------------------
  SUBROUTINE AppStream_v42_ComputeRHS(AppStream,GWHeads,GWReturnFlow,Runoff,ReturnFlow,PondDrain,TributaryFlow,DrainInflows,RiparianET,ETData,RiparianETFrac,StrmGWConnector,StrmLakeConnector,Matrix)
    CLASS(AppStream_v42_Type)   :: AppStream
    REAL(8),INTENT(IN)          :: GWHeads(:,:),GWReturnFlow(:),Runoff(:),ReturnFlow(:),PondDrain(:),TributaryFlow(:),DrainInflows(:),RiparianET(:)
    TYPE(ETType),INTENT(IN)     :: ETData
    REAL(8),INTENT(OUT)         :: RiparianETFrac(:)
    ...
    REAL(8)                                 :: rInflow,rOutflow,Bypass_Recieved,rBypassOut,rRipET,rEvapAct,rEvapPot,rArea
  ...
  END SUBROUTINE AppStream_v42_ComputeRHS
