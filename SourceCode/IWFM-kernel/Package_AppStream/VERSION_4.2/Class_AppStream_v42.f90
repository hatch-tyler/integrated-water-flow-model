...
! ******************************************************************
  ...
  SUBROUTINE AppStream_v42_SetStaticComponent(AppStream,cFileName,AppGrid,Stratigraphy,IsRoutedStreams,StrmGWConnector,StrmLakeConnector,iStat)
    CLASS(AppStream_v42_Type),INTENT(INOUT) :: AppStream
...
! ******************************************************************
            ...
            DO indxNode=1,NNodes
                ...
                DO indx=1,99
                    ...
                    IF (iGWNodes(nGWNodes) .EQ. 0) THEN
                        CALL AppStream%Logger%SetLastMessage('Groundwater node '//TRIM(IntToText(iGWNode_Work))//' listed in stream reach '//TRIM(IntToText(pReach%ID))//' ('//TRIM(pReach%cName)//') for stream node '//TRIM(IntToText(iStrmNodeID))//' is not in the model!',f_iFatal,ThisProcedure)
                        iStat = -1
                        RETURN
                    END IF
  ...
  SUBROUTINE ReadStreamRatingTables(iGWNodeIDs,NRTB,Stratigraphy,StrmGWConnector,DataFile,AppStream,iStat)
    ...
    DO indxNode=1,AppStream%NStrmNodes
        ...
        CALL AppStream%Nodes(iNode)%RatingTable%New(AppStream%Logger,NRTB,HRTB,QRTB,iStat)
        IF (iStat .EQ. -1) RETURN
  ...
  SUBROUTINE ReadFractionsForGW(DataFile,iStrmNodeIDs,StrmGWConnector,Logger,iStat)
    ...
    DO indx=1,iNStrmNodes
        ...
        IF (iStrmNodes(indx) .EQ. 0) THEN
            CALL Logger%SetLastMessage('Stream node ID '//TRIM(IntToText(iStrmNode))//' listed for partial stream-aquifer interaction is not in the model!',f_iFatal,ThisProcedure)
            iStat = -1
            RETURN
        END IF
    END DO
