...
MODULE Class_BaseAppStream
  ...
  USE StrmHydrograph              , ONLY: StrmHydrographType                     , &
                                          ...
                                          iHydStage                              , &
                                          iHydBoth
  USE Class_StrmEvap              , ONLY: StrmEvapType
  ...
  CONTAINS
      ...
      PROCEDURE(Abstract_SetStrmFlow),PASS,DEFERRED                    :: SetStrmFlow
      ...
      PROCEDURE(Abstract_GetDiversionRechargeZone),PASS,DEFERRED       :: GetDiversionRechargeZone
      PROCEDURE(Abstract_GetStrmGWFlow_GivenStrmFlow),PASS,DEFERRED    :: GetStrmGWFlow_GivenStrmFlow
      ...
      PROCEDURE(Abstract_ComputeRHS),PASS,DEFERRED                     :: ComputeRHS
      PROCEDURE,PASS                                                   :: Kill
      ...
      PROCEDURE,NOPASS                                                 :: ComputeStreamLength
  ...
  ! --- BUDGET RELATED DATA
  ! -------------------------------------------------------------
  INTEGER,PARAMETER           :: f_iNStrmBudColumns = 17
  ...
  ABSTRACT INTERFACE
     ...
     SUBROUTINE Abstract_SetStrmFlow(AppStream,iStrmNode,rFlow)
        IMPORT                   :: BaseAppStreamType
        CLASS(BaseAppStreamType) :: AppStream
        INTEGER,INTENT(IN)       :: iStrmNode
        REAL(8),INTENT(IN)       :: rFlow
     END SUBROUTINE Abstract_SetStrmFlow
     ...
     SUBROUTINE Abstract_GetDiversionRechargeZone(AppStream,cStrmSimMainFileName,cWorkingDirectory,iElemIDs,iDiver,iElems,rFracs,iStat)
        IMPORT                              :: BaseAppStreamType 
        CLASS(BaseAppStreamType),INTENT(IN) :: AppStream
        ...
        INTEGER,INTENT(OUT)                 :: iStat
     END SUBROUTINE Abstract_GetDiversionRechargeZone

  
     FUNCTION Abstract_GetStrmGWFlow_GivenStrmFlow(AppStream,iStrmNode,rStrmFlow,rGWHeads,StrmGWConnector) RESULT(rStrmGWFlow)
        IMPORT                               :: BaseAppStreamType,StrmGWConnectorType
        CLASS(BaseAppStreamType),INTENT(IN)  :: AppStream
        INTEGER,INTENT(IN)                   :: iStrmNode
        REAL(8),INTENT(IN)                   :: rStrmFlow,rGWHeads(:,:)
        TYPE(StrmGWConnectorType),INTENT(IN) :: StrmGWConnector
        REAL(8)                              :: rStrmGWFlow
     END FUNCTION Abstract_GetStrmGWFlow_GivenStrmFlow
     ...
     SUBROUTINE Abstract_Simulate(AppStream,GWHeads,Runoff,ReturnFlow,PondDrain,TributaryFlow,DrainInflows,RiparianET,ETData,RiparianETFrac,StrmGWConnector,StrmLakeConnector,Matrix)
        IMPORT                      :: BaseAppStreamType,StrmGWConnectorType,StrmLakeConnectorType,MatrixType,ETType
        CLASS(BaseAppStreamType)    :: AppStream
     ...
     SUBROUTINE Abstract_ComputeRHS(AppStream,GWHeads,Runoff,ReturnFlow,PondDrain,TributaryFlow,DrainInflows,RiparianET,ETData,RiparianETFrac,StrmGWConnector,StrmLakeConnector,Matrix)
        IMPORT                      :: BaseAppStreamType,StrmGWConnectorType,StrmLakeConnectorType,MatrixType,ETType
        CLASS(BaseAppStreamType)    :: AppStream
        REAL(8),INTENT(IN)          :: GWHeads(:,:),Runoff(:),ReturnFlow(:),PondDrain(:),TributaryFlow(:),DrainInflows(:),RiparianET(:)
        TYPE(ETType),INTENT(IN)     :: ETData 
        ...
        TYPE(StrmLakeConnectorType) :: StrmLakeConnector
        TYPE(MatrixType)            :: Matrix
     END SUBROUTINE Abstract_ComputeRHS
...
! ******************************************************************
  ...
  SUBROUTINE GetBudget_MonthlyFlows_GivenFile(Budget,iBudgetType,iLocationIndex,iStrmReachIDs,cBeginDate,cEndDate,rFactVL,rFlows,cFlowNames,iStat)
    ...
    INTEGER                      :: iNTimeSteps,iLocationIndexOrdered,iDimActual
  ...
  SUBROUTINE GetBudget_TSData(AppStream,iBudgetType,iLocationIndex,iCols,cBeginDate,cEndDate,cInterval,rFactLT,rFactAR,rFactVL,rOutputDates,rOutputValues,iDataTypes,inActualOutput,iStat)
    ...
    !Local variables
    INTEGER :: indx,iLocationIndexOrdered
    
    SELECT CASE (iBudgetType)
        CASE (f_iBudgetType_StrmNode)
            CALL AppStream%StrmNodeBudget%GetBudget_TSData(iLocationIndex,iCols,cBeginDate,cEndDate,cInterval,rFactLT,rFactAR,rFactVL,rOutputDates,rOutputValues,iDataTypes,inActualOutput,iStat)
        ...
        CASE (f_iBudgetType_StrmReach)
            IF (AppStream%StrmReachBudRawFile_Defined) THEN
                ...
                !However, reaches are odered based on their reach IDs in Budget file
                !Update the index based on the ordered reach numbers in Budget file
                iLocationIndexOrdered = RoutingOrderedReachIndex_To_IDOrderedReachIndex(AppStream%Reaches%ID,iLocationIndex)
                
                !Read data
                DO indx=1,SIZE(iCols)
                    CALL AppStream%StrmReachBudRawFile%ReadData(iLocationIndexOrdered,iCols(indx),cInterval,cBeginDate,cEndDate,1d0,0d0,0d0,rFactLT,rFactAR,rFactVL,iDataTypes(indx),inActualOutput,rOutputDates,rOutputValues(:,indx),iStat)
                    IF (iStat .NE. 0) RETURN
                END DO
            ELSE
                iStat          = 0
  ...
  SUBROUTINE GetRequiredDiversions_AtSomeDiversions(AppStream,iDivers,rDivers,iStat)
    ...
    CALL AppStream%AppDiverBypass%GetRequiredDiversions_AtSomeDiversions(iDivers,rDivers,iStat)
  ...
  ! -------------------------------------------------------------
  ! --- GET SURFACE EVAPORTAION AT ALL STREAM NODES
  ! -------------------------------------------------------------
  SUBROUTINE GetStrmEvap(AppStream,rEvap)
    CLASS(BaseAppStreamType),INTENT(IN) :: AppStream
    REAL(8),INTENT(OUT)                 :: rEvap(:)
    
    rEvap = AppStream%StrmEvap%rEvap
  ...
  SUBROUTINE GetNames(AppStream,iLocationType,cNamesList)
    ...
    CHARACTER(LEN=*),INTENT(OUT)        :: cNamesList(:)  !Assumes array is previously dimensioned based on the number of locations
    ...
    SELECT CASE (iLocationType)
        ...
        CASE (f_iLocationType_Bypass)
            CALL AppStream%AppDiverBypass%GetBypassNames(cNamesList)
            
        CASE (f_iLocationType_Diversion)
            CALL AppStream%AppDiverBypass%GetDiversionNames(cNamesList)
  ...
  SUBROUTINE GetSupplyAdjustData(AppStream,iDiverRank,iColAdjust,DeliRequired,DeliMax,DeliActual,IrigFracs)
    ...
    ASSOCIATE (pDivers => AppStream%AppDiverBypass%Diver)
      ...
      DeliMax      = pDivers%MaxDiver * (1d0 - pDivers%Ratio_RecvLoss - pDivers%Ratio_NonRecvLoss - pDivers%Ratio_Spills)
...
! ******************************************************************
  ...
  ! --- SET STREAM HEADS
  ! -------------------------------------------------------------
  SUBROUTINE SetHeads(AppStream,rHeads)
    CLASS(BaseAppStreamType) :: AppStream
    REAL(8),INTENT(IN)       :: rHeads(:)
    
    AppStream%State%Head = rHeads
...
! ******************************************************************
  ...
  ! --- READ RESTART DATA
  ! -------------------------------------------------------------
  SUBROUTINE ReadTSData(AppStream,lDiverAdjusted,TimeStep,iDiversions,rDiversions,iStrmInflows,rStrmInflows,iBypasses,rBypasses,StrmLakeConnector,iStat)
     CLASS(BaseAppStreamType)      :: AppStream
     ...
     !Read stream evaporation related data
     CALL AppStream%StrmEvap%ReadTSData(TimeStep,iStat)
     IF (iStat .EQ. -1) RETURN
     
     !Read diversion/bypass related timeseries data
     CALL AppStream%AppDiverBypass%ReadTSData(lDiverAdjusted,TimeStep,iDiversions,rDiversions,iBypasses,rBypasses,StrmLakeConnector,iStat)  
     IF (iStat .EQ. -1) RETURN
...
! ******************************************************************
  ...
  ! -------------------------------------------------------------
  SUBROUTINE PrintResults(AppStream,TimeStep,lEndOfSimulation,QTRIB,QROFF,QRTRN,QRPONDDRAIN,QTDRAIN,QRVET,BottomElev,StrmGWConnector,StrmLakeConnector)
    CLASS(BaseAppStreamType)               :: AppStream
    TYPE(TimeStepType),INTENT(IN)          :: TimeStep
  ...
  SUBROUTINE WriteStrmReachFlowsToBudRawFile(QTRIB,QROFF,QRTRN,QRPONDDRAIN,QTDRAIN,QRVET,StrmGWConnector,StrmLakeConnector,AppStream)
    CLASS(BaseAppStreamType)                           :: AppStream
    REAL(8),DIMENSION(AppStream%NStrmNodes),INTENT(IN) :: QTRIB,QROFF,QRTRN,QRPONDDRAIN,QTDRAIN,QRVET
    TYPE(StrmGWConnectorType),INTENT(IN)               :: StrmGWConnector
    TYPE(StrmLakeConnectorType),INTENT(IN)             :: StrmLakeConnector
    ...
    REAL(8),DIMENSION(AppStream%NReaches) :: UpstrmFlows,DownstrmFlows,TributaryFlows,DrainInflows,Runoff, &
                                             ...
                                             SurfaceEvap,PondDrains,rSpills
    ...
    DummyArray(7,:)  = rSpills
    DummyArray(8,:)  = PondDrains
    DummyArray(9,:)  = StrmGWFlows_InModel
    DummyArray(10,:) = StrmGWFlows_OutModel
    DummyArray(11,:) = LakeInflows
    DummyArray(12,:) = RiparianET
    DummyArray(13,:) = SurfaceEvap
    DummyArray(14,:) = Diversions
    DummyArray(15,:) = Bypasses
    DummyArray(16,:) = Error
    DummyArray(17,:) = DiversionShorts
 ...
 END SUBROUTINE WriteStrmReachFlowsToBudRawFile
  ...
  SUBROUTINE WriteStrmNodeFlowsToBudRawFile(QTRIB,QROFF,QRTRN,QRPONDDRAIN,QTDRAIN,QRVET,StrmGWConnector,StrmLakeConnector,AppStream)
    ...
    REAL(8),DIMENSION(AppStream%StrmNodeBudget%NBudNodes) :: UpstrmFlows,DownstrmFlows,TributaryFlows,DrainInflows,LakeInflows,  &
                                                             ...
                                                             Diversions,Bypasses,DiversionShorts,RiparianET,SurfaceEvap,         &
                                                             PondDrains,rSpills
...
! ******************************************************************
  ...
  ! -------------------------------------------------------------
  ! --- COMPUTE STREAM LENGTH ASSOCIATED WITH EACH NODE
  ! -------------------------------------------------------------
  SUBROUTINE ComputeStreamLength(rX,rY,iGWNodes,iUpstrmNodes,iDownstrmNodes,rLengths,iStat)
    REAL(8),INTENT(IN)  :: rX(:),rY(:)
    INTEGER,INTENT(IN)  :: iGWNodes(:),iUpstrmNodes(:),iDownstrmNodes(:)
    REAL(8),INTENT(OUT) :: rLengths(:)
    INTEGER,INTENT(OUT) :: iStat 
    ...
    !Compute
    DO indxReach=1,SIZE(iUpstrmNodes)
        iUpstrmNode   = iUpstrmNodes(indxReach)
        iDownstrmNode = iDownstrmNodes(indxReach)
        B_DISTANCE    = 0.0
        ...
        DO indxNode=iUpstrmNode,iDownstrmNode-1
            ...
            !Length of the stream at node
            CA                 = rX(iGWDownstrmNode) - rX(iGWNode)
            CB                 = rY(iGWDownstrmNode) - rY(iGWNode)
            F_DISTANCE         = 0.5d0 * SQRT(CA*CA + CB*CB)
            rLengths(indxNode) = B_DISTANCE + F_DISTANCE
        ...
        !Length at last node of the reach
        rLengths(iDownstrmNode) = B_DISTANCE

    END DO
    
  END SUBROUTINE ComputeStreamLength
  ...
  FUNCTION PrepareStreamBudgetHeader(NLocations,iPrintReachBudgetOrder,iReachIDs,iStrmNodeIDs,NTIME,TimeStep,cVersion,cReachNames,iBudNodes) RESULT(Header)
    ...
    !Local variables
    INTEGER,PARAMETER           :: TitleLen           = 238  , &
                                   NTitles            = 3    , &
    ...
    ASSOCIATE (pLocation => Header%Locations(1))
      ...
      pLocation%iDataColumnTypes       = [f_iVR ,&  !Upstream inflow
                                          ...
                                          f_iVR ,&  !Runoff
                                          f_iVR ,&  !Return flow
                                          f_iVR ,&  !Diversion spills received
                                          f_iVR ,&  !Pond drain
      ...
      ASSOCIATE (pColumnHeaders => pLocation%cColumnHeaders           , &
        ...
        TextTime            = ArrangeText(TRIM(UnitT),17)
        pColumnHeaders(:,1) = ['                 ','     Upstream','   Downstream','    Tributary','        Tile ','             ','     Return  ','  Diversion  ','      Pond   ','Gain from GW ',' Gain from GW','    Gain from','   Riparian ','   Surface  ','             ','      By-pass','             ','    Diversion']
        pColumnHeaders(:,2) = ['      Time       ','      Inflow ','    Outflow  ','     Inflow  ','        Drain','       Runoff','      Flow   ','    Spills   ','      Drain  ','inside Model ','outside Model','      Lake   ','      ET    ',' Evaporation','    Diversion','        Flow ','  Discrepancy','    Shortage ']
  ...
  ! -------------------------------------------------------------
  ! --- CONVERT ROUTING-ORDERED REACH INDEX TO ID-ORDERED INDEX
  ! --- Note: Reach IDs are ordered based on the flow routing (upstream to downstream)
  ! ---       Reach budget output in Budget file is ordered based on reach ID numbers
  ! ---       To retrieve correct budget information, routing-ordered index must be converted to ID-ordered index for a reach
  ! -------------------------------------------------------------
  FUNCTION RoutingOrderedReachIndex_To_IDOrderedReachIndex(iStrmReachIDs,iRoutingOrderedIndex) RESULT(IDOrderedIndex)
    INTEGER,INTENT(IN) :: iStrmReachIDs(:),iRoutingOrderedIndex
    INTEGER            :: IDOrderedIndex
    
    !Local variables
    INTEGER :: ID,IDOrderedList(SIZE(iStrmReachIDs))
    
    ID           = iStrmReachIDs(iRoutingOrderedIndex)
    IDOrderedList = iStrmReachIDs
    CALL ShellSort(IDOrderedList)
    IDOrderedIndex = LocateInList(ID,IDOrderedList)
    
  END FUNCTION RoutingOrderedReachIndex_To_IDOrderedReachIndex
