...
! ******************************************************************
  ...
  CONTAINS
    ...
    PROCEDURE,PASS :: GetDiversionSpillsIntoAllNodes
    PROCEDURE,PASS :: GetDiversionSpillsIntoSomeNodes
    PROCEDURE,PASS :: GetDiversionSpillsIntoAllReaches
    ...
    IF (AppDiverBypass%NBypass .GT. 0) THEN
        IF (SUM(AppDiverBypass%Bypasses%RechargeSpecs%iNDest) .GT. 0) THEN
            ...
            IF (ErrorCode .NE. 0) THEN
                ...
                RETURN
            END IF
            CALL ElemToRecvLossPointers(NElements,AppDiverBypass%Bypasses%RechargeSpecs,AppDiverBypass%ElemToBypassRecvLoss)
        END IF
    END IF
    ...
    IF (AppDiverBypass%NDiver .GT. 0) THEN
        IF (SUM(AppDiverBypass%Diver%RechargeSpecs%iNDest) .GT. 0) THEN
           ...
           IF (ErrorCode .NE. 0) THEN
               ...
               RETURN
           END IF
           CALL ElemToRecvLossPointers(NElements,AppDiverBypass%Diver%RechargeSpecs,AppDiverBypass%ElemToDiverRecvLoss)
        END IF
    END IF
...
! ******************************************************************
  ...
  ! -------------------------------------------------------------
  ! --- GET DIVERSION SPILLS INTO EACH STREAM NODE
  ! -------------------------------------------------------------
  SUBROUTINE GetDiversionSpillsIntoAllNodes(AppDiverBypass,rSpillsReceived)
    CLASS(AppDiverBypassType),INTENT(IN) :: AppDiverBypass
    REAL(8),INTENT(OUT)                  :: rSpillsReceived(:)
    
    !Local variables
    INTEGER :: indxDiver,indxDest,iStrmNode
    REAL(8) :: rFrac,rSpills
    
    !Initialize
    rSpillsReceived = 0.0
    
    !Compile spills into nodes
    DO indxDiver=1,AppDiverBypass%NDiver
        rSpills = AppDiverBypass%Diver(indxDiver)%rSpills
        DO indxDest=1,AppDiverBypass%Diver(indxDiver)%SpillSpecs%iNDest
            iStrmNode = AppDiverBypass%Diver(indxDiver)%SpillSpecs%iDestList(indxDest)
            IF (iStrmNode .EQ. 0) CYCLE
            rFrac                      = AppDiverBypass%Diver(indxDiver)%SpillSpecs%rFracs(indxDest)
            rSpillsReceived(iStrmNode) = rSpillsReceived(iStrmNode) +  rSpills * rFrac
        END DO
    END DO
    
  END SUBROUTINE GetDiversionSpillsIntoAllNodes
  
  
  ! -------------------------------------------------------------
  ! --- GET DIVERSION SPILLS INTO SOME STREAM NODES
  ! -------------------------------------------------------------
  SUBROUTINE GetDiversionSpillsIntoSomeNodes(AppDiverBypass,iStrmNodes,rSpillsReceived)
    CLASS(AppDiverBypassType),INTENT(IN) :: AppDiverBypass
    INTEGER,INTENT(IN)                   :: iStrmNodes(:)
    REAL(8),INTENT(OUT)                  :: rSpillsReceived(:)
    
    !Local variables
    INTEGER :: indxDiver,indxDest,iStrmNode,iLoc
    REAL(8) :: rFrac,rSpills
    ...
    DO indxDiver=1,AppDiverBypass%NDiver
        ...
        DO indxDest=1,AppDiverBypass%Diver(indxDiver)%SpillSpecs%iNDest
            iStrmNode = AppDiverBypass%Diver(indxDiver)%SpillSpecs%iDestList(indxDest)
            IF (iStrmNode .EQ. 0) CYCLE
            iLoc = LocateInList(iStrmNode,iStrmNodes)
            IF (iLoc .LT. 1) CYCLE
            rFrac                 = AppDiverBypass%Diver(indxDiver)%SpillSpecs%rFracs(indxDest)
            rSpillsReceived(iLoc) = rSpillsReceived(iLoc) +  rSpills * rFrac
        END DO
    END DO
    
  END SUBROUTINE GetDiversionSpillsIntoSomeNodes
  
  
  ! -------------------------------------------------------------
  ! --- GET DIVERSION SPILLS INTO ALL STREAM REACHES
  ! -------------------------------------------------------------
  SUBROUTINE GetDiversionSpillsIntoAllReaches(AppDiverBypass,iNStrmNodes,iNReaches,Reaches,rSpillsReceived)
    CLASS(AppDiverBypassType),INTENT(IN) :: AppDiverBypass
    INTEGER,INTENT(IN)                   :: iNStrmNodes,iNReaches
    TYPE(StrmReachType),INTENT(IN)       :: Reaches(iNReaches)
    REAL(8),INTENT(OUT)                  :: rSpillsReceived(:)
    
    !Local variables
    INTEGER :: indxReach,iUpstrmNode,iDownstrmNode
    REAL(8) :: rSpillsAll(iNStrmNodes)
    
    !Initialize
    rSpillsReceived = 0.0
    
    !Retrive spills to all nodes
    CALL GetDiversionSpillsIntoAllNodes(AppDiverBypass,rSpillsAll)
    ...
    DO indxReach=1,iNReaches
        ...
        rSpillsReceived(indxReach) = SUM(rSpillsAll(iUpstrmNode:iDownstrmNode))
    END DO
    
  END SUBROUTINE GetDiversionSpillsIntoAllReaches
  ...
  SUBROUTINE GetDiversionRechargeZone(AppDiverBypass,cDivSpecFileName,iElemIDs,iDiver,iElems,rFracs,iStat)
    ...
    TYPE(LossDestinationType),ALLOCATABLE :: RechargeZones(:)
    ...
    IF (AppDiverBypass%NDiver .GT. 0) THEN    
        ALLOCATE (iElems , SOURCE=AppDiverBypass%Diver(iDiver)%RechargeSpecs%iDestList)
        ALLOCATE (rFracs , SOURCE=AppDiverBypass%Diver(iDiver)%RechargeSpecs%rFracs)
        GOTO 10
    END IF
...
10  CALL vDiverSpecFile%Kill()
  ...
  PURE SUBROUTINE GetDiversionsForDeliveries(AppDiverBypass,iDivers,rDelis,rDivers)
    ...
    DO indxDiver=1,SIZE(iDivers)
        ...
        rFrac  = 1d0 - AppDiverBypass%Diver(iDiver)%Ratio_RecvLoss - AppDiverBypass%Diver(iDiver)%Ratio_NonRecvLoss - AppDiverBypass%Diver(iDiver)%Ratio_Spills
  ...
  PURE FUNCTION GetSubregionalRecvLosses(AppDiverBypass,AppGrid) RESULT(RecvLosses)
    ...
    !Local variables
    INTEGER :: indx,indxDest,iElem,iRegion
    REAL(8) :: Frac
    ...
    ASSOCIATE (pDivers => AppDiverBypass%Diver      , &
      ...
      DO indx=1,AppDiverBypass%NDiver
        DO indxDest=1,pDivers(indx)%RechargeSpecs%iNDest
          iElem               = pDivers(indx)%RechargeSpecs%iDestList(indxDest)
          Frac                = pDivers(indx)%RechargeSpecs%rFracs(indxdest)
      ...
      DO indx=1,AppDiverBypass%NBypass
        DO indxDest=1,pBypasses(indx)%RechargeSpecs%iNDest
          iElem               = pBypasses(indx)%RechargeSpecs%iDestList(indxDest)
          Frac                = pBypasses(indx)%RechargeSpecs%rFracs(indxDest)
          iRegion             = AppGrid%AppElement(iElem)%Subregion
    ...
    SELECT CASE (iSource)
      ...
      CASE (f_iDiverRecvLoss)
        ...
        IF (NRecharge .GT. 0) THEN
          ElemRecvLosses = CompileRecvLosses(NElements,NRecharge,AppDiverBypass%Diver%RecvLoss,AppDiverBypass%Diver%RechargeSpecs)
        ELSE
          ElemRecvLosses = 0.0
        END IF
      ...
      CASE (f_iBypassRecvLoss) 
        ...
        IF (NRecharge .GT. 0) THEN
          ElemRecvLosses = CompileRecvLosses(NElements,NRecharge,AppDiverBypass%Bypasses%RecvLoss,AppDiverBypass%Bypasses%RechargeSpecs)
        ELSE
          ElemRecvLosses = 0.0
        END IF
      ...
      CASE (f_iAllRecvLoss)
        ...
        ELSE
          ...
          NRecharge = AppDiverBypass%NBypass
          IF (NRecharge .GT. 0)  &
            ElemRecvLosses = ElemRecvLosses + CompileRecvLosses(NElements,NRecharge,AppDiverBypass%Bypasses%RecvLoss,AppDiverBypass%Bypasses%RechargeSpecs)
        END IF
  ...
  CONTAINS  
    ...
    PURE FUNCTION CompileRecvLosses(NElements,NRecharge,RecvLosses,Recharge) RESULT(ElemRecvLosses)
      ...
      TYPE(LossDestinationType),INTENT(IN) :: Recharge(:)
      REAL(8)                              :: ElemRecvLosses(NElements)
    
      !Local variables
      INTEGER :: indx,indxDest,iElem
      REAL(8) :: Frac
      ...
      !Compile
      DO indx=1,NRecharge
        DO indxDest=1,Recharge(indx)%iNDest
          iElem                 = Recharge(indx)%iDestList(indxdest)
          Frac                  = Recharge(indx)%rFracs(indxDest)
...
! ******************************************************************
  ...
  SUBROUTINE SetDiverRequired(AppDiverBypass,DeliRequired)
    ...
    ASSOCIATE (pDivers => AppDiverBypass%Diver)
      ...
      DO indx=1,AppDiverBypass%NDiver
          rFrac                       = 1d0 - pDivers(indx)%Ratio_RecvLoss - pDivers(indx)%Ratio_NonRecvLoss - pDivers(indx)%Ratio_Spills
  ...
  SUBROUTINE SetDiversionRead(AppDiverBypass,iDiver,rDiversion)
    ...
    INTEGER                                :: ID
    REAL(8)                                :: RecvLoss,NonRecvLoss,rSpills,Factor,rSum,rNormFracs(4)
    ...
    rNormFracs(3) = AppDiverBypass%Diver(iDiver)%rFracSpills
    rNormFracs(4) = AppDiverBypass%Diver(iDiver)%Deli%FracDeli
    CALL NormalizeArray(rNormFracs)
    
    !Deliveries
    AppDiverBypass%Diver(iDiver)%Deli%SupplyRequired = rDiversion * rNormFracs(4)
    ...
    !Spills
    rSpills = rDiversion * rNormFracs(3)
    
    !Total diversions
    AppDiverBypass%Diver(iDiver)%DiverRead = AppDiverBypass%Diver(iDiver)%Deli%SupplyRequired + RecvLoss + NonRecvLoss + rSpills
              
    !Ratio of recoverable and non-recoverable losses and spills to total diversions
    IF (AppDiverBypass%Diver(iDiver)%DiverRead .GT. 0.0) THEN
        ...
        AppDiverBypass%Diver(iDiver)%Ratio_Spills      = rNormFracs(3)
    ELSE !If read diversion is zero, then use the original fractions for recoverable and non-recoverable losses and spills
        rSum = AppDiverBypass%Diver(iDiver)%FracRecvLoss + AppDiverBypass%Diver(iDiver)%FracNonRecvLoss + AppDiverBypass%Diver(iDiver)%rFracSpills + AppDiverBypass%Diver(iDiver)%Deli%FracDeli
        IF (rSum .GT. 0.0) THEN
            ...
            AppDiverBypass%Diver(iDiver)%Ratio_Spills      = AppDiverBypass%Diver(iDiver)%rFracSpills     / rSum
        ELSE
            AppDiverBypass%Diver(iDiver)%Ratio_RecvLoss    = 0.0
            AppDiverBypass%Diver(iDiver)%Ratio_NonRecvLoss = 0.0
            AppDiverBypass%Diver(iDiver)%Ratio_Spills      = 0.0
        END IF
    END IF
...
! ******************************************************************
  ...
  SUBROUTINE ReadTSData(AppDiverBypass,lDiverAdjusted,TimeStep,iDiversions,rDiversions,iBypasses,rBypasses,StrmLakeConnector,iStat)
    ...
    REAL(8)                      :: RecvLoss,NonRecvLoss,Factor,rSum,rBypassReceived,rSpills
      ...
      CASE (0)
        ...
        DO indx=1,AppDiverBypass%NDiver
            ASSOCIATE (pDiver => AppDiverBypass%Diver(indx))
                ...
                !Spills
                rSpills = AppDiverBypass%DiverFile%rValues(pDiver%iColSpills) * pDiver%rFracSpills
                
                !Total diversions
                pDiver%DiverRead = pDiver%Deli%SupplyRequired + RecvLoss + NonRecvLoss + rSpills
                ...
                IF (pDiver%DiverRead .GT. 0.0) THEN
                    ...
                    pDiver%Ratio_Spills      = rSpills     / pDiver%DiverRead
                ELSE
                    rSum = pDiver%FracRecvLoss + pDiver%FracNonRecvLoss + pDiver%rFracSpills + pDiver%Deli%FracDeli
                    IF (rSum .GT. 0.0) THEN
                        pDiver%Ratio_RecvLoss    = pDiver%FracRecvLoss    / rSum
                        pDiver%Ratio_NonRecvLoss = pDiver%FracNonRecvLoss / rSum
                        pDiver%Ratio_Spills      = pDiver%rFracSpills     / rSum
                    ELSE
                        pDiver%Ratio_RecvLoss    = 0.0
                        pDiver%Ratio_NonRecvLoss = 0.0
                        pDiver%Ratio_Spills      = 0.0
                    END IF
                END IF
...
! ******************************************************************
  ...
  SUBROUTINE PrintResults(AppDiverBypass)
    ...
    !Local variables
    REAL(8) :: DummyArray(f_iNDiverDetailColumns,AppDiverBypass%NDiver)
    ...
    DummyArray(5,:) = AppDiverBypass%Diver%rSpills
    DummyArray(6,:) = AppDiverBypass%Diver%Deli%SupplyActual
    DummyArray(7,:) = AppDiverBypass%Diver%Deli%SupplyRequired - AppDiverBypass%Diver%Deli%SupplyActual
...
! ******************************************************************
  ...
  SUBROUTINE ElemToRecvLossPointers(NElements,RechargeZones,ElemToRecvLoss)
    ...
    TYPE(LossDestinationType),TARGET,INTENT(IN) :: RechargeZones(:)
    ...
    DO indxZone=1,SIZE(RechargeZones)
      ASSOCIATE (pZone => RechargeZones(indxZone))
        DO indx=1,pZone%iNDest
          iElem                                 = pZone%iDestList(indx)
  ...
  FUNCTION PrepareDiverDetailsBudgetHeader(NDiver,TimeStep,NTIME,iElemIDs,iStrmNodeIDs,iSubregionIDs,Diversions,cVersion) RESULT(Header)
    ...
    !Local variables
    INTEGER,PARAMETER           :: TitleLen           = 118  , &
                                   NTitles            = 3    , &
    ...
    DO indxDiver=1,NDiver
        ...
        SELECT CASE (iDestType)
            ...
            CASE (f_iFlowDest_Outside)      
                ...
                Text1_DSS(indxDiver)   = 'DELI_SHRT_SUBRG_0'
            ...
            CASE (f_iFlowDest_Element) 
                ...
                Text_DSS(indxDiver)    = 'ACT_DELI_ELM_'//TRIM(IntToText(iDestID))
                Text1_DSS(indxDiver)   = 'DELI_SHRT_ELM_'//TRIM(IntToText(iDestID))
            ...
            CASE (f_iFlowDest_ElementSet)
                ...
                Text_DSS(indxDiver)    = 'ACT_DELI_ELMGRP_'//TRIM(IntToText(iDestID))
                Text1_DSS(indxDiver)   = 'DELI_SHRT_ELMGRP_'//TRIM(IntToText(iDestID))
            ...
            CASE (f_iFlowDest_Subregion)
                ...
                Text_DSS(indxDiver)    = 'ACT_DELI_SBRG_'//TRIM(IntToText(iDestID))
                Text1_DSS(indxDiver)   = 'DELI_SHRT_SBRG_'//TRIM(IntToText(iDestID))
        END SELECT
    END DO
    ...
    ASSOCIATE (pDSSOutput => Header%DSSOutput)
        ...
        DO indxLocation=1,NDiver
            ...
            ELSE
                iStrmNodeID = iStrmNodeIDs(Diversions(indxLocation)%iStrmNode)
            END IF 
            DO indxCol=1,SIZE(FParts)
                iCount                        = iCount+1
                pDSSOutput%cPathNames(iCount) = '/IWFM_DIV_DETL/'                                                                    //  &  !A part
                                                'DIV'//TRIM(IntToText(iDiverID))//'_SN'//TRIM(IntToText(iStrmNodeID))//'/'           //  &  !B part
                                                'VOLUME/'                                                                            //  &  !C part
                                                '/'                                                                                  //  &  !D part
  ...
  SUBROUTINE ComputeDiversions(AppDiverBypass,NStrmNodes)
    ...
    !Update actual diversions, deliveries,recoverable and non-recoverbale losses, and spills
    DO indxDiver=1,AppDiverBypass%NDiver
      ASSOCIATE (pDiver => AppDiverBypass%Diver(indxDiver))
        ...
        pDiver%rSpills     = pDiver%DiverActual * pDiver%Ratio_Spills
  ...
  SUBROUTINE AddBypass(AppDiverBypass,ID,iNode_Exp,iColBypass,cName,rFracRecvLoss,rFracNonRecvLoss,iNRechargeElems,iRechargeElems,rRechargeFractions,iDestType,iDest,StrmLakeConnector,iStat)
    ...
    INTEGER                               :: iNBypass
    TYPE(LossDestinationType)             :: Recharge
    ...
    Recharge%iDestList = iRechargeElems
    Recharge%rFracs    = rRechargeFractions
    CALL NormalizeArray(Recharge%rFracs)
    ...
    TempBypass(iNBypass)%RechargeSpecs   = Recharge
