...
! ******************************************************************
  ...
  ! -------------------------------------------------------------
  ! --- COMPUTE EFFECT OF PUMPING ON THE RHS VECTOR ONLY 
  ! -------------------------------------------------------------
  SUBROUTINE ComputeRHS(AppPumping,AppGrid,Stratigraphy,rStorage,Matrix)
    CLASS(AppPumpingType)             :: AppPumping
    TYPE(AppGridType),INTENT(IN)      :: AppGrid
    TYPE(StratigraphyType),INTENT(IN) :: Stratigraphy
    ...
    !Local variables
    INTEGER                                                      :: indxNode,indxLayer,NNodes,NLayers,indx
    REAL(8)                                                      :: rPumpRequired,rStoragePos,rStor,rNodalPumpActual_New, &
    ...
    !$OMP PARALLEL DEFAULT(PRIVATE) SHARED(NNodes,NLayers,AppPumping,rUpdateRHS,rStorage,Matrix,iCompIDs) 
  ...
  ! -------------------------------------------------------------
  ! --- SIMULATE PUMPING BY MODIFYING THE RHS VECTOR AND JACOBIAN
  ! -------------------------------------------------------------
