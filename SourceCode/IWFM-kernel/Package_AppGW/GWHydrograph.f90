...
! ******************************************************************
  ...
  SUBROUTINE CellVelocityFile_New(IsForInquiry,cFileName,NLayers,AppGrid,CellVelocityFile,UNITLTOU,UNITVROU,FACTLTOU,XC,YC,iStat)
    ...
    CALL CellVelocityFile%WriteData('*'//REPEAT(' ',30)//REPEAT('*',50))
    WRITE (Text,'(A,i12,*(I18))') '*                   LAYER',(indxLayer,indxLayer,indxLayer,indxLayer=1,NLayers)
    CALL CellVelocityFile%WriteData(TRIM(Text))
    WRITE (Text,'(A,*(A))')'*     TIME        ELEMENT          VX                VY                VZ',('                VX                VY                VZ',indxLayer=2,NLayers)
    CALL CellVelocityFile%WriteData(TRIM(Text))
  ...
  SUBROUTINE FaceFlowOutput_New(IsForInquiry,InFile,cWorkingDirectory,AppFace,NLayers,ElementIDs,UNITVLOU,TimeStep,FaceFlowOutput,iStat)
    ...
    CALL InFile%ReadData(cFileName,iStat)  ;  IF (iStat .EQ. -1) RETURN  ;  cFileName = StripTextUntilCharacter(cFileName,f_cInlineCommentChar)  ;  CALL CleanSpecialCharacters(cFileName)  
  ...
  SUBROUTINE AllHeadOutFile_New(cFileName,UNITLTOU,NNodes,NLayers,NodeIDs,TimeStep,NTIME,OutFile,iStat)
    ...
    !If HDF file, create the dataset and return (PrepareTSDOutputFile does not handle HDF)
    IF (OutFile%iGetFileType() .EQ. f_iHDF) THEN
        BLOCK
            INTEGER   :: NColumns_HDF(1)
            CHARACTER :: cDataSetName_HDF(1)*17
            NColumns_HDF(1)     = NNodes * NLayers
            cDataSetName_HDF(1) = '/GWHeadAtAllNodes'
            CALL OutFile%CreateHDFDataSet(cPathNames=cDataSetName_HDF,NColumns=NColumns_HDF,NTime=NTIME+1,TimeStep=TimeStep,DataType=0d0,iStat=iStat)
        END BLOCK
        RETURN
    END IF
...
! ******************************************************************
  ...
  SUBROUTINE PrintInitialValues(GWHydData,AppGrid,Stratigraphy,rHeads,rFACTLTOU,rFACTVROU,StrmConnectivity,TimeStep)
    ...
    IF (GWHydData%lHeadTecplotFile_Defined) THEN
        ...
        IF (GWHydData%lTecPlot_PrintGWHeads) THEN
            ...
            CALL GWHydData%HeadTecplotFile%PrintInitialValues(AppGrid,StrmConnectivity,rHeads,rFactorsHead,'(*(F16.3,2X))',cVarNamesHead,TimeStep)
        ELSE
            ...
            END DO
            CALL GWHydData%HeadTecplotFile%PrintInitialValues(AppGrid,StrmConnectivity,rDepthToGW,rFactorsHead,'(*(F16.3,2X))',cVarNamesHead,TimeStep)
        END IF
    END IF
    ...
    IF (GWHydData%lVelocityTecplotFile_Defined) THEN
        ...
        rDummyVels         = 0.0
        CALL GWHydData%VelocityTecplotFile%PrintInitialValues(AppGrid,StrmConnectivity,rDummyVels,rFactorsVel,'(2(F16.3,2X),*(G16.3E2,2X))',cVarNamesVel,TimeStep)
    END IF
  ...
  SUBROUTINE PrintResults(GWHydData,AppGrid,Stratigraphy,AppBC,GWState,rFaceFlows,rSWShedFaceFlows,rFACTLTOU,rFACTVLOU,rFACTVROU,TimeStep,lEndOfSimulation)
    ...
    IF (GWHydData%lHeadTecplotFile_Defined) THEN
        IF (GWHydData%lTecPlot_PrintGWHeads) THEN
            CALL GWHydData%HeadTecplotFile%PrintResults(GWState%Head,rFACTLTOU,'(*(F16.3,2X))',TimeStep)
        ELSE
            DO indxLayer=1,Stratigraphy%NLayers
                rDepthToGW(:,indxLayer) = Stratigraphy%GSElev - GWState%Head(:,indxLayer) 
            END DO
            CALL GWHydData%HeadTecplotFile%PrintResults(rDepthToGW,rFACTLTOU,'(*(F16.3,2X))',TimeStep)
        END IF
    END IF
    ...
    IF (GWHydData%lVelocityTecplotFile_Defined) THEN
        ...
        DO indxLayer=1,3*Stratigraphy%NLayers,3
            ...
            indx                 = indx + 1
        END DO
        CALL GWHydData%VelocityTecplotFile%PrintResults(rVels,rFACTVROU,'(*(G16.3E2,2X))',TimeStep)
    END IF
  ...
  SUBROUTINE CellVelocitiesOutput_PrintResults(AppGrid,NLayers,TimeStep,GWState,FACTVROU,ElemCentroid_X,ElemCentroid_Y,CellVelocityFile)
    ...
    !First line
    WRITE (Text,'(A16,I9,2X,*(G16.4E2,2X))') TimeStep%CurrentDateAndTime,AppGrid%AppElement(1)%ID,(Vx(indxLayer,1),Vy(indxLayer,1),Vz(indxLayer,1),indxLayer=1,NLayers)
    CALL CellVelocityFile%WriteData(TRIM(Text))
    
    !Rest of the lines
    DO indxElem=2,NElements
        WRITE (Text,'(16X,I9,2X,*(G16.4E2,2X))') AppGrid%AppElement(indxElem)%ID,(Vx(indxLayer,indxElem),Vy(indxLayer,indxElem),Vz(indxLayer,indxElem),indxLayer=1,NLayers)
        CALL CellVelocityFile%WriteData(TRIM(Text))        
    END DO
