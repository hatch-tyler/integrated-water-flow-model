...
! ******************************************************************
  ...
  SUBROUTINE TileDrain_New(InFile,AppGrid,Stratigraphy,iStrmNodeIDs,TimeUnitConductance,TileDrains,iStat)
    ...
    TimeUnitConductance = UpperCase(ADJUSTL(StripTextUntilCharacter(ALine,f_cInlineCommentChar)))
...
! ******************************************************************
  ...
  ! -------------------------------------------------------------
  ! --- COMPUTE EFFECT OF TILE DRAIN/SUBSURFACE IRRIGATION FLOWS ON RHS VECTOR ONLY 
  ! -------------------------------------------------------------
  SUBROUTINE ComputeRHS(AppTileDrain,NNodes,HN,Matrix)
    CLASS(AppTileDrainType) :: AppTileDrain
