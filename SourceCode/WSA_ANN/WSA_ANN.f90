...
! ******************************************************************
  ...
  SUBROUTINE New(WSA,cFileName,cSIMWorkingDirectory,TimeStep,AppGrid,AppStream,iStat)
    ...
    cStrmFlowTSInFile = ADJUSTL(StripTextUntilCharacter(cStrmFlowTSInFile,'/'))
    ...
    DO indx=1,WSA%iNWSA
        ...
        ALine = ADJUSTL(ALine)
        ALine = StripTextUntilCharacter(ALine,'/',Back=.TRUE.)
    ...
    cANNParamFile = ADJUSTL(StripTextUntilCharacter(cANNParamFile,'/'))
    ...
    cGenericTSInFile = ADJUSTL(StripTextUntilCharacter(cGenericTSInFile,'/'))
    ...
    cVarOutFileName = ADJUSTL(StripTextUntilCharacter(cVarOutFileName,'/'))
    ...
    cWSAOutFileName = ADJUSTL(StripTextUntilCharacter(cWSAOutFileName,'/'))
    CALL MainInputFile%ReadData(WSA%rFactorWSAOut,iStat)  ;  IF (iStat .NE. 0) RETURN
    CALL MainInputFile%ReadData(ALine,iStat)              ;  IF (iStat .NE. 0) RETURN
    CALL CleanSpecialCharacters(ALine)
    WSA%cUnitWSAOut = ADJUSTL(StripTextUntilCharacter(ALine,'/'))
  ...
  SUBROUTINE ANNModel_New(WSA,cFileName,cSIMWorkingDirectory,iStat)
    ...
    DO indxWSA=1,iNWSA
        ...
        CALL InFile%ReadData(ALine,iStat)  ;  IF (iStat .NE. 0) RETURN
        CALL CleanSpecialCharacters(ALine)
        ALine = StripTextUntilCharacter(ADJUSTL(ALine),'/')
        CALL ReplaceString(Aline,',',' ',iStat)  ;  IF (iStat .NE. 0) RETURN
