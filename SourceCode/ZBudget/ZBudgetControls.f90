...
CONTAINS
  ...
  SUBROUTINE ProcessZBudgets(cFileName,LenFileName,iStat)
    ...
    CALL MainControlFile%ReadData(ALine,iStat)  ;  IF (iStat .EQ. -1) RETURN ; CALL CleanSpecialCharacters(ALine) ; cUnit_AR = ADJUSTL(StripTextUntilCharacter(ALine,f_cInlineCommentChar))
    CALL MainControlFile%ReadData(Fact_VL,iStat)  ;  IF (iStat .EQ. -1) RETURN
    CALL MainControlFile%ReadData(ALine,iStat)  ;  IF (iStat .EQ. -1) RETURN ; CALL CleanSpecialCharacters(ALine) ; cUnit_VL = ADJUSTL(StripTextUntilCharacter(ALine,f_cInlineCommentChar))
    ...
    IF (cPrintBeginDateAndTime .TSGT. cPrintEndDateAndTime) CALL DefaultLogger%LogMessage('Print-out end date and time cannot be less than the beginning date and time!',f_iFatal,ThisProcedure)
  ...
  SUBROUTINE ProcessZBudget(MainControlFile,iStat)
      ...
      CALL MainControlFile%ReadData(cZoneDefFileName,iStat)  ;  IF (iStat .EQ. -1) RETURN  ;  cZoneDefFileName   = ADJUSTL(StripTextUntilCharacter(cZoneDefFileName,f_cInlineCommentChar))    ;  CALL CleanSpecialCharacters(cZoneDefFileName)

      !If no zone definition file is defined, return
      IF (cZoneDefFileName .EQ. '') RETURN
      
      !Read Z-Budget control data
      CALL MainControlFile%ReadData(cHDFFileName,iStat)  ;  IF (iStat .EQ. -1) RETURN    ;  cHDFFileName   = ADJUSTL(StripTextUntilCharacter(cHDFFileName,f_cInlineCommentChar))    ;  CALL CleanSpecialCharacters(cHDFFileName)
      CALL MainControlFile%ReadData(cOutFileName,iStat)  ;  IF (iStat .EQ. -1) RETURN    ;  cOutFileName   = ADJUSTL(StripTextUntilCharacter(cOutFileName,f_cInlineCommentChar))    ;  CALL CleanSpecialCharacters(cOutFileName)
      CALL MainControlFile%ReadData(cPrintInterval,iStat)  ;  IF (iStat .EQ. -1) RETURN  ;  cPrintInterval = ADJUSTL(StripTextUntilCharacter(cPrintInterval,f_cInlineCommentChar))  ;  CALL CleanSpecialCharacters(cPrintInterval)
      ...
      IF (iGetFileType_FromName(cHDFFileName) .NE. f_iHDF) CALL DefaultLogger%LogMessage(TRIM(cHDFFileName)//' is not an HDF5 file!',f_iFatal,ThisProcedure)
      CALL ZBudget%New(DefaultLogger,cHDFFileName,iStat)
      IF (iStat .EQ. -1) RETURN
      ...
      !Let the user know
      CALL DefaultLogger%LogMessage('Processing '//TRIM(LowerCase(ZBudget%Header%cDescriptor)),f_iMessage,'',Destination=f_iSCREEN_FILE)
      
      !Create the zone list
      CALL ZoneList%New(DefaultLogger,ZBudget%Header%iNData,ZBudget%Header%lFaceFlows_Defined,ZBudget%SystemData,TRIM(cZoneDefFileName),iStat)
      IF (iStat .EQ. -1) RETURN
      ...
      IF (iLoc .GT. 0) THEN
          CALL DefaultLogger%LogMessage('An undefined zone (-99) cannot be processed!',f_iWarn,ThisProcedure)
